...
TL;DR: 这是第一篇关于3D高斯溅射（3DGS）知识产权保护的系统性综述，提出了一个自下而上的分析框架，涵盖扰动机制、保护范式、鲁棒性威胁，并指出了技术基础和鲁棒性表征方面的研究空白和未来方向。


<details>
  <summary>Details</summary>
Motivation: 3DGS已成为实时3D场景合成的主流表示方法，但其商业价值和显式参数化结构引发了知识产权保护的新兴关切。目前研究进展分散，缺乏对底层机制、保护范式和鲁棒性挑战的统一视角。

Method: 提出了首篇系统性综述，引入自下而上框架分析：1）基于高斯的扰动机制；2）被动与主动保护范式；3）生成AI时代下的鲁棒性威胁。

Result: 揭示现有技术在理论基础和鲁棒性表征方面存在明显空白，为深入研究提供了机会窗口。

Conclusion: 从鲁棒性、效率、保护范式三个维度提出六个研究方向，为3DGS资产的可靠可信知识产权保护提供发展路线图。
...
TL;DR: TruKAN架构结合KAN结构和可学习激活函数，提出使用截断幂函数替代KAN中的B样条基，在保持表达力的同时提升准确率、训练效率与可解释性，在视觉任务上超越现有KAN变体。


<details>
  <summary>Details</summary>
Motivation: 解决计算效率与遵循Kolmogorov-Arnold Network (KAN)原理之间的权衡问题，即在保持KAN表达能力的同时提升准确率和训练效率，并增强模型的可解释性。

Method: 提出TruKAN架构：1) 用k阶样条理论中的截断幂函数族替代B样条基；2) 每层结合截断幂项与多项式项；3) 采用共享或独立节点；4) 集成到EfficientNet-V2框架中进行视觉任务评估；5) 使用混合优化提升收敛稳定性。

Result: 相比MLP、原始KAN及SineKAN等变体，TruKAN在复杂视觉任务上取得更高准确率、更佳计算效率和内存使用表现，且在共享与独立节点配置的实验中展示了灵活性。

Conclusion: TruKAN通过简化的基函数和节点配置平衡了近似效果与透明度，在计算效率、准确率和可解释性方面均优于现有KAN模型，超越了先前KAN研究中的有限设定。
...
TL;DR: 本文提出MQA-RefAVS任务，用于评估Ref-AVS中分割掩码的质量，包括IOU估计、错误分类和修复建议，并构建了基准数据集和方法。


<details>
  <summary>Details</summary>
Motivation: 当前语言引导视听分割主要生成掩码，但缺乏对掩码质量的丰富诊断。需要在不依赖真实标注的情况下评估分割质量，以支持错误检测和下游改进。

Method: 提出MQA-RefAVS任务，构建MQ-RAVSBench基准数据集，并设计基于多模态大语言模型的MQ-Auditor方法，通过显式推理生成量化评估和定性分析。

Result: MQ-Auditor显著优于现有的开源和商业MLLMs，能有效整合到现有Ref-AVS系统中，检测分割失败并支持改进。

Conclusion: MQA-RefAVS任务填补了视听分割质量评估的空白，提出的基准和方法为无参考掩码质量评估提供了有效解决方案，具有实际应用价值。
...
TL;DR: 作者提出了一种称为GPAIR的超快速迭代重建方法，用于3D光声计算机层析成像，通过使用连续各向同性高斯核转换空间网格，实现了计算加速，重建速度可达到亚秒级


<details>
  <summary>Details</summary>
Motivation: 传统迭代重建算法在处理大规模3D成像时重建时间过长（数百秒到数小时），计算负担严重限制了迭代重建算法的实际应用价值

Method: 提出GPAIR方法：1）使用连续各向同性高斯核转换传统空间网格；2）推导压力波的解析闭合形式表达式；3）实现强大的GPU加速可微分Triton算子

Result: 在含有840万体素的3D动物实验目标中，GPAIR表现出超快速的亚秒级重建速度，相比传统方法实现了数量级的计算加速

Conclusion: 这种革命性的超快速图像重建实现了近实时的大规模3D光声重建，显著推进了3D PACT的临床应用潜力
...
TL;DR: 该研究评估了Vision Transformer基础模型结合降维和聚类技术，用于无监督动物图像物种级聚类的效果，结果表明接近完美的分类性能，并能提取种内变异信息。


<details>
  <summary>Details</summary>
Motivation: 动物图像手工标记是生态研究的瓶颈，限制生物多样性监测的规模和效率，需要探索自动化的图像聚类方法。

Method: 建立了包含5个ViT模型、5种降维技术和4种聚类算法的综合基准框架，在60个物种上进行测试，每个物种使用200张已验证图像。

Result: 使用DINOv3嵌入结合t-SNE和监督层次聚类达到近乎完美的物种级聚类（V-measure: 0.958）；无监督方法也表现出竞争力（0.943），仅需剔除1.14%的异常图像。

Conclusion: 该方法能有效实现动物图像的物种级聚类，且能提取种内变异模式，为生态学家提供了实用的工具包和方法选择建议。
...
### [11] [Entropy-Aware Structural Alignment for Zero-Shot Handwritten Chinese Character Recognition](https://arxiv.org/abs/2602.03913)
...
### [12] [Phaedra: Learning High-Fidelity Discrete Tokenization for the Physical Science](https://arxiv.org/abs/2602.03915)
*Levi Lingsch,Georgios Kissas,Johannes Jakubik,Siddhartha Mishra*

Main category: cs.CV

TL;DR: 本文探讨了现有图像分词器(为视觉感知设计)对科学图像的适用性问题，提出了Phaedra模型以提升偏微分方程(PDE)数据重建和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 科学图像具有大动态范围，需要保持物理和光谱特性，而现有为视觉感知设计的tokenizer在处理这类数据时存在局限，无法同时捕捉细节和精确幅度。

Method: 提出Phaedra模型，受经典形状-增益量化和恰当正交分解启发，从物理和谱空间角度评估多种图像tokenizer的精度，并提出改进方案。

Result: Phaedra在多个PDE数据集上显著改善了重建效果，展现出强大的分布外泛化能力，适用于不同条件的已知PDE、未知PDE及真实地球观测和天气数据。

Conclusion: 针对科学图像的特性设计的Phaedra模型，比传统视觉tokenizer更能保持物理和光谱属性，为科学计算中的tokenization提供了更优方案。
...
### [13] [SpatiaLab: Can Vision-Language Models Perform Spatial Reasoning in the Wild?](https://arxiv.org/abs/2602.03916)
...
### [14] [Entropy Reveals Block Importance in Masked Self-Supervised Vision Transformers](https://arxiv.org/abs/2602.03918)
*Peihao Xiang,Kaida Wu,Ou Bai*

Main category: cs.CV

TL;DR: 该论文提出了Gardener，一种无需数据、一次性完成的块级剪枝方法，利用预训练权重信息熵评估ViT块的重要性，在多个下游视频识别基准上实现了高效模型压缩。


<details>
  <summary>Details</summary>
Motivation: 尽管masked self-supervised vision transformers在预训练中表现出色，但其庞大的模型规模给资源受限的部署和高效迁移学习带来挑战。关键问题是：是否所有transformer块对下游性能都同等重要？

Method: 通过信息熵分析预训练块权重，发现权重信息熵与通过迭代块移除和微调获得的oracle灵敏度强相关。基于此，提出了Gardener剪枝方法，通过简单的信息论测量识别冗余块，无需任何数据。

Result: 在VideoMAE-B上评估显示，即使在剪枝高达91.7%的块后，模型仍保持有竞争力的迁移性能。Gardener在计算开销可忽略的情况下，匹配或优于现有无数据剪枝基线，并接近基于灵敏度的剪枝方法。

Conclusion: masked self-supervised vision transformers中存在大量块级冗余，信息论分析为模型压缩和资源高效迁移学习提供了一条原则化和高效的途径。
...
### [15] [TiCLS : Tightly Coupled Language Text Spotter](https://arxiv.org/abs/2602.04030)
*Leeje Jang,Yijun Lin,Yao-Yi Chiang,Jerod Weinman*

Main category: cs.CV

TL;DR: TiCLS是一种端到端文本定位方法，通过显式整合字符级预训练语言模型的外部语言学知识来解决场景文本识别中文本片段化、模糊等问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视觉线索和局部字符依赖关系，但忽略了外部语言学知识的优势，同时以往的尝试存在语言模型目标与场景文本单词级粒度不匹配的问题。

Method: 提出TiCLS文本定位器，包含语言解码器，融合视觉和语言学特征。该解码器可通过预训练语言模型初始化，从而提升对模糊或片段化文本的识别鲁棒性。

Result: 在ICDAR 2015和Total-Text数据集上的实验表明，TiCLS达到最先进性能。

Conclusion: 预训练语言模型引导的语言学整合对场景文本定位是有效的。
...
TL;DR: 提出基于HLS的HW/SW协同设计方法，用于在FPGA上实现参数化的CNN加速器，以同时优化性能、延迟、功耗和面积等多个约束条件


<details>
  <summary>Details</summary>
Motivation: 现有FPGA CNN加速器主要关注GOPS性能，但实际嵌入式深度学习应用需要同时满足延迟、功耗、面积和成本等多个约束条件，传统方法难以有效平衡这些约束

Method: 硬件软件协同设计方法，使用HLS工具描述参数化的CNN加速器，便于设计参数调整和跨多个约束条件的优化

Result: 实验结果表明，提出的参数化设计方法优于非参数化设计方法，且可方便扩展到其他类型的深度学习应用

Conclusion: 基于HLS的参数化HW/SW协同设计方法有效解决了FPGA CNN加速器多约束优化问题，为嵌入式深度学习应用提供了灵活高效的实现方案
...
TL;DR: 本文介绍了HPA10M数据集和iSight框架，这是一个用于IHC图像自动评估的多任务学习模型，在多个评估指标上优于基础模型和初步病理学家人工评估。


<details>
  <summary>Details</summary>
Motivation: 尽管AI在H&E染色切片分析中已显示出潜力，但在IHC领域的应用受到领域特异性变化的限制，需要专门的数据集和模型来支持IHC评估和诊断。

Method: 1) 构建HPA10M数据集（包含1049.5万张IHC图像，涵盖45种正常组织和20种主要癌症类型）；2) 开发iSight多任务学习框架，结合WSI视觉特征和组织元数据，通过令牌级注意力机制同时预测染色强度、位置、数量、组织类型和恶性状态。

Result: 1) iSight在保留测试数据上实现了：位置85.5%、强度76.6%、数量75.7%的准确率，优于基础模型2.5-10.2%；2) 预测校准良好（误差0.0150-0.0408）；3) 在8位病理学家的用户研究中，iSight在HPA数据集上优于初步人工评估；4) AI辅助后病理学家间一致性提高（Cohen's κ从0.63/0.74提高到0.70/0.76）。

Conclusion: 该工作为提升IHC诊断准确性的AI系统奠定了基础，展示了iSight整合到临床工作流程中可提高IHC评估的一致性和可靠性，凸显了专家-AI协同评估在改善IHC判读中的潜力。
...
TL;DR: VideoBrain框架通过双智能体自适应采样，在减少30-40%帧数的同时，在长视频理解任务上实现3.5%-9.0%的性能提升


<details>
  <summary>Details</summary>
Motivation: 当前长视频理解面临计算约束与信息分布式存储的矛盾：均匀采样可能导致信息丢失，而单次关键帧选择无法纠正错误决策。现有基于文本LLM协调视觉工具的方法无法直接感知视频帧并进行推理

Method: 提出VideoBrain端到端框架：1）包含两个互补智能体：CLIP智能体用于跨视频语义检索，均匀智能体用于区间内密集时序采样；2）VLM直接感知帧并推理信息充分性；3）引入行为感知奖励函数和数据处理流程，避免智能体滥用

Result: 在四个长视频基准测试中：1）性能提升3.5%-9.0%；2）减少30-40%的帧使用量；3）在短视频基准上表现出强大的跨数据集泛化能力

Conclusion: VideoBrain通过自适应采样策略和双智能体架构，有效解决了长视频理解中信息获取与计算效率的平衡问题，证明了VLM直接感知和推理的价值
...
TL;DR: 本研究针对材料图像分割提出跨模态评估框架，比较不同架构在多种成像模式下的性能差异，为实际部署提供架构选择、可靠性评估和可解释性工具。


<details>
  <summary>Details</summary>
Motivation: 现有分割架构通常在单一成像模态上评估，忽略了实际部署中不同成像模式（如SEM、AFM、XCT、光学显微镜）的性能差异，材料研究人员缺乏针对特定成像设置选择合适架构的工具，也无法评估模型在新样本上的可信度。

Method: 1. 建立跨模态评估框架，涵盖SEM、AFM、XCT和光学显微镜四种成像模式；2. 评估六种编码器-解码器组合在七个数据集上的分割性能；3. 通过分布外检测和反事实解释提供部署反馈，揭示影响预测的微观结构特征。

Result: 1. 最优架构随成像上下文系统变化：UNet在高对比度2D成像中表现最佳，DeepLabv3+在最具挑战性的案例中表现最优；2. 框架为实际部署提供了架构选择指导；3. 通过分布外检测和反事实解释增强了模型可靠性和可解释性。

Conclusion: 该研究填补了材料表征领域的实用空白，为研究人员提供了针对特定成像设置选择分割架构的工具，以及评估模型在新样本上可信度的可靠性指标和可解释性方法，有助于推动材料图像分割在实际应用中的可靠部署。
...
TL;DR: 提出首个同时解决事件视频建模中表示冗余、结构冗余及频谱信息利用不足三大挑战的紧凑全息表示架构，通过将3D时空信息嵌入2D表示实现高效率动作识别。


<details>
  <summary>Details</summary>
Motivation: 解决现有事件视频动作识别方法的三大不足：1）稠密体素表示的运算冗余；2）多分支架构的结构冗余；3）全局运动模式中频谱信息利用率低。

Method: 1）提出紧凑全息时空表示（CHSR），将水平空间信息嵌入时间-高度视图，用2D表示保留3D时空上下文；2）设计全局频谱门控（GSG）模块，在频域通过FFT进行全局令牌混合以增强表示能力。

Result: HoloEv-Net-Basic在三大数据集上分别比现有方法提升10.29%、1.71%、6.25%；轻量版HoloEv-Net-Small参数量减少5.4倍，FLOPs减少300倍，延迟降低2.4倍，同时保持极具竞争力的准确率。

Conclusion: HoloEv-Net同时解决了事件视频动作识别的三大核心挑战，实现了高效高性能的事件视频建模，其轻量版本特别适用于边缘设备部署。
...
TL;DR: DiMo是一个基于离散扩散的框架，通过迭代掩码标记细化实现文本与运动之间的双向理解和生成，统一了文本到运动、运动到文本和无文本运动到运动任务。


<details>
  <summary>Details</summary>
Motivation: 现有的掩码建模运动生成方法主要关注文本到运动任务，作者希望拓展到双向文本-运动理解和生成，并统一多个任务在一个模型中。

Method: 使用离散扩散风格的迭代掩码标记细化方法，结合残差向量量化提高运动标记保真度，以及组-相对策略优化增强对齐和可控性。

Result: 在HumanML3D和KIT-ML数据集上的实验表明DiMo具有强大的运动质量和有竞争力的双向理解能力，同时展示了无文本运动补全、文本引导运动预测和运动描述修正的能力。

Conclusion: DiMo框架通过统一的离散扩散方法实现了文本与运动之间的双向理解和生成，为运动生成和相关任务提供了灵活且高质量的解决方案。
...
TL;DR: VTok是一个统一视频标记化框架，通过分离空间和时间表示来压缩视频表征复杂度，在视频理解和生成任务中均表现优异，同时显著降低序列长度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统通过简单帧采样策略进行视频标记化，缺乏对视频时空关系的有效建模，导致计算复杂度高且表达能力有限。VTok旨在通过更紧凑高效的视频表示方法来解决这一问题。

Method: 提出分离视频的空间和时间表示：保留单个关键帧的空间特征，将后续每帧编码为单个残差标记，从而将复杂度从帧数×每帧标记数的乘积降为两者的和。

Result: 在视频理解和文本到视频生成基准测试中表现显著优于基线方法，例如在TV-Align基准上准确率提升3.4%，在VBench得分上提升1.9%，同时每视频标记序列更短，运动更连贯且指令跟随能力更强。

Conclusion: VTok提供了一个标准化的视频标记化范式，能够有效平衡表达能力和计算效率，为未来视频理解与生成研究提供了新的框架。
...
TL;DR: 针对现有视频自编码器存在的固定压缩率、CNN架构不灵活和解码器确定性过强问题，本文提出了基于Transformer的一维扩散视频自编码器框架，支持自适应压缩并改进了解码质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视频自编码器存在三个主要问题：(1)固定比特率压缩，简单视频浪费词汇量；(2)CNN架构不灵活，无法处理可变长度潜变量建模；(3)解码器确定性过强，从压缩潜变量中恢复细节困难。这些问题限制了视频生成模型的效果和灵活性。

Method: 提出One-DVA框架：1）使用基于查询的视觉Transformer提取时空特征；2）引入可变长度dropout机制动态调整潜变量长度；3）采用像素空间的扩散Transformer作为解码器，将潜变量作为条件输入；4）使用两阶段训练策略进行优化。

Result: One-DVA在相同压缩比下达到了与3D-CNN VAE相当的重建指标性能，同时支持自适应压缩，能够实现更高压缩比。通过正则化潜变量分布和微调解码器，进一步提升了生成建模的效果。

Conclusion: One-DVA框架有效解决了现有视频自编码器的局限性，通过Transformer架构、自适应压缩机制和扩散解码器，显著提升了视频压缩和生成的能力，为下游生成任务提供了更好的支持。
...
TL;DR: SPOT-Occ是一种基于原型引导的稀疏Transformer解码器，用于高效实现3D占用预测，在显著提高速度的同时保证精度。


<details>
  <summary>Details</summary>
Motivation: 从相机实现高精度实时3D占用预测对自动驾驶安全部署至关重要。虽然转向稀疏3D表示解决了编码瓶颈，但解码器面临着如何高效聚合稀疏非均匀体素特征的难题，现有密集注意力方法计算成本过高。

Method: 提出原型基础的稀疏Transformer解码器，采用两阶段过程：1）通过稀疏原型选择机制自适应识别最显著的体素特征作为原型；2）引入基于真实掩码的去噪范式，确保查询-原型关联的稳定性和一致性。

Result: SPOT-Occ模型在速度上显著优于先前方法，同时提升了预测精度。开源代码已发布。

Conclusion: 原型引导的稀疏注意力机制有效解决了3D占用预测中稀疏特征聚合的计算效率问题，为自动驾驶中的实时3D感知提供了高效解决方案。
...
TL;DR: 本文提出了一种深度引导的框架来解决单目视频人体网格恢复中的度量一致性和时间稳定性问题，通过深度融合几何先验和多尺度特征来提高对深度模糊、尺度漂移和遮挡的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 单目视频人体网格恢复面临深度模糊和尺度不确定性导致的度量一致性和时间稳定性挑战，现有方法主要依赖RGB特征和时间平滑，在深度排序、尺度漂移和遮挡引发的不稳定性方面存在困难。

Method: 方法包含三个协同组件：1)深度引导的多尺度融合模块，通过置信度感知门控自适应整合几何先验和RGB特征；2)深度引导的度量感知姿态形状估计器，利用深度校准的骨骼统计进行尺度一致初始化；3)运动-深度对齐细化模块，通过运动动态和几何线索的跨模态注意力强制时间一致性。

Result: 在三个具有挑战性的基准测试中取得优异结果，在重度遮挡下的鲁棒性和空间精度方面显示显著提升，同时保持计算效率。

Conclusion: 提出的深度引导框架有效解决了单目视频人体网格恢复中的度量一致性和时间稳定性问题，通过深度融合几何先验和运动动态实现了更好的空间精度和时间连贯性。
...
TL;DR: 该论文提出了一种新颖的解耦分层多模态蒸馏框架DHMD，通过将每个模态特征分解为模态无关和模态独有成分，采用两阶段知识蒸馏策略来提升多模态情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态情感识别方法难以有效处理多模态异质性和不同模态贡献度差异问题，需要更灵活的知识迁移和跨模态特征对齐机制。

Method: 1. 使用自回归机制将每个模态特征解耦为模态无关和模态独有成分；2. 设计两阶段知识蒸馏：粗粒度基于图蒸馏单元在解耦特征空间中进行动态图蒸馏，细粒度通过跨模态字典匹配对齐语义粒度。

Result: 在CMU-MOSI/CMU-MOSEI数据集上相对SOTA方法分别提升ACC$_7$:1.3%/2.4%、ACC$_2$:1.3%/1.9%、F1:1.9%/1.8%；可视化显示图边和字典激活在特征空间中呈现有意义的分布模式。

Conclusion: DHMD框架通过解耦特征分层蒸馏有效解决了多模态异质性问题，实现了灵活的知识迁移和精准的特征对齐，显著提升了多模态情感识别性能。
...
TL;DR: KVSmooth提出了一种无需训练即插即用的方法，通过基于注意力熵引导的自适应平滑来缓解多模态大语言模型中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型尽管在各项任务中取得显著进展，但幻觉问题（生成视觉不一致的对象、属性或关系）仍然是其可靠部署的主要障碍。现有模型在解码过程中常出现语义漂移，导致输出随着序列长度增加而偏离视觉事实。

Method: KVSmooth对KV-Cache中的键和值应用指数移动平均（EMA），同时通过每个令牌注意力分布的熵动态量化其沉降程度，以自适应调整平滑强度。这是一种基于注意力熵引导的自适应隐藏状态平滑方法。

Result: KVSmooth显著减少了幻觉（CHAIR_S从41.8降至18.2），同时提高了整体性能（F1分数从77.5提升至79.2），实现了精度和召回率的同步提升。相比之下，先前的方法往往以牺牲一方来提升另一方。

Conclusion: KVSmooth是一种高效的无训练推理方法，无需额外训练或模型修改，能有效缓解多模态大语言模型的幻觉问题，具有较高的有效性和通用性。
...
TL;DR: 提出多模态自表示学习（MSRL）方法，通过利用异构多视角特征的自表示特性来学习不变表示，无需有监督微调。


<details>
  <summary>Details</summary>
Motivation: 解决在不同预训练模型下，相同样本的特征分布存在固有差异的问题，探索在大规模无标注视觉数据中学习不变表示的挑战。

Method: 1.使用多种预训练模型从大规模无标注数据中提取异构多视角特征；2.在每个冻结的预训练骨干上堆叠独立的线性模型；3.引入基于自表示学习的信息传递机制支持特征的聚合；4.提出分配概率分布一致性方案，利用不同视角的互补信息指导多视角自表示学习；5.通过该方案增强不同线性模型间表示的不变性。

Result: 在多个基准视觉数据集上的大量实验表明，所提出的MSRL方法持续优于多种最先进的方法。

Conclusion: MSRL方法能有效学习多视角不变表示，通过信息传递机制和分配概率分布一致性，在大规模无监督视觉数据上取得了优越的性能。
...
TL;DR: CoFT框架通过双提示学习和协作过滤实现无监督视觉-语言模型适应，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前大规模视觉语言模型（如CLIP）具有强大的零样本泛化能力，但适应下游任务通常需要昂贵的标注数据。现有的无监督自训练方法依赖伪标签，但常面临伪标签可靠性差、确认偏差以及低置信度样本未充分利用的问题。

Method: 提出协作微调（CoFT）框架，通过双模型跨模态协作机制利用无标签数据。核心是双提示学习策略——使用正负文本提示来建模样本依赖的伪标签清洁度，无需手动阈值或噪声假设。负提示还作为正则化项提升噪声监督下的鲁棒性。采用两阶段训练：从高效参数微调过渡到基于协作过滤伪标签的全模型微调。CoFT+进一步通过迭代微调、动量对比学习和LLM生成提示增强适应性。

Result: 大量实验表明，CoFT在无监督视觉-语言适应任务上持续优于现有无监督方法，甚至超过少样本监督基线。

Conclusion: CoFT框架通过双提示学习和协作过滤机制，有效解决了无监督自训练中的伪标签不可靠和样本利用不足问题，为大规模视觉语言模型的无监督适应提供了创新方法。
...
TL;DR: 提出基于双提示调优的主动CLIP适配不确定性建模框架，通过正负提示分别增强分类判别性和不确定性估计，在有限标注预算下优于现有主动学习方法。


<details>
  <summary>Details</summary>
Motivation: CLIP等预训练视觉语言模型虽具强迁移性，但在有限标注预算下适配下游图像分类任务仍具挑战。现有主动学习方法多依赖基于熵的标准或表示聚类，未从模型角度显式建模不确定性。

Method: 在CLIP文本分支引入两个可学习提示：正提示增强与轻量调优的视觉嵌入对应的任务特定文本嵌入的判别性；负提示以反向方式训练，显式建模预测正确的概率，为主动样本选择提供不确定性信号。

Result: 在不同微调范式下的大量实验表明，该方法在相同标注预算下持续优于现有主动学习方法。

Conclusion: 通过双提示调优框架为CLIP主动适配提供基于模型的不确定性建模，提升了有限标注下的分类性能。
...
TL;DR: VecSet-Edit是基于VecSet大型重建模型的首个网格编辑流程，提出掩码引导、注意力对齐和漂移感知策略实现高精度3D网格编辑。


<details>
  <summary>Details</summary>
Motivation: 当前3D编辑方法主要聚焦于3D高斯泼溅或多视图图像，直接网格编辑研究不足；先前体素方法存在分辨率限制和手动3D掩码标注繁琐的问题。

Method: 1. 分析VecSet token空间特性，发现子集控制不同几何区域
2. 提出掩码引导token播种和注意力对齐token门控策略，仅用2D图像条件精确定位目标区域
3. 设计漂移感知token修剪机制，在去噪过程中排除几何异常值
4. 开发细节保持纹理烘焙模块，保留原始网格几何细节和纹理信息

Result: 建立首个基于VecSet LRM的网格编辑流程，实现仅需2D图像条件的高精度局部编辑，同时保持原始网格细节完整性。

Conclusion: VecSet-Edit通过分析token空间特性并设计针对性策略，成功克服了体素方法的局限性，为高质量3D网格编辑提供了有效解决方案。
...
TL;DR: 提出了SAGA（分阶段注意力引导攻击）框架，通过渐进式聚焦在LVLMs的高度力区域来实现更高效的对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 针对大视觉语言模型（LVLMs）的对抗攻击对揭示多模态系统的安全漏洞至关重要。基于输入变换的攻击（如随机裁剪）表明局部扰动可能比全局图像操作更有效，但这些方法具有随机性和低效性。本文旨在开发更高效的注意力引导攻击方法。

Method: 提出SAGA框架：基于两个重要观察（1）区域注意力分数与对抗损失敏感性正相关；（2）攻击高注意力区域会引起注意力向后续显著区域的结构化重新分配。SAGA通过渐进式聚焦在高注意力区域，在限制的像素扰动预算内更高效地生成对抗样本。

Result: SAGA在10个不同的LVLMs上一致实现了最先进的攻击成功率，同时产生高度不可察觉的对抗样本，源代码已开源。

Conclusion: SAGA通过注意力引导的渐进式攻击策略，在受限扰动预算下实现了更高效的对抗攻击，为评估LVLMs的安全鲁棒性提供了有效工具。
...
TL;DR: UltraSeg系列是用于结肠镜息肉分割的超轻量模型家族，包括针对单中心数据的UltraSeg-108K（0.108M参数）和多中心多模态图像的UltraSeg-130K（0.13M参数），能在单核CPU上达到90FPS且保持高精度。


<details>
  <summary>Details</summary>
Motivation: 结直肠癌早期检测依赖于实时、准确的息肉识别和切除，但当前高精度分割模型依赖GPU，无法在基层医院、移动内窥镜单元或胶囊机器人中部署。需要为资源受限环境提供可行解决方案。

Method: 通过联合优化编码器-解码器宽度、引入约束扩张卷积扩大感受野、集成跨层轻量融合模块，在极低压缩（<0.3M参数）情况下实现模型优化。

Result: 在7个公共数据集上评估，UltraSeg保持了31M参数U-Net 94%以上的Dice分数，仅使用其0.4%的参数量，单核CPU上达到90FPS。

Conclusion: UltraSeg为结肠镜提供了CPU原生解决方案，为更广泛的微创手术视觉应用提供可重复蓝图，适用于资源受限的临床环境。
...
TL;DR: 提出了一个名为ISFM的创新多模态图像融合框架，它通过交互式空间-频率融合机制增强特征表示，在六个数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法虽然引入频域信息来增强空间特征，但通常采用简单的串行或并行融合策略，缺乏空间和频率特征之间的有效交互，限制了信息融合效果。

Method: 1）使用模态特定提取器（MSE）提取各模态特征；2）提出多尺度频率融合（MFF）自适应整合多尺度高低频分量；3）核心创新是交互式空间-频率融合（ISF），用频率特征指导跨模态空间特征学习。

Result: 在六个多模态图像融合数据集上的实验结果表明，ISFM框架在保留纹理细节和显著信息方面均优于现有的最先进方法。

Conclusion: ISFM通过引入交互式空间-频率融合机制，有效提升了多模态图像融合的性能，证明了空间和频率特征交互的重要性。
...
TL;DR: 提出了Med-MMFL基准，这是首个面向医疗领域的多模态联邦学习全面基准，涵盖了多种模态、任务和联邦场景，旨在推动医疗多模态联邦学习的标准化评估。


<details>
  <summary>Details</summary>
Motivation: 当前医疗联邦学习领域缺乏标准化Benchmark，现有工作主要关注单模态或双模态，且任务类型有限，制约了医疗MultiModal FL（MMFL）的系统性理解和进展，因此需要建立全面、标准化的评估基准。

Method: 设计了Med-MMFL基准，涵盖2-4种模态的10种独特医疗模态（包括文本、病理图像、ECG、X射线、放射学报告、多种MRI序列），评估6种代表性FL算法；在多种联邦场景（自然联邦、合成IID、合成非IID）下进行实验；评估分割、分类、模态对齐（检索）和VQA等任务。

Result: 建立了首个医疗领域多模态联邦学习综合基准。发布了完整的基准实现（包括数据处理和分区管道）以支持可重复性和公平比较。

Conclusion: Med-MMFL基准填补了医疗多模态联邦学习标准化评估的空白，为未来MMFL方法在真实医疗场景下的公平比较和可重复研究提供了重要基础，有望推动该领域的系统性进展。
...
TL;DR: TrajVG是一个通过估计3D轨迹实现跨帧对应关系的三维重建框架，解决了传统方法在物体运动视频中的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统前馈式多帧3D重建模型在物体运动视频中表现不佳。全局参考在多重运动下变得模糊，而局部点云图过度依赖估计的相对位姿且容易漂移，导致跨帧错位和结构重复。

Method: 通过估计相机坐标下的3D轨迹来显式预测跨帧3D对应关系。耦合稀疏轨迹、每帧局部点图和相对相机位姿，使用几何一致性目标：1）具有控制梯度流的双向轨迹-点图一致性；2）由静态跟踪锚点驱动的位姿一致性目标，抑制动态区域的梯度。采用混合监督训练，将相同的耦合约束重新表述为仅使用伪2D轨道的自监督目标。

Result: 在3D跟踪、位姿估计、点图重建和视频深度等多个任务的广泛实验中，TrajVG超越了当前的前馈性能基准。

Conclusion: TrajVG通过显式建模3D轨迹和几何一致性约束，显著提升了动态场景下三维重建的准确性和鲁棒性。
...
TL;DR: 研究通过模拟人类视觉经验，发现结合中央视觉和时序稳定性学习能提升物体语义表征的编码能力——中央视觉强化前景物体特征提取，而时序稳定性（尤其是在注视眼动期间）有助于编码更广泛的物体语义信息。


<details>
  <summary>Details</summary>
Motivation: 人类能在仅有极少监督的情况下从视觉流中习得物体语义表征。视觉系统通常以高分辨率处理视野中心区域，并对时间上相邻的视觉输入学习相似的表征，这强调了注视点周围缓慢变化信息的重要性。本研究旨在探究中央视觉和时序稳定性学习在基于人类视觉经验形成语义物体表征中的作用。

Method: 使用Ego4D数据集模拟五个月的人类视觉经验，并用最先进的眼动预测模型生成注视坐标。基于这些预测提取模拟中央视觉的裁剪区域，并在这些区域上训练时间对比自监督学习模型。

Result: 结合时序稳定性和中央视觉，模型能更好地编码物体表征的不同语义层面：中央视觉有助于提取前景物体特征，而时序稳定性（特别是在注视眼动期间）使得模型能编码更广泛的物体语义信息。

Conclusion: 这些发现为解释人类如何从自然视觉经验中发展语义物体表征提供了新的机制性见解。
...
TL;DR: 针对MLLM在多步推理任务中视觉信息逐渐稀释的问题，研究者提出了Vision-aligned Latent Reasoning（VaLR）框架，通过动态生成视觉对齐的潜在标记来指导模型在潜在空间中进行推理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）虽然在多种理解任务上取得了进展，但在需要大量多步骤推理的问题上表现不佳。这主要是因为长上下文生成过程中视觉信息逐渐被稀释，导致模型无法充分利用测试时扩展能力。为了解决这个问题，研究者需要开发一个能有效保留视觉知识并进行多步推理的框架。

Method: 提出了Vision-aligned Latent Reasoning（VaLR）框架，在链式思维推理的每一步之前动态生成视觉对齐的潜在标记，引导模型在潜在空间基于感知线索进行推理。VaLR通过将MLLM的中间嵌入与视觉编码器的嵌入进行对齐，来训练模型在推理过程中保留视觉知识。

Result: 实证结果表明，VaLR在一系列需要长上下文理解或精确视觉感知的基准测试中，始终优于现有方法。特别是在VSI-Bench上，性能从33.0%显著提升至52.9%，相对于Qwen2.5-VL实现了19.9%的性能提升，并展现出之前MLLM中未观察到的测试时扩展行为。

Conclusion: VaLR是一个简单而有效的推理框架，通过视觉对齐的潜在空间推理，成功解决了MLLM在长上下文多步推理任务中视觉信息稀释的问题，显著提升了模型在复杂视觉推理任务上的性能。
...
TL;DR: 这是一个医学影像基础模型OmniRad，在120万医疗图像上通过自监督预训练，强调特征复用和跨任务迁移性，在多种下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像分析需要能够支持多种成像模态和异构下游任务的预训练视觉表征。为了改进特征复用和跨任务迁移性，需要设计专门针对放射学的预训练模型。

Method: 开发了OmniRad模型，在120万医疗图像上进行自监督预训练，采用放射学设计原则。在下游评估中采用了多种适应策略：包括使用轻量级任务特定适配器和冻结主干，以及端到端微调进行分类任务。

Result: 在MedMNISTv2集合中，OmniRad比竞争基础模型提升了最多2.05%的分类F1分数。在密集预测任务中，在六个MedSegBench数据集上使用冻结表征时实现了平均Dice分数的改进。定性分析和潜在空间可视化显示了改进的特征聚类和模态相关分离。

Conclusion: OmniRad作为放射学基础模型，证明了其在多种医疗影像任务中具有出色的表征质量和跨任务迁移能力，为医疗影像分析提供了强大的预训练基础。
...
TL;DR: 论文提出DU-VLM方法，将视觉降质理解重构为层次化结构预测任务，统一多模态链式思维与强化学习进行训练，并可用于零样本引导扩散模型进行图像恢复。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在定性描述图像降质方面表现出色，但在理解降质背后的参数物理机制方面存在不足，需要同时估计降质类型、参数键及连续物理值。

Method: 将降质理解重构为层次化结构预测任务，提出DU-VLM多模态链式思维模型，通过监督微调和带结构化奖励的强化学习训练，误差受值空间量化网格约束。

Result: DU-VLM在准确性和鲁棒性上显著优于通用基线，能够泛化到未见分布，并可作为零样本控制器引导预训练扩散模型实现高保真图像恢复。

Conclusion: 该工作通过统一的自回归令牌预测范式解决了视觉降质理解的层次化结构预测问题，提出的DU-VLM模型展现出优越性能，同时构建的DU-110k数据集为未来研究提供了重要资源。
...
TL;DR: ImmuVis是一个用于成像质谱流式细胞术（IMC）的高效卷积基础模型，通过标记自适应超卷积技术处理不同研究中的可变标记集，在自监督预训练后表现出优越的性能和计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型假设固定的通道空间，但多路成像技术（如IMC）中的分子标记集在实际研究中存在差异，这使得标准视觉主干无法直接应用，需要开发能够处理任意标记子集的通用模型。

Method: 1. 提出标记自适应超卷积技术，通过学习标记嵌入生成卷积核，使单个模型能够处理任意测量标记子集而无需重新训练。
2. 在最大IMC数据集IMC17M（28个队列，24,405张图像，265个标记，超1700万个图像块）上进行自监督掩码重建预训练。
3. 采用异方差似然目标实现校准的不确定性估计。

Result: 1. 在虚拟染色和下游分类任务中优于现有SOTA方法和消融实验。
2. 计算成本显著低于基于Transformer的替代方案。
3. 是唯一能够通过异方差似然目标提供校准不确定性的模型。

Conclusion: ImmuVis作为一个实用、高效的基础模型，为真实世界的IMC建模提供了有效的解决方案，能够适应不同研究中的标记集变化，在保持高性能的同时显著降低计算成本。
...
TL;DR: 该数据集包含11,884张标注图像，记录了在训练手臂上模拟采血（静脉穿刺）过程，包含注射器、止血带、消毒湿巾、手套和训练手臂五个医学相关类别的多边形标注，划分为训练/验证/测试集，可用于医疗训练自动化和人机交互研究。


<details>
  <summary>Details</summary>
Motivation: 当前医疗培训中缺乏高质量、结构化的视觉数据集来支持自动化评估和反馈系统。该数据集的创建旨在推动医疗程序自动化监控、技能评估和教育工具的发展，特别是针对静脉穿刺等基础临床操作。

Method: 从高清视频中提取图像，应用结构相似性指数（SSIM）过滤减少冗余，进行自动人脸匿名化处理，使用多边形标注五个关键类别，并导出为兼容YOLOv8等现代目标检测框架的分割格式。

Result: 创建了一个包含11,884张标注图像的高质量数据集，分为训练集（70%）、验证集（15%）和测试集（15%），所有数据和标注文件已在Zenodo平台公开可用。

Conclusion: 该数据集为医疗培训自动化和人机交互研究提供了重要的基础资源，支持多种应用如工具检测、流程识别、工作流分析和教育系统开发，有助于提高医学培训的质量和效率。
...
TL;DR: AGILE从动作生成而非直接重建的角度，提出解决单目视频中手-物交互重建的两个痛点：1）神经渲染常因遮挡导致几何碎裂且无法用于仿真；2）基于SfM的初始化对野外视频鲁棒性差。


<details>
  <summary>Details</summary>
Motivation: 当前从单目视频重建手-物交互的方法面临两大障碍：一是依赖神经渲染容易在遮挡严重的场景中产生碎裂的、无法用于仿真的几何体；二是依赖脆弱的SfM初始化导致处理野外拍摄视频时频繁失败。需要一种更稳健的方法来生成可用于仿真的物理有效资产。

Method: AGILE框架采用动作生成的范式：1）使用VLM引导生成模型合成完整、水密的物体网格和高保真纹理，不受视频遮挡影响；2）跳过脆弱的SfM，提出基于锚点跟踪的策略，在交互起始帧用基础模型初始化物体姿态并通过生成的资产与视频的视觉相似度进行时序传播；3）借助接触感知的优化融合语义、几何和交互稳定性约束以保证物理合理性。

Result: 在HO3D、DexYCB和野外视频数据集上的大量实验表明，AGILE在全局几何精度上优于基线方法，在挑战性序列中展现出卓越的鲁棒性（之前的方法常失败）。

Conclusion: AGILE通过优先考虑物理有效性，生成了可用于仿真的资产，为机器人应用提供了有效的真实到仿真的重定向能力。
...
TL;DR: 提出了RGBD Referring Multi-Object Tracking任务，结合RGB、深度和语言模态实现3D感知的目标跟踪


<details>
  <summary>Details</summary>
Motivation: 现有的RMOT模型仅依赖2D RGB数据，在复杂空间语义（如'最靠近相机的人'）下准确检测和关联目标困难，且在严重遮挡下难以保持可靠身份识别，缺乏显式的3D空间信息

Method: 提出DRTrack框架，通过多模态大语言模型引导实现深度感知的目标定位，结合RGB、深度和语言输入，并利用深度信息增强轨迹关联的鲁棒性

Result: 构建了DRSet数据集，包含187个场景的RGB图像和深度图，240条语言描述，其中56条包含深度相关信息。在DRSet上的实验证明了所提框架的有效性

Conclusion: 提出DRMOT新任务和DRTrack框架，解决了传统RMOT在3D空间语义建模和遮挡情况下的局限性，为交互式AI系统的目标跟踪提供了更准确的解决方案
...
TL;DR: 本文提出了一种基于视觉语言模型（VLM）的无标注空间目标检测与分割方法，通过伪标签生成和师生蒸馏框架，在无需人工标注的情况下显著提升了航天器分割性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在开放世界零样本视觉识别方面表现优异，但在空间应用领域的潜力尚未充分探索。空间领域的人工标注特别困难，主要因为低可见度、光照变化和物体与行星背景融合等因素，因此开发无需大量人工标注的空间目标检测与分割方法至关重要。

Method: 提出了基于VLM的无标注检测与分割流程：首先使用预训练的视觉语言模型为少量未标注的真实数据自动生成伪标签，然后将这些伪标签应用于师生标签蒸馏框架来训练轻量级模型。尽管伪标签存在噪声，但蒸馏过程相比直接零样本VLM推理仍能带来显著性能提升。

Result: 在SPARK-2024、SPEED+和TANGO数据集上的分割任务评估显示，该方法能在平均精度（AP）上实现最高10个百分点的提升，证明了其在空间目标分割任务中的有效性。

Conclusion: 该方法为空间目标检测与分割提供了一种有效的无标注解决方案，通过伪标签生成和蒸馏框架，在多项基准数据集上取得了优于直接零样本VLM推理的性能，为空间应用中的视觉识别任务开辟了新途径。
...
TL;DR: XtraLight-MedMamba：基于状态空间的超轻量级深度学习框架，用于从WSI中分类肿瘤性管状腺瘤，参数仅约3.2万，准确率达97.18%


<details>
  <summary>Details</summary>
Motivation: 常规结肠镜检查中癌前息肉的准确风险分层对降低结直肠癌风险至关重要，但低级别异型增生的评估受限于主观组织病理学解释。数字病理学和深度学习的发展为识别与恶性进展相关的细微形态学模式提供了新机会

Method: 提出XtraLight-MedMamba框架，结合ConvNext浅层特征提取器和并行视觉Mamba来建模长短程依赖；集成空间与通道注意力桥模块增强多尺度特征提取；采用固定非负正交分类器实现参数减少和泛化提升

Result: 在使用约32,000参数的条件下，模型在低级别管状腺瘤数据集上达到97.18%准确率和0.9767 F1分数，显著优于参数复杂度更高的transformer和传统Mamba架构

Conclusion: XtraLight-MedMamba作为一种超轻量级深度学习框架，能有效识别WSI中与恶性进展相关的细微形态学模式，为结直肠癌风险分层提供了一种参数高效且性能优异的解决方案
...
TL;DR: 本文提出LitS，一种用于2D和3D点云的新型邻域描述符，通过单位圆上的分段常数函数记录点的局部几何信息，能够适应不同点云类型并抵抗密度变化和噪声。


<details>
  <summary>Details</summary>
Motivation: 随着3D扫描技术的发展，点云成为表示3D空间数据的基础形式，而其分析严重依赖于能够准确表征局部几何特征的邻域描述符。现有方法在处理可变密度和噪声等问题时存在局限性，需要一种更健壮且适应性强的描述符。

Method: LitS是基于局部参考系的方向性邻域描述符，在单位圆上定义为分段常数函数。每个方向对应一个锥形区域，函数值表示该方向的邻居数量。提供'常规'和'累积'两种版本，包含两个可调参数以适应不同场景。通过分析相邻点之间LitS的变化可以获得全局结构信息。

Result: LitS能够有效捕捉点云局部排列的细微特征，对常见的数据问题如密度变化和噪声具有鲁棒性。该描述符在多种点云类型和不同应用场景中表现出良好的适应性。

Conclusion: LitS是一种多功能的邻域描述符，能够准确表征点云的局部几何特征，并通过局部到全局的分析框架增强对点云结构的理解。其参数化设计和两种变体使其能够适应广泛的点云分析任务。
...
### [75] [Laminating Representation Autoencoders for Efficient Diffusion](https://arxiv.org/abs/2602.04873)
*Ramón Calvo-González,François Fleuret*

Main category: cs.CV

TL;DR: FlatDINO：一种变分自编码器，将DINOv2的密集patch特征压缩为1D序列（32个token），实现8倍序列长度缩减和48倍维度压缩，降低扩散模型计算成本


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在SSL patch特征（如DINOv2）上生成高质量图像，但这些密集patch网格包含大量冗余，导致扩散计算成本昂贵

Method: 提出FlatDINO变分自编码器，将DINOv2特征压缩为一维连续token序列（仅32个token），实现特征压缩后训练扩散Transformer（DiT-XL）

Result: 在ImageNet 256x256上，FlatDINO潜在空间训练的DiT-XL获得gFID 1.80（使用分类器自由引导），前向传播FLOPs减少8倍，训练步骤FLOPs最多减少4.5倍

Conclusion: FlatDINO能有效压缩SSL特征表示，大幅降低扩散模型计算成本，同时保持生成质量，这是初步结果，工作仍在进行中
...
### [76] [PerpetualWonder: Long-Horizon Action-Conditioned 4D Scene Generation](https://arxiv.org/abs/2602.04876)
...
### [77] [CoWTracker: Tracking by Warping instead of Correlation](https://arxiv.org/abs/2602.04877)
...
### [78] [C-IDS: Solving Contextual POMDP via Information-Directed Objective](https://arxiv.org/abs/2602.03939)
*Chongyang Shi,Michael Dorothy,Jie Fu*

Main category: eess.SY

TL;DR: 本文提出了一种在上下文部分可观测马尔可夫决策过程中同时优化累积回报和主动减少潜在上下文不确定性的策略合成方法，通过C-IDS算法实现信息导向的目标优化。


<details>
  <summary>Details</summary>
Motivation: 传统POMDP方法在处理未知上下文时通常将其视为潜状态变量，但这种方法可能无法有效识别上下文并获取高回报。作者希望设计既能最大化累积回报又能主动减少上下文不确定性的策略。

Method: 提出信息导向目标函数，在奖励最大化基础上增加潜在上下文与智能体观测间的互信息项。开发C-IDS算法来合成最大化该目标的策略，将目标解释为线性信息比的拉格朗日松弛，并证明温度参数是信息比的上界。

Result: 建立了在K个episode上的次线性贝叶斯遗憾界。在连续Light-Dark环境中的实验表明，该方法在上下文识别速度和回报方面均优于将未知上下文作为潜状态变量的标准POMDP求解器。

Conclusion: 本文提出的信息导向方法和C-IDS算法能够有效处理CPOMDP中的策略合成问题，通过同时考虑奖励最大化和不确定性减少，实现了更快的上下文识别和更高的累积回报。
...
### [79] [Safety-Critical Reinforcement Learning with Viability-Based Action Shielding for Hypersonic Longitudinal Flight](https://arxiv.org/abs/2602.03968)
*Hossein Rastgoftar*

Main category: eess.SY

TL;DR: 本文提出了一个面向非线性动力系统的安全关键型强化学习框架，结合动作屏蔽和可达性分析确保持续满足硬安全约束，并通过状态分区与模式相关奖励实现正常跟踪和恢复行为的统一控制。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在安全关键系统中常面临奖励机制与硬安全约束的冲突问题，难以保证在连续状态/输入空间下的绝对安全性，同时需要兼顾正常操作和安全恢复两种模式的统一控制架构。

Method: 1. 通过动作屏蔽和基于可达性的可采纳动作集独立于奖励强制硬安全约束
2. 基于安全箱成员资格将状态空间划分为安全/不安全区域
3. 使用模式相关奖励促进安全区内精确跟踪和不安全区内恢复行为
4. 通过状态聚合构建连续动力系统的有限状态抽象
5. 在动作选择和值更新中严格限制为可采纳动作

Result: 框架在具有气动和推进耦合的纵向点质量高超音速飞行器模型上验证，使用攻角和油门作为控制输入，实现了安全约束下的有效学习与控制。

Conclusion: 该框架为连续非线性动力系统提供了一种结合强化学习与形式化安全保证的有效方法，能够独立于奖励机制确保硬安全约束，并通过统一架构处理正常运行和安全恢复两种模式。
...
### [80] [Towards X-embodiment safety: A control theory perspective on transferring safety certificates across dynamical systems](https://arxiv.org/abs/2602.03987)
*Nikolaos Bousias,George Pappas*

Main category: eess.SY

TL;DR: 这篇论文提出了转换控制屏障函数(tCBF)框架，用于在不匹配动态系统之间传递安全保证。


<details>
  <summary>Details</summary>
Motivation: 控制屏障函数(CBFs)是保证控制系统安全约束的有力工具，但直接应用于复杂高维动态系统存在挑战。通常安全证明更适合于简化或替代系统模型，而这些模型与感兴趣的系统动态不完全匹配。

Method: 提出转换控制屏障函数(tCBF)框架，通过模拟函数和显式裕度项，使在一个系统上定义的安全约束能够在另一个系统上强制执行。该方法使用基于二次规划的安全滤波器来实现安全条件。

Result: 在四旋翼导航任务上证明了框架的有效性，转换屏障确保目标系统的碰撞避免，同时对名义控制器保持最小干扰。

Conclusion: 转换控制屏障函数为跨异构动态系统强制执行安全提供了一种通用机制，具有广泛应用潜力。
...
### [81] [Modular Safety Guardrails Are Necessary for Foundation-Model-Enabled Robots in the Real World](https://arxiv.org/abs/2602.04056)
...
TL;DR: 将基础模型集成到机器人中带来了新的安全挑战，需要超越物理约束满足的安全概念。提出了模块化安全防护栏架构，包含监控和干预层，强调跨层协同设计机会，以推进安全的现实世界物理AI部署。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型在机器人领域的应用加速现实世界部署，引入了解释自开放语义推理和物理动作的新安全挑战。现有方法在任务、环境和人类期望是开放、长尾且随时间适应的情况下存在不足。

Method: 提出模块化的安全防护栏架构，由监控（评估）和干预层组成，作为自主堆栈中全面安全的基础。强调通过表示对齐和保守性分配的跨层协同设计机会。

Result: 论文论证了模块化安全防护栏架构的必要性和可行性，指出了基于表示对齐和保守性分配的跨层协同设计能实现更快、更少保守且更有效的安全实施。

Conclusion: 当前的安全方法不足以应对基础模型支持的机器人系统中的复杂安全挑战。模块化安全防护栏架构结合跨层协同设计能更好地解决行动安全、决策安全和以人为中心的安全问题，呼吁社区探索更丰富的防护栏模块和有原则的协同设计策略。
...
### [82] [Lyapunov Constrained Soft Actor-Critic (LC-SAC) using Koopman Operator Theory for Quadrotor Trajectory Tracking](https://arxiv.org/abs/2602.04132)
...
### [83] [Mitigation of Structural Harmonic Instability in Virtual Admittance-Based Grid-Forming Inverters via Mimicking Skin Effect](https://arxiv.org/abs/2602.04221)
*Jaekeun Lee,Jae-Jung Jung,Shenghui Cui*

Main category: eess.SY

TL;DR: 分析了虚拟导纳-电流控制(VA-CC)方案中谐波不稳定的根源，并提出通过并联虚拟电阻的改进方法增强谐波范围内的无源性。


<details>
  <summary>Details</summary>
Motivation: 虚拟导纳-电流控制方案广泛用于模拟电网形成逆变器内电压源前的等效电感，但最近研究发现该方案存在谐波不稳定问题，需要从物理上更可解释的角度理解其根源。

Method: 1）识别了VA-CC方案中延迟无关的结构性谐波不稳定机制——滤波器和虚拟电感相互作用引入了非无源二阶传递函数项，表现为负电阻；2）提出简单有效的改进方法：在VA结构中集成并联虚拟电阻，通过模拟集肤效应增强高频阻尼而不改变现有电流控制器或电压前馈控制。

Result: 实验验证表明，所提方法在相同电网条件下实现了鲁棒的谐波稳定性，而传统方法则失败。

Conclusion: 提出的并联虚拟电阻方法能有效解决VA-CC方案的谐波不稳定问题，通过增强谐波范围内的无源性确保系统稳定性，为电网形成逆变器的控制提供了改进方案。
...
### [84] [Parameter Privacy-Preserving Data Sharing: A Particle-Belief MDP Formulation](https://arxiv.org/abs/2602.04262)
...
### [85] [Peak Bounds for the Estimation Error under Sensor Attacks](https://arxiv.org/abs/2602.04568)
...
### [86] [Reinforcement Learning-based Home Energy Management with Heterogeneous Batteries and Stochastic EV Behaviour](https://arxiv.org/abs/2602.04578)
*Meng Yuan,Ye Wang,Xinghuo Yu,Torsten Wik,Changfu Zou*

Main category: eess.SY

TL;DR: 基于深度强化学习的家庭能源管理框架，联合优化能源支出、电池损耗，兼顾居住舒适度和电动汽车充电需求。


<details>
  <summary>Details</summary>
Motivation: 光伏、电动汽车和储能系统在家庭中的普及增加了能源管理复杂性，同时为能源调节带来新机遇。现有方法难以在不确定环境下有效协调这些资源并考虑电池的异质损耗特性。

Method: 将家庭能源调度问题建模为约束马尔可夫决策过程，采用拉格朗日SAC算法求解，显式纳入固定电池和电动汽车电池的异构退化特性，并考虑用户行为随机性（到达/离开时间、行驶距离）。

Result: 一年期数值模拟表明，该框架能有效满足室内温度约束和电动汽车充电需求，消除热振荡，相比两种基准规则方法显著降低运行总成本，同时将电池退化成本降低8.44%。

Conclusion: 所提出的DRL框架能够在不确定性环境下实现多目标优化，在保证物理约束的同时获得显著经济效益，为家庭能源管理提供了有效解决方案。
...
### [87] [Safe Adaptive Control of Parabolic PDE-ODE Cascades](https://arxiv.org/abs/2602.04656)
*Yun Jiang,Ji Wang*
...
### [88] [Dynamic Constraint Tightening for Nonlinear MPC for Autonomous Racing via Contraction Analysis](https://arxiv.org/abs/2602.04744)
...
### [89] [Control Lyapunov Functions for Optimality in Sontag-Type Control](https://arxiv.org/abs/2602.04756)
...
### [90] [SQP-Based Cable-Tension Allocation for Multi-Drone Load Transport](https://arxiv.org/abs/2602.04801)
*Lamberto Vazquez-Soqui,Fatima Oliva-Palomo,Diego Mercado-Ravell,Pedro Castillo*

Main category: eess.SY

TL;DR: 本文提出了一种实时优化层，通过SQP算法改进分层负载-位置-姿态控制器，解决多无人机空中载荷运输系统的张力分配问题，实现能量均衡和安全运输。


<details>
  <summary>Details</summary>
Motivation: 多无人机空中载荷运输系统具有更大载荷能力和容错性，但存在张力分配不确定性问题，导致能量分布不均、电缆松弛或无人机与电缆碰撞。需要实时优化解决方案。

Method: 采用顺序二次规划算法，最小化缆索张力平方和，并加入电缆对齐惩罚项防止缆索过度收敛，同时保持参考轨迹不变。方法在四旋翼无人机上进行数值仿真验证。

Result: SQP算法在标准硬件上仅需几毫秒运行时间，支持实时应用。敏感性分析显示电缆对齐惩罚增益可在线调整，实现安全裕度与能耗的可控权衡，且跟踪性能无下降。

Conclusion: 该框架为实际部署中的安全、能量均衡协作载荷运输提供了可扩展的解决方案，具有实时应用的可行性。
...
### [91] [DINO-AD: Unsupervised Anomaly Detection with Frozen DINO-V3 Features](https://arxiv.org/abs/2602.03870)
*Jiayu Huo,Jingyuan Hong,Liyun Chen*

Main category: eess.IV

TL;DR: 基于DINO-V3表示的无监督异常检测框架DINO-AD，通过嵌入相似性匹配和前景感知K-means聚类实现了优异的异常定位性能


<details>
  <summary>Details</summary>
Motivation: 医疗图像中无需像素级标注的无监督异常检测对可扩展和标签高效的诊断系统至关重要，但现有方法在精确和可解释的异常定位方面仍存挑战

Method: 提出DINO-AD框架：1）使用嵌入相似性匹配策略选择语义对齐的支持图像；2）采用前景感知K-means聚类模块建模正常特征分布；3）通过余弦相似度比较查询特征与聚类正常嵌入来生成异常图

Result: 在Brain和Liver数据集上均优于现有方法，AUROC最高达98.71；定性结果显示更清晰准确的异常定位；消融研究验证了各组件有效性

Conclusion: DINO-AD框架利用自监督视觉特征实现了鲁棒和可泛化的异常检测，为医疗图像诊断提供了高效的无监督解决方案
...
TL;DR: ARRD系统缺乏不确定度评估，限制临床应用可信度；CONRep框架通过整合符合性预测为VLM生成的影像学报告提供统计可靠的不确定度量化。


<details>
  <summary>Details</summary>
Motivation: 当前自动化放射学报告草拟系统大多缺乏明确的不确定度估计，限制了临床信任和安全部署；需要一种能提供统计可靠不确定度量化的方法。

Method: 提出CONRep框架，包含符合性预测（CP）：标签层面校准预先定义发现的二元预测，句子层面基于图像-文本语义对齐评估自由文本印象的不确定度。

Result: 在公开胸片数据集上验证，高置信度分类的输出与放射科医生标注和真实印象的一致性显著高于低置信度输出，展示了可靠的置信度分层能力。

Conclusion: CONRep在不修改基础模型的情况下实现校准的置信度分层，提高了自动化放射学报告系统的透明度、可靠性和临床可用性。
...
TL;DR: AtlasPatch是一个高效、可扩展的WSI预处理框架，通过基于异构数据集训练的组织检测模型和并行化处理技术，实现准确的组织检测和高通量patch提取，性能达到SOTA水平但计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 当前WSI预处理工具存在两大问题：一是组织检测依赖不准确的启发式阈值分割，二是基于AI的方法在有限多样性数据上训练，计算复杂度过高，这成为计算病理学工作流的主要瓶颈。

Method: AtlasPatch包含三个核心模块：1）在约30,000张WSI缩略图半手动标注数据集上微调Segment-Anything模型用于组织检测；2）从缩略图组织掩码外推到全分辨率切片；3）根据用户指定放大倍数提取patch坐标，支持直接流入图像编码器生成嵌入或存储图片，并行化处理跨CPU/GPU硬件。

Result: AtlasPatch在分割精度、计算复杂度和下游多示例学习任务中均达到最佳水平，同时大幅降低了计算成本。工具已在GitHub开源。

Conclusion: AtlasPatch提供了一个高效、准确且可扩展的WSI预处理解决方案，显著降低计算瓶颈，促进计算病理学工作流的可访问性和可重复性。
...
### [94] [Knowledge Model Prompting Increases LLM Performance on Planning Tasks](https://arxiv.org/abs/2602.03900)
...
### [95] [AgentArk: Distilling Multi-Agent Intelligence into a Single LLM Agent](https://arxiv.org/abs/2602.03955)
...
### [96] [Active Epistemic Control for Query-Efficient Verified Planning](https://arxiv.org/abs/2602.03974)
*Shuhui Qu*
...
### [97] [Adaptive Test-Time Compute Allocation via Learned Heuristics over Categorical Structure](https://arxiv.org/abs/2602.03975)
*Shuhui Qu*
...
### [98] [Monitorability as a Free Gift: How RLVR Spontaneously Aligns Reasoning](https://arxiv.org/abs/2602.03978)
...
### [99] [When AI Persuades: Adversarial Explanation Attacks on Human Trust in AI-Assisted Decision Making](https://arxiv.org/abs/2602.04003)
*Shutong Fan,Lan Zhang,Xiaoyong Yuan*

Main category: cs.AI

TL;DR: 论文提出了对抗性解释攻击(AEAs)——通过在LLM生成的有误决策解释中巧妙设计表达框架来操控用户对错误结果的信任，系统研究了AI解释作为对抗性认知通道的安全威胁。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统越来越多地应用于人类决策循环，用户依赖模型解释来理解和信任AI输出。现有的对抗性攻击主要针对模型计算行为，但AI与用户之间的解释通道形成了一个新的认知层攻击面，攻击者可能通过操控解释的表述方式来诱导用户对错误输出的信任。

Method: 1. 提出对抗性解释攻击(AEAs)概念，通过操纵LLM生成解释的表达框架来调制用户对错误输出的信任度。2. 定义信任错校准差距指标，衡量在对抗性解释下用户对正确和错误输出的信任差异。3. 设计受控实验(n=205)，系统研究解释框架的四个维度对用户信任的影响：推理模式、证据类型、沟通风格和呈现格式。

Result: 实验发现：1. 用户对对抗性和良性解释报告的信任度几乎相同，对抗性解释保留了大部分良性信任，即使对应的是错误输出。2. 最脆弱的场景是当AEA模拟专家沟通方式时——结合权威证据、中性语气和领域适宜推理。3. 在困难任务、事实驱动领域、以及教育程度较低、年龄较小或高度信任AI的参与者中脆弱性最高。

Conclusion: 这是首个将解释视为对抗性认知通道并进行系统安全研究的工作，量化了AI辅助决策中解释如何影响人类信任。研究表明，攻击者可以通过精心设计的解释框架维持用户对错误AI输出的高信任度，揭示了AI-用户沟通渠道中未受保护的安全威胁。
...
### [100] [Axiomatic Foundations of Counterfactual Explanations](https://arxiv.org/abs/2602.04028)
...
### [101] [Scaling In-Context Online Learning Capability of LLMs via Cross-Episode Meta-RL](https://arxiv.org/abs/2602.04089)
*Xiaofeng Lin,Sirou Zhu,Yilei Chen,Mingyu Chen,Hejian Sang,Ioannis Paschalidis,Zhipeng Wang,Aldo Pacchiano,Xuezhou Zhang*

Main category: cs.AI

TL;DR: ORBIT框架通过元强化学习训练LLMs在上下文中从交互中学习，解决了LLMs在线学习能力不足的问题。小模型(Qwen3-14B)在未见环境中表现优异，匹敌GPT-5.2。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型（LLMs）虽然在静态预测和指令跟随任务中表现优异，但在需要在线实时感知、延迟反馈、且需平衡信息收集与利用的现实决策任务中存在局限性。特别是LLMs难以可靠地利用上下文中的交互经验进行在线学习。

Method: 提出ORBIT框架，这是一种多任务、多片段的元强化学习（meta-RL）训练方法，专门用于训练LLMs在上下文中从交互经验中学习。该方法通过元训练让模型学会如何利用上下文中的交互信息进行在线决策。

Result: Meta训练后，相对较小的开源模型Qwen3-14B在新环境中的在线学习性能大幅提升，匹配GPT-5.2水平，且显著优于标准RL微调方法。扩展实验进一步显示随着模型规模增大性能持续提升，表明基于推理时学习的智能体仍有巨大提升空间。代码已开源于GitHub仓库。

Conclusion: ORBIT框架通过多任务多片段元强化学习训练，使LLMs具备在上下文中从交互学习的能力。实验验证了该方法的有效性，并为基于推理时学习的决策智能体未来发展展示了巨大潜力。
...
### [102] [Interfaze: The Future of AI is built on Task-Specific Small Models](https://arxiv.org/abs/2602.04101)
...
### [103] [OMG-Agent: Toward Robust Missing Modality Generation with Decoupled Coarse-to-Fine Agentic Workflows](https://arxiv.org/abs/2602.04144)
*Ruiting Dai,Zheyu Wang,Haoyu Yang,Yihan Liu,Chengzhi Wang,Zekun Zhang,Zishan Huang,Jiaman Cen,Lisi Mo*

Main category: cs.AI

TL;DR: 本文提出OMG-Agent框架，通过动态粗到细的智能体工作流解决多模态数据缺失问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态系统面临数据不完整导致的可靠性问题：传统重建方法依赖内部记忆易产生幻觉，检索增强框架则受限于检索刚性。更重要的是，端到端架构存在语义-细节纠缠的结构冲突，损害保真度

Method: OMG-Agent采用三阶段智能体工作流：1) MLLM驱动的语义规划器通过渐进上下文推理解决输入歧义，生成确定性结构化语义计划；2) 非参数证据检索器将抽象语义锚定于外部知识；3) 检索注入执行器利用检索证据作为灵活特征提示合成高保真细节

Result: 在多个基准测试上的广泛实验表明，OMG-Agent持续超越最先进方法，在极端缺失情况下保持鲁棒性（如在CMU-MOSI上70%缺失率时获得2.6分提升）

Conclusion: OMG-Agent通过将静态映射范式转变为动态粗到细的智能体工作流，有效解决多模态数据不完整问题，在语义规划和细节合成之间实现更好平衡
...
### [104] [Steering LLMs via Scalable Interactive Oversight](https://arxiv.org/abs/2602.04210)
...
TL;DR: 提出了可扩展交互式监督(Scalable Interactive Oversight)框架，通过将复杂意图分解为决策树并聚合用户反馈，解决人类监督AI执行超越自身能力任务的挑战，在网页开发任务中实现PRD对齐度提升54%。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在复杂任务（如vibe coding）上的自动化程度提高，出现了监督鸿沟：用户因领域知识不足、意图表述困难、输出验证不可靠而难以有效指导模型，这引发了如何在AI处理超越人类能力任务时保持可控性的可扩展监督挑战。

Method: 提出可扩展交互式监督框架：1)将复杂意图递归分解为可管理的决策树，2)在每个节点收集低负担的用户反馈，3)递归聚合局部反馈形成全局精确指导，避免开放式提示的依赖。通过强化学习仅使用在线用户反馈优化框架。

Result: 在网页开发任务验证中，非专业人士能生成专家级产品需求文档，对齐度提升54%。框架能通过强化学习仅用在线反馈进行优化，为AI规模化时保持人类控制提供了可行路径。

Conclusion: 该框架通过结构化的交互监督机制，有效弥合了人类能力与AI任务复杂度之间的鸿沟，为解决可扩展监督问题提供了实用方案，并展示了仅通过在线反馈优化监督系统的可能性。
...
### [105] [Empirical-MCTS: Continuous Agent Evolution via Dual-Experience Monte Carlo Tree Search](https://arxiv.org/abs/2602.04248)
...
### [106] [Digital Twins & ZeroConf AI: Structuring Automated Intelligent Pipelines for Industrial Applications](https://arxiv.org/abs/2602.04385)
*Marco Picone,Fabio Turazza,Matteo Martinelli,Marco Mamei*

Main category: cs.AI

TL;DR: 本文提出了一种基于数字孪生技术的零配置AI管道框架，旨在解决工业物联网环境中AI集成复杂度高、异构系统难以协同的问题，实现智能功能的快速部署。


<details>
  <summary>Details</summary>
Motivation: 工业信息物理系统日趋复杂，物联网与工业物联网系统的碎片化（表现在通信协议、数据格式和设备能力的多样性）导致底层物理层与高层智能功能之间存在巨大鸿沟，传统方法往往孤立且紧耦合，限制了AI功能的可扩展性和复用性。

Method: 提出模块化且互操作的解决方案：引入零配置AI管道概念，由数字孪生负责数据管理和智能增强，将数字孪生与AI组件解耦，减少配置需求。该方法在微型工厂场景中进行了验证。

Result: 在微型工厂场景中，该方案表现出对并发机器学习模型和动态数据处理的良好支持，能有效加速复杂工业环境中智能服务的部署。

Conclusion: 所提出的基于数字孪生的零配置AI管道方法，通过解耦数字孪生与AI组件实现模块化互操作，为工业信息物理系统中AI功能的快速集成和协同提供了可行方案。
...
### [107] [ReThinker: Scientific Reasoning by Rethinking with Guided Reflection and Confidence Control](https://arxiv.org/abs/2602.04496)
...
TL;DR: ReThinker是一个置信度感知的智能体框架，通过Solver-Critic-Selector架构实现动态计算分配，在专家级科学推理任务上取得了最先进的性能


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在专家级科学推理（特别是Humanity's Last Exam基准）上面临挑战，传统工具流水线僵化、多智能体协调脆弱、测试时扩展效率低限制了性能提升

Method: 提出ReThinker框架，采用分阶段的Solver-Critic-Selector架构，根据模型置信度动态分配计算；开发了反向数据合成流水线和自适应轨迹回收策略，将成功推理轨迹转化为高质量监督数据

Result: 在HLE、GAIA和XBench基准测试中，ReThinker始终优于现有的基础模型结合工具的方法和深度研究系统，在专家级推理任务上达到了最先进的结果

Conclusion: ReThinker通过置信度感知的动态计算分配和高质量数据合成策略，有效提升了大型语言模型在复杂科学推理任务上的性能，为专家级推理提供了一种新的解决方案
...
### [108] [From Competition to Collaboration: Designing Sustainable Mechanisms Between LLMs and Online Forums](https://arxiv.org/abs/2602.04572)
...
### [109] [Vibe AIGC: A New Paradigm for Content Generation via Agentic Orchestration](https://arxiv.org/abs/2602.04575)
*Jiaheng Liu,Yuanxing Zhang,Shihao Li,Xinping Lei*

Main category: cs.AI

TL;DR: 论文提出 Vibe AIGC 新范式，通过智能体编排解决当前生成式AI在意图与执行之间的差距问题


<details>
  <summary>Details</summary>
Motivation: 当前以模型为中心的生成式AI存在‘可用性天花板’，表现为意图-执行差距。用户的高层意图与单次生成的黑盒随机性之间存在根本性差异，需要新的范式来弥合人类想象与机器执行间的鸿沟

Method: 受Vibe Coding启发，提出Vibe AIGC范式——通过智能体编排实现自主组合的分层多智能体工作流。用户成为提供高层次Vibe表示的‘指挥官’，中央元规划器将Vibe解构为可执行、可验证、自适应的智能体管道，从随机推理转向逻辑编排

Result: Vibe AIGC能够弥合人类意图与AI执行之间的差距，将AI从脆弱的推理引擎转变为强大的系统级工程合作伙伴

Conclusion: 这一范式转变将重新定义人机协作经济，使复杂长期数字资产的创造民主化，代表了生成式AI从模型中心向agentic编排的重要演进
...
### [110] [WideSeek-R1: Exploring Width Scaling for Broad Information Seeking via Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.04634)
*Zelai Xu,Zhexuan Xu,Ruize Zhang,Chunyang Zhu,Shi Yu,Weilin Liu,Quanlu Zhang,Wenbo Ding,Chao Yu,Yu Wang*

Main category: cs.AI

TL;DR: 论文提出WideSeek-R1框架，通过多智能体宽度扩展应对广泛信息查询任务，相比传统深度扩展单智能体方法，实现了可扩展的并行执行。


<details>
  <summary>Details</summary>
Motivation: 随着任务范围扩大，关键瓶颈从单个智能体的能力转向组织能力，现有多智能体系统依赖人工设计的工作流程，无法有效并行化工作。

Method: 提出lead-agent-subagent架构，通过多智能体强化学习训练，使用共享LLM模型但隔离上下文和专用工具，在2万个广泛信息查询任务数据集上进行联合优化。

Result: WideSeek-R1-4B在WideSearch基准测试中获得40.0%的item F1分数，与单智能体DeepSeek-R1-671B性能相当，且随着并行子智能体数量增加表现出持续性能提升。

Conclusion: 宽度扩展在多智能体系统中是有效的补充维度，WideSeek-R1框架通过并行执行实现了与大型单智能体模型相当的性能，验证了宽度扩展策略的有效性。
...
### [111] [Agentic AI in Healthcare & Medicine: A Seven-Dimensional Taxonomy for Empirical Evaluation of LLM-based Agents](https://arxiv.org/abs/2602.04813)
...
### [112] [Are AI Capabilities Increasing Exponentially? A Competing Hypothesis](https://arxiv.org/abs/2602.04836)
...
### [113] [Group-Evolving Agents: Open-Ended Self-Improvement via Experience Sharing](https://arxiv.org/abs/2602.04837)
...
### [114] [Fluid Representations in Reasoning Models](https://arxiv.org/abs/2602.04843)
...
### [115] [Less is More: Optimizing Probe Selection Using Shared Latency Anomalies](https://arxiv.org/abs/2602.03965)
...
### [116] [Multi-Tier UAV Edge Computing Towards Long-Term Energy Stability for Low Altitude Networks](https://arxiv.org/abs/2602.04258)
*Yufei Ye,Shijian Gao,Xinhu Zheng,Liuqing Yang*

Main category: cs.NI

TL;DR: 该论文提出了一种创新的多层级无人机边缘计算系统，通过分层架构和动态优化减少任务执行延迟并确保无人机能量稳定性。


<details>
  <summary>Details</summary>
Motivation: 无人机在低空边缘计算中具有高度机动性优势，但它们面临任务执行延迟和能量不稳定的挑战。现有方案在动态环境中难以平衡延迟与能量消耗的矛盾。

Method: 论文设计了分层架构，轻型无人机作为边缘服务器，重型无人机作为后备服务器。利用李雅普诺夫优化分解问题，实施动态优先级调整，并采用区块坐标下降算法协同优化任务分配、计算资源和轨迹控制。

Result: 实验仿真结果表明，该方案可显著降低轻量级无人机的传输能量消耗超过26%，并且在长期能量稳定性方面优于现有基准方法。

Conclusion: 所提多层级无人机边缘计算系统有效解决了任务延迟与能量稳定的权衡问题，通过分层协作和自适应优化机制，展现了无人机边缘计算的实用价值。同时提出了系统可扩展性和实际部署成本等未来研究方向。
...
### [117] [LLM-Empowered Cooperative Content Caching in Vehicular Fog Caching-Assisted Platoon Networks](https://arxiv.org/abs/2602.04471)
...
### [118] [Semantic Rate Distortion and Posterior Design: Compute Constraints, Multimodality, and Strategic Inference](https://arxiv.org/abs/2602.03949)
*Emrah Akyol*
...
### [119] [Joint Sleep Mode Activation and Load Balancing with Dynamic Cell Load: A Combinatorial Bandit Approach](https://arxiv.org/abs/2602.04808)
...
### [120] [Game of Coding for Vector-Valued Computations](https://arxiv.org/abs/2602.04810)
*Hanzaleh Akbari Nodehi,Parsa Moradi,Soheil Mohajer,Mohammad Ali Maddah-Ali*

Main category: cs.IT

TL;DR: 将编码博弈理论从标量计算扩展到N维欧几里得空间，建立向量值计算框架，无需诚实多数假设即可保证正确解码。


<details>
  <summary>Details</summary>
Motivation: 经典编码理论需要诚实节点占多数才能保证有效解码，但新兴的无许可应用（尤其是去中心化机器学习）需要超越这一限制。现有的编码博弈研究仅限于标量计算，而现实世界任务通常处理高维数据，需要扩展到向量空间。

Method: 将编码博弈框架扩展到N维欧几里得空间，提供向量值计算的严格问题表述，完全表征所得高维博弈的均衡策略。

Result: 分析表明，在标量设置中建立的弹性特性在向量体系中被保留，为无需诚实多数假设的大规模安全去中心化计算奠定了理论基础。

Conclusion: 该研究将编码博弈扩展到多维空间，为安全的大规模分布式计算提供了理论基础，突破了传统编码理论对诚实多数的依赖，特别适合去中心化机器学习等应用。
...
### [121] [Capacity Bounds on Doppler OFDM Channels](https://arxiv.org/abs/2602.04862)
*Pablo Orellana,Zheng Li,Jean-Marc Kelif,Sheng Yang,Shlomo Shamai*

Main category: cs.IT

TL;DR: 研究了LEO卫星信道中的多普勒残余不确定性建模，提出了基于子空间对齐叠加传输的方案，能在大信噪比和准相干场景下接近容量界。


<details>
  <summary>Details</summary>
Motivation: LEO卫星系统的高移动性带来了显著的多普勒效应，虽然在接收端可以部分补偿，但残余的频率不确定性会形成一种结构化的信道不确定性，限制系统可达速率。本文旨在建模这种效应并设计高效传输方案。

Method: 首先建立了$\mathbf{H}=\mathbf{F}+s\mathbf{G}$的信道模型，其中$s$是未知的标量随机参数。推导了基于显式传输方案的可达速率下界，并通过对偶方法得到了容量上界。提出了实用的叠加传输方案，采用子空间对齐和连续干扰消除，其中粗层流可作为隐式导频来解码精层数据。

Result: 描述了在近相干和高信噪比区域内的渐进容量特性。通过多普勒-OFDM仿真表明，所提出的SN方案能以低复杂度实现接近最优的速率。

Conclusion: 本文提出的子空间对齐叠加方案能够有效处理LEO卫星信道中的多普勒残余不确定性，在大信噪比和准相干场景下实现接近理论容量的性能，为实际系统提供了一种低复杂度且高效的解决方案。
...
### [122] [Understanding the Impact of Differentially Private Training on Memorization of Long-Tailed Data](https://arxiv.org/abs/2602.03872)
*Jiaming Zhang,Huanyi Xie,Meng Ding,Shaopeng Fu,Jinyan Liu,Di Wang*

Main category: cs.LG

TL;DR: DP-SGD在长尾数据上泛化性能较差，本文首次从特征学习的理论视角分析了这一现象，发现DP-SGD在长尾子群体的测试误差显著高于整体误差。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习模型通过记忆训练样本来获得高预测精度，这引发了隐私担忧，但常用的DP-SGD算法在长尾数据上泛化性能较差，需要理论解释。

Method: 建立了首个分析DP-SGD在长尾数据上的理论框架，从特征学习视角研究梯度裁剪和噪声注入如何共同影响模型对有信息但代表性不足样本的记忆能力。

Result: 理论分析表明DP-SGD在长尾子群体上的测试误差显著大于整体测试误差，梯度裁剪和噪声注入会损害模型对低代表性样本的记忆能力。实验在合成和真实数据集上验证了理论发现。

Conclusion: 本文为理解DP-SGD在长尾数据上的性能下降提供了首个理论框架，揭示了隐私保护训练算法在非平衡数据上的内在局限性，对实际应用有重要指导意义。
...
### [123] [Reversible Deep Learning for 13C NMR in Chemoinformatics: On Structures and Spectra](https://arxiv.org/abs/2602.03875)
...
### [124] [GOPO: Policy Optimization using Ranked Rewards](https://arxiv.org/abs/2602.03876)
*Kyuseong Choi,Dwaipayan Saha,Woojeong Kim,Anish Agarwal,Raaz Dwivedi*

Main category: cs.LG

TL;DR: GOPO：一种使用奖励排序而非具体数值的强化学习方法，在摘要、指令遵循等任务上相比GRPO取得更好效果，训练更高效


<details>
  <summary>Details</summary>
Motivation: 传统RLHF使用绝对奖励值进行策略优化，但在奖励不可验证的任务中（如摘要、指令遵循），这种绝对数值的利用导致了性能欠佳

Method: 提出Group Ordinal Policy Optimization (GOPO)，仅使用奖励的相对排序，丢弃具体数值大小，通过基于排序的奖励转换来优化策略

Result: 相比GRPO：(1)获得更高的训练/验证奖励轨迹；(2)在大多数中间训练步骤中获得更好的LLM评估；(3)以更少训练步骤达到同等质量的策略

Conclusion: GOPO在多种任务和模型规模上均展现出一致的改进效果，验证了基于排序的策略优化方法在奖励不可验证任务中的有效性
...
### [125] [Supervised Learning as Lossy Compression: Characterizing Generalization and Sample Complexity via Finite Blocklength Analysis](https://arxiv.org/abs/2602.04107)
...
### [126] [Echo State Networks for Time Series Forecasting: Hyperparameter Sweep and Benchmarking](https://arxiv.org/abs/2602.03912)
*Alexander Häußer*
...
### [127] [SpecMD: A Comprehensive Study On Speculative Expert Prefetching](https://arxiv.org/abs/2602.03921)
*Duc Hoang,Ajay Jaiswal,Mohammad Samragh,Minsik Cho*

Main category: cs.LG

TL;DR: 提出SpecMD标准化框架来评估MoE专家缓存策略在不同硬件配置上的性能，发现传统缓存策略不适用于MoE，并设计新的Least-Stale策略大幅提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有MoE模型的专家缓存策略与硬件规格的交互关系缺乏系统性研究，需要标准化基准测试框架来理解和优化MoE的缓存性能。

Method: 开发SpecMD标准化框架，在多种硬件配置上系统基准测试MoE缓存策略；基于观察提出新的Least-Stale淘汰策略。

Result: 发现MoE专家访问不符合时间局部性假设；Least-Stale策略相比LRU减少85倍碰撞缺失，在仅5% VRAM缓存容量下达到88%命中率和34.7%的TTFT降低。

Conclusion: MoE缓存需要专门设计的策略，Least-Stale策略能显著提升性能，为实际部署提供重要指导意义。
...
### [128] [WIND: Weather Inverse Diffusion for Zero-Shot Atmospheric Modeling](https://arxiv.org/abs/2602.03924)
*Michael Aich,Andreas Fürst,Florian Sestak,Carlos Ruiz-Gonzalez,Niklas Boers,Johannes Brandstetter*

Main category: cs.LG

TL;DR: WIND是一个通用的天气气候基础模型，通过自监督视频重构预训练和逆问题推理，无需任务特定微调即可完成多种天气气候任务。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习在天气气候建模中高度碎片化，需要为不同任务训练专门模型，缺乏统一的基础模型。

Method: 使用无条件视频扩散模型进行自监督视频重构预训练，学习大气动力学；推理时将各类问题统一为逆问题，通过后验采样解决，无需任务特定微调。

Result: WIND能够处理概率预报、时空降尺度、稀疏重构、守恒律约束等重要天气气候问题，并能生成全球变暖情景下的极端天气反事实情景，计算高效。

Conclusion: WIND通过生成式视频建模与逆问题求解的结合，为AI大气建模提供了高效的范式转变，有望统一天气气候建模的碎片化格局。
...
### [129] [Autonomous AI Agents for Real-Time Affordable Housing Site Selection: Multi-Objective Reinforcement Learning Under Regulatory Constraints](https://arxiv.org/abs/2602.03940)
...
### [130] [Grables: Tabular Learning Beyond Independent Rows](https://arxiv.org/abs/2602.03945)
*Tamara Cucumides,Floris Geerts*

Main category: cs.LG

TL;DR: 表格学习仍以行独立预测为主，难以处理事务、时序和关系型表格中的行间依赖关系。本文提出 grables 模块化框架，通过构造图和节点预测器分离来量化结构利用效果。实验证实消息传递能捕捉行间依赖，而提取结构信息结合强大表格学习器可获得稳定提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于 i.i.d. 假设的行级预测方法无法处理标签存在行间依赖的表格数据（如事务、时序、关系型表格），限制了自然目标（如全局计数、重叠、关系模式）的实现。需要量化不同架构如何利用表格结构，并系统评估结构利用对预测性能的影响。

Method: 提出 grables 模块化接口：构造器将表格提升为图，节点预测器在图节点上进行预测。该框架分离结构提取与预测计算，可精确分析表达能力的来源。实验中比较了消息传递、基于结构特征的混合方法等在不同数据集上的表现。

Result: 实验证明：消息传递能捕捉行级模型遗漏的行间依赖；在合成任务、交易数据和 RelBench 临床试验数据集上，明确提取行间结构特征并结合强大表格学习器的混合方法持续带来性能提升。

Conclusion: 表格学习中利用行间结构至关重要，尤其对于非独立同分布的表格数据。grables 框架为量化结构利用提供了系统方法，而结合结构提取与表格学习器的混合策略是提升预测性能的有效途径。
...
### [131] [Non-linear PCA via Evolution Strategies: a Novel Objective Function](https://arxiv.org/abs/2602.03967)
...
### [132] [eCP: Informative uncertainty quantification via Equivariantized Conformal Prediction with pre-trained models](https://arxiv.org/abs/2602.03986)
*Nikolaos Bousias,Lars Lindemann,George Pappas*

Main category: cs.LG

TL;DR: 本文研究通过将预训练模型进行群对称化处理来改进保形预测（CP），利用几何信息将非一致性得分分布在轨道上，以缩小预测不确定性区域。


<details>
  <summary>Details</summary>
Motivation: 保形预测（CP）虽然提供了有限样本下的形式化覆盖保证，但在长时任务中不确定性区域会显著扩大，导致统计保证失效。这促使研究者通过在CP中融入几何信息来解决此问题。

Method: 提出对预训练预测器进行群平均处理，将非一致性质量分布在轨道上，每个样本被视作轨道代表，通过对称群元素连接的样本来缓解不确定性。该方法在理论上可证明缩小非一致性得分。

Result: 提出的方法在凸序意义下缩小非一致性得分，意味着改进的指数尾界和更尖锐的保形预测集（特别在高置信水平下），并通过行人轨迹预测实验验证理论主张。

Conclusion: 通过在保形预测中融入对称性几何信息，可以有效缩小不确定性区域，特别适用于需要高置信水平的长时预测任务。
...
### [133] [Resilient Load Forecasting under Climate Change: Adaptive Conditional Neural Processes for Few-Shot Extreme Load Forecasting](https://arxiv.org/abs/2602.04609)
...
TL;DR: PromptSplit是一个内核框架，用于检测和分析生成模型之间的依赖prompt的差异，可扩展且提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 随着各种prompt引导生成模型的涌现，需要原则性的方法来识别导致模型行为差异的prompt类型，从而理解模型分歧的根源。

Method: 构建联合prompt-output张量积嵌入表示，计算核协方差矩阵，利用这些矩阵加权差的特征空间识别跨prompt的行为差异主方向，并采用随机投影近似降低计算复杂度。

Result: 在文本到图像、文本到文本和图像标注场景下的实验表明，PromptSplit能准确检测真实行为差异并定位负责的prompt。

Conclusion: PromptSplit为生成模型的分歧检测提供了可解释的工具，能够有效地识别模型行为差异及其对应的prompt。
...
TL;DR: 该论文提出了一个统一的投影残差视角来分析参数高效微调（PEFT），并引入了Layer Card作为层选择诊断工具，实现了在保持性能的同时显著降低微调成本和推理时适配层数量。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型规模增长，全参数微调成本过高使得PEFT成为默认的下游适应策略。但在实际部署中，由于推理延迟和微调成本等约束，必须选择微调哪些层。当前实践通常均匀地应用PEFT到所有层，缺乏对层选择的深入理解和有效利用。

Method: 1. 建立了基于冻结基础模型的统一投影残差视角分析PEFT；2. 在局部二次近似下，将层适应分解为三个关键量：投影残差范数（衡量可校正偏差）、激活能量（控制特征条件）和层耦合（量化跨层残差交互）；3. 开发了Layer Card诊断工具，汇总每层的残差信号强度、计算成本和性能信息；4. 基于Layer Card指导的层选择策略，灵活优化不同目标（性能最大化或成本最小化）。

Result: 1. 对于平方损失和线性适配器，投影残差范数等于归一化梯度范数，激活能量控制病态条件和噪声放大，弱耦合产生近似可加的层贡献；2. 在Qwen3-8B上的实验表明，选择性微调子集层可以达到接近全层LoRA的性能，同时显著降低微调成本和推理时的适配层数量。

Conclusion: 该研究提供了一种更科学的PEFT层选择方法，通过Layer Card诊断工具可以灵活权衡性能与成本，为实际部署提供了更加成本-性能感知的替代方案，有助于在边缘计算和快速部署场景中优化模型适应策略。
...
TL;DR: TL;DR: GROOVE是一种半监督多模态表示学习方法，使用新型GroupCLIP损失和反翻译自编码器框架，针对弱配对高内容扰动数据进行表示学习，在跨模态匹配和插补任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 研究动机：在高内容扰动数据中，跨模态样本通常只通过共享的扰动标签弱配对，缺乏直接对应关系。现有的对比学习方法（如CLIP和SupCon）无法有效处理这种弱配对设置，因此需要开发专门的方法来学习跨模态纠缠表示。

Method: 核心方法：1. 提出GroupCLIP，一种组级对比损失，弥合了CLIP（用于配对跨模态数据）和SupCon（用于单模态监督对比学习）之间的差距。2. 结合了即时反翻译自编码器框架，在共享潜在空间中鼓励跨模态纠缠表示，同时保持组级一致性。3. 引入了全面的组合评估框架，系统评估不同最优传输对齐器的性能，并通过模拟实验系统变化共享与模态特定扰动效应。

Result: 主要结果：1. 在模拟实验和两个真实单细胞遗传扰动数据集中，GROOVE在跨模态匹配和插补任务上表现最佳。2. 消融研究显示GroupCLIP是性能提升的关键驱动因素。3. 评估框架显示目前没有统一的跨模态对齐器在所有设置或模态对中都表现最优。4. 该方法特别适合仅能获得弱配对信息的场景。

Conclusion: 结论：1. 在弱配对场景下，利用组级约束进行多模态表示学习至关重要。2. GroupCLIP能有效弥合现有对比学习方法的差距，是GROOVE成功的关键。3. 提出的评估框架为系统评估表示学习方法提供了新工具，可用于分析不同对齐方法的性能差异。
...
TL;DR: 基于共识的贝叶斯框架，用于检测企业目录访问图中的恶意用户行为，通过建模主题动态和意见演化来识别异常


<details>
  <summary>Details</summary>
Motivation: 企业目录系统中的恶意行为通常通过逻辑不一致性来识别，但现有方法缺乏对不确定性和动态演化的建模能力。本文旨在开发一个能够捕捉用户行为演化、量化不确定性、并检测结构违规的框架。

Method: 将目录建模为主题，用户建模为多层级交互图中的智能体。使用影响加权意见动力学模拟访问演化，通过动态矩阵编码用户间逻辑依赖，共享影响矩阵捕捉目录相似性。恶意行为被注入为违反强连通分量结构规范的跨组件逻辑扰动。利用意见动力学理论保证主题收敛，通过缩放意见方差检测异常。引入随时间演化的贝叶斯异常评分机制，结合静态和在线先验。

Result: 在合成访问图上的模拟验证了该方法对逻辑不一致性的敏感性，以及在动态扰动下的鲁棒性。能够有效检测恶意行为，同时量化不确定性。

Conclusion: 提出的共识贝叶斯框架为检测企业目录系统中的恶意用户行为提供了一种有效方法，能够建模动态演化、处理不确定性，并在合成数据上表现出良好的检测性能。
...
### [142] [SEIS: Subspace-based Equivariance and Invariance Scores for Neural Representations](https://arxiv.org/abs/2602.04054)
...
### [143] [Stroke Lesions as a Rosetta Stone for Language Model Interpretability](https://arxiv.org/abs/2602.04074)
...
TL;DR: 提出BLUM框架，利用脑损伤-症状映射作为外部参考结构评估LLM扰动效果，发现LLM错误模式与人类错误高度相似并能预测实际脑损伤位置。


<details>
  <summary>Details</summary>
Motivation: 当前LLM解释性方法依赖内部指标且缺乏外部验证，需要建立因果性脑行为关系的外部参照框架。

Method: 利用410名卒中后失语症患者数据训练症状到病变模型，对比分析LLM扰动后的错误模式并将其投影到人类病变空间。

Result: LLM错误模式预测实际脑损伤位置显著优于随机水平（图片命名67%，句子完成68.3%），语义错误对应腹侧通路损伤，语音错误对应背侧通路损伤。

Conclusion: 该方法为LLM解释性开辟了新途径，临床神经科学为人工语言系统评估提供了外部验证，促使研究行为对齐是否反映共享计算原理。
...
### [144] [A Probabilistic Framework for Solving High-Frequency Helmholtz Equations via Diffusion Models](https://arxiv.org/abs/2602.04082)
...
### [145] [Federated Concept-Based Models: Interpretable models with distributed supervision](https://arxiv.org/abs/2602.04093)
...
### [146] [CoRe: Context-Robust Remasking for Diffusion Language Models](https://arxiv.org/abs/2602.04096)
*Kevin Zhai,Sabbir Mollah,Zhenyi Wang,Mubarak Shah*

Main category: cs.LG

TL;DR: 本文提出了CoRe框架，通过在推理时探测token对上下文扰动的敏感性来识别不稳定的token进行修订，而非依赖静态置信度，从而改善掩码扩散模型的标准解码问题。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型的标准解码存在上下文刚性问题：token基于瞬时的置信度被保留，而早期预测缺乏完整上下文，导致初始不一致性误导后续生成。现有的修订策略依赖静态置信度，但这些信号本质上是短视的，不一致的token可能对模型本身显得很自信。

Method: 提出无训练的推理时修订框架CoRe，通过探测token对有针对性的掩码上下文扰动的敏感性来识别上下文脆弱的token，将修订形式化为对上下文偏移的鲁棒优化目标，并高效地近似该目标以优先修订不稳定token。

Result: 在LLaDA-8B-Base上，CoRe在推理和代码基准测试中持续改进，优于计算匹配的基线方法，并将MBPP基准提升了高达9.2个百分点。

Conclusion: CoRe通过关注token对上下文变化的敏感性而非静态置信度，有效缓解了掩码扩散模型的上下文刚性问题，在多个基准测试中取得了显著改进。
...
### [147] [Rethinking Perplexity: Revealing the Impact of Input Length on Perplexity Evaluation in LLMs](https://arxiv.org/abs/2602.04099)
...
TL;DR: Semi-dual neural OT训练存在虚假解问题，本文通过加性噪声平滑方法结合内在维度分析，建立了可计算的噪声级来保证统计最优性，并提供早停准则以避免优化条件恶化。


<details>
  <summary>Details</summary>
Motivation: 神经最优传输半对偶目标在训练中会收敛到错误的或退化解，尤其在数据集中分布在低维流形时。

Method: 对加性噪声平滑方法进行理论分析，推导出可计算的终端噪声水平，考虑了最优计划的稳定性、平滑引入的偏差和有限样本误差。

Result: 建立了内在维度依赖的统计最优收敛速率，证明了当噪声低于统计最优水平时，优化条件会恶化而统计精度不会提升。

Conclusion: 提供了基于内在维度的量化指导原则，通过噪声平滑与早停规则可以避免虚假解，同时在统计上达到最优。
...
TL;DR: PLANET框架提出了一种图拓扑感知的多模态交互与对齐方法，通过解耦多模态交互和模态对齐来解决现有多模态图基础模型的局限性


<details>
  <summary>Details</summary>
Motivation: 现有图基础模型主要关注文本属性图，而多模态属性图未被充分利用。现有MGFMs存在两个基本局限性：缺乏显式的模态交互建模和次优的模态对齐能力

Method: 提出PLANET框架，采用分而治之策略解耦不同粒度的模态交互和对齐。在嵌入粒度上使用嵌入域门控进行局部语义丰富和拓扑感知的跨模态上下文注入；在节点粒度上通过节点级离散化检索构建离散语义表示空间来确保全局模态对齐

Result: 大量实验表明PLANET在多种图中心和模态生成任务上显著优于现有最先进的基线方法

Conclusion: PLANET通过创新的解耦策略有效解决了多模态图基础模型中的模态交互和对齐问题，展示了在广泛下游任务中的应用潜力
...
TL;DR: 提出XaaS分层架构，将解释性作为独立系统服务而非模型附属功能，通过解耦推理与解释、分布式缓存、轻量验证和自适应引擎三大创新，在边缘设备上实现低延迟、高扩展的可解释AI部署。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI在边缘/IoT系统中存在耦合式设计的效率问题，导致冗余计算、高延迟和扩展性差。需要将解释性作为独立系统服务而非模型附属功能，以适应异构边缘设备资源限制。

Method: 1) 基于语义相似度的分布式解释缓存减少重复计算；2) 轻量级验证协议确保缓存与新生成解释的保真度；3) 自适应解释引擎根据设备能力和用户需求选择解释方法。

Result: 在制造业质检、自动驾驶感知、医疗诊断三个真实场景中，XaaS将延迟降低38%，同时保持高解释质量，实现透明可靠AI在大型异构IoT系统的部署。

Conclusion: XaaS架构填补了可解释AI研究与边缘实践之间的鸿沟，为大范围异构物联网系统部署透明可靠AI提供了可行方案。
...
TL;DR: 本文提出了扩散最后一层（DLL）方法，作为神经算子的轻量级概率头部附件，用于对预测不确定性进行建模。


<details>
  <summary>Details</summary>
Motivation: 实际系统通常具有随机性，因此不确定性量化对于神经算子的可靠部署至关重要。现有方法在这方面存在不足，需要一种轻量级、高效的不确定性建模方案。

Method: DLL利用PDE解分布通常具有相对光滑和低维结构的特点，通过低秩Karhunen-Loève展开直接参数化函数空间中的条件输出分布，从而实现对不确定性的高效表达建模。

Result: 在随机PDE算子学习基准测试中，DLL提高了泛化能力和不确定性感知预测性能。在确定性长时滚转设置中，DLL增强了滚转稳定性并为骨干神经算子提供了有意义的认知不确定性估计。

Conclusion: DLL作为一种简单附加组件，可有效增强神经算子的不确定性量化能力，在随机系统和确定性系统中均表现出优越性能。
...
### [156] [Benchmarking Uncertainty Quantification of Plug-and-Play Diffusion Priors for Inverse Problems Solving](https://arxiv.org/abs/2602.04189)
*Xiaoyu Qiu,Taewon Yang,Zhanhao Liu,Guanyang Wang,Liyue Shen*

Main category: cs.LG

TL;DR: 提出系统评估插播扩散先验（PnPDP）不确定性量化（UQ）的框架


<details>
  <summary>Details</summary>
Motivation: 当前插播扩散先验（PnPDP）方法在逆问题求解中仅评估单一样本的点估计精度，忽视了其随机性和逆问题固有的不确定性特征，而科学任务中通常需要后验分布作为输出，现有基准无法评估不确定性量化表现

Method: 设计理论玩具模型模拟来评估各种PnPDP求解器的不确定性行为，提出基于UQ的分类体系，在玩具模拟和真实世界科学逆问题上进行大量实验

Result: 实验观察到与分类体系和理论分析一致的不确定性行为模式，为理解和评估PnPDP的不确定性提供了新见解

Conclusion: 系统研究了扩散逆问题求解器的不确定性量化基准，填补了现有评估方法的空白，提供了理解PnPDP不确定性行为的新视角
...
### [157] [LORE: Jointly Learning the Intrinsic Dimensionality and Relative Similarity Structure From Ordinal Data](https://arxiv.org/abs/2602.04192)
...
### [158] [From Sparse Sensors to Continuous Fields: STRIDE for Spatiotemporal Reconstruction](https://arxiv.org/abs/2602.04201)
*Yanjie Tong,Peng Chen*

Main category: cs.LG

TL;DR: STRIDE是一个两阶段框架，通过时间编码器将传感器测量映射到潜在状态，并使用调制隐式神经表示解码器在任意位置重建场，在稀疏传感下性能优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以跨轨迹和参数设置泛化，或依赖与离散化绑定的解码器，无法自然地在不同网格和分辨率间迁移。

Method: 两阶段框架：1) 时间编码器将短窗口传感器测量映射到潜在状态；2) 调制隐式神经表示(FMMNN作为主干)在任意查询位置重建场。

Result: 在四个挑战性基准测试中，STRIDE在极稀疏传感下优于强基线，支持超分辨率，且对噪声保持鲁棒性。

Conclusion: STRIDE通过学习有限维嵌入实现了从稀疏点测量到高维时空场的有效重建，为参数化PDE动力学的学习提供了有前景的解决方案。
...
### [159] [Cascading Robustness Verification: Toward Efficient Model-Agnostic Certification](https://arxiv.org/abs/2602.04236)
...
### [160] [From Ambiguity to Action: A POMDP Perspective on Partial Multi-Label Ambiguity and Its Horizon-One Resolution](https://arxiv.org/abs/2602.04255)
*Hanlin Pan,Yuhao Tang,Wanfu Gao*

Main category: cs.LG

TL;DR: 本文提出一种基于部分可观测马尔可夫决策过程的偏多标签学习框架，联合解决标签歧义消除和特征选择问题，通过两阶段强化学习提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统偏多标签学习中真实标签未标注，导致标签消歧困难，且歧义候选标签容易将误差传播给下游任务（如特征工程），需要更有效的方法同时解决这两个问题。

Method: 将标签消歧和特征选择建模为POMDP问题：第一阶段通过强化学习训练transformer策略生成硬伪标签；第二阶段将特征选择建模为顺序强化学习问题，逐步选择特征并输出可解释的全局排序。

Result: 实验在多个数据集和评估指标上验证了该框架的优越性，理论分析提供了PML-POMDP对应关系和超额风险界限的数学证明。

Conclusion: 该POMDP框架能有效解决偏多标签学习中的标签消歧和特征选择问题，实验和理论分析均证明了其有效性。该内容未通过合规测试，已被隐藏。
...
### [161] [Thickening-to-Thinning: Reward Shaping via Human-Inspired Learning Dynamics for LLM Reasoning](https://arxiv.org/abs/2602.04265)
...
### [162] [Multi-Integration of Labels across Categories for Component Identification (MILCCI)](https://arxiv.org/abs/2602.04270)
*Noga Mudrik,Yuxi Chen,Gal Mishne,Adam S. Charles*

Main category: cs.LG

TL;DR: MILCCI是一种新的数据驱动方法，能够从多试验时间序列数据中识别可解释成分，捕捉跨试验变异性，并整合标签信息来理解每个类别在数据中的表示。


<details>
  <summary>Details</summary>
Motivation: 许多领域通过重复测量收集大规模时间数据，每个试验都带有跨多个类别的元数据变量标签。关键挑战是理解这些标签如何编码在多试验观测中，并区分每个类别标签条目的不同影响。

Method: MILCCI基于稀疏的每试验分解，利用每个类别内的标签相似性实现微妙的、标签驱动的跨试验成分组成调整，并区分每个类别的贡献。该方法还学习每个成分对应的时间痕迹，该痕迹在每个试验内随时间演化并在试验间灵活变化。

Result: 通过在合成数据和真实世界数据（包括投票模式、在线页面浏览趋势和神经元记录）上的演示，验证了MILCCI的性能。

Conclusion: MILCCI为分析多试验时间序列数据提供了一种有效的方法，能够从复杂的时间数据中提取可解释的、标签驱动的成分，有助于理解不同类别标签如何影响观测结果。
...
### [163] [Multi Objective Design Optimization of Non Pneumatic Passenger Car Tires Using Finite Element Modeling, Machine Learning, and Particle swarm Optimization and Bayesian Optimization Algorithms](https://arxiv.org/abs/2602.04277)
...
### [164] [Convolution Operator Network for Forward and Inverse Problems (FI-Conv): Application to Plasma Turbulence Simulations](https://arxiv.org/abs/2602.04287)
...
### [165] [Efficient Equivariant High-Order Crystal Tensor Prediction via Cartesian Local-Environment Many-Body Coupling](https://arxiv.org/abs/2602.04323)
*Dian Jin,Yancheng Yuan,Xiaoming Tao*

Main category: cs.LG

TL;DR: 提出了Cartesian Environment Interaction Tensor Network (CEITNet)，通过构建多通道笛卡尔局部环境张量并进行可学习通道空间交互，实现了高阶晶体张量性质的高效端到端预测。


<details>
  <summary>Details</summary>
Motivation: 端到端预测晶体高阶张量性质一直具有挑战性：虽然球谐等变模型具有表达力，但其Clebsch-Gordan张量积对于高阶目标会产生大量计算和内存成本。

Method: 为每个原子构建多通道笛卡尔局部环境张量，通过可学习的通道空间交互进行灵活的多体混合，在通道空间进行学习，并使用笛卡尔张量基组装等变输出。

Result: 在阶-2介电、阶-3压电和阶-4弹性张量预测的基准数据集上，CEITNet在关键精度指标上超越了先前的高阶预测方法，同时提供了高计算效率。

Conclusion: CEITNet提供了一种高效且准确的高阶晶体张量性质预测方法，克服了传统球谐等变模型的高计算成本问题。
...
### [166] [RISE: Interactive Visual Diagnosis of Fairness in Machine Learning Models](https://arxiv.org/abs/2602.04339)
*Ray Chen,Christan Grant*

Main category: cs.LG

TL;DR: RISE工具通过可视化残差排序来诊断跨域公平性问题，揭示传统汇总统计忽略的准确性与公平性权衡


<details>
  <summary>Details</summary>
Motivation: 在域偏移下评估公平性具有挑战性，因为标量指标常常掩盖了不平等在何处以及如何产生

Method: 提出RISE交互式可视化工具，将排序后的残差转换为可解释的模式，通过残差曲线结构与形式化公平概念连接

Result: RISE能够进行局部差异诊断、跨环境子群比较，检测隐藏的公平问题，展示准确性-公平性权衡

Conclusion: 通过事后分析，RISE暴露了聚合统计忽略的权衡关系，支持更明智的模型选择决策
...
### [167] [UnMaskFork: Test-Time Scaling for Masked Diffusion via Deterministic Action Branching](https://arxiv.org/abs/2602.04344)
*Kou Misaki,Takuya Akiba*

Main category: cs.LG

TL;DR: UMF通过将MDLMs的解码过程构造为搜索树并使用蒙特卡洛树搜索优化生成路径，在复杂编程基准测试中优于现有测试时扩展方法，并展示出良好的数学推理扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展策略已有效提升自回归LLMs的推理能力，但掩码扩散语言模型(MDLMs)因其迭代和非自回归生成特性，天然适合采用更先进的搜索策略进行优化。

Method: 提出UnMaskFork(UMF)框架，将MDLMs的去掩码轨迹构造为搜索树，使用蒙特卡洛树搜索优化生成路径，通过多个MDLMs执行确定性部分去掩码动作进行空间探索。

Result: 在复杂编程基准测试中，UMF始终优于现有的测试时扩展基线方法，同时在数学推理任务上展示出强大的扩展能力。

Conclusion: UMF证明了MDLMs在推理任务中通过结构化搜索策略优化的潜力，为非自回归模型的测试时计算利用提供了新方向。
...
### [168] [MirrorLA: Reflecting Feature Map for Vision Linear Attention](https://arxiv.org/abs/2602.04346)
...
### [169] [Mosaic Learning: A Framework for Decentralized Learning with Model Fragmentation](https://arxiv.org/abs/2602.04352)
...
### [170] [Counterfactual Explanations for Hypergraph Neural Networks](https://arxiv.org/abs/2602.04360)
...
TL;DR: EXaMCaP是一种基于熵增益最大化的子集选择方法，用于高效评估图表理解数据集对多模态大语言模型的能力提升，替代耗时的全量微调评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过全量微调MLLMs来验证图表理解训练集的效果，但这种方法耗时严重，阻碍了数据集的迭代优化。需要一种更高效的评估方法来评估数据集对模型能力提升的贡献。

Method: 提出EXaMCaP方法，基于熵增益最大化从大规模图表理解数据集中选择高多样性子集，通过迭代选择样本来最大化集合熵增益，替代全量微调来评估数据集效果。

Result: 实验表明EXaMCaP在评估图表理解训练集能力提升方面优于基线方法，在不同子集大小下都表现良好，且兼容多种MLLM架构。

Conclusion: EXaMCaP提供了一种高效评估图表理解数据集效果的方法，显著减少了评估时间成本，支持数据集的快速迭代优化。
...
TL;DR: 该研究探索了联邦学习在可持续供应链管理中应用于生鲜杂货零售的需求预测和减少浪费，提出基于区块链的联邦学习模型，实现多零售商间不共享原始数据下的协作训练。


<details>
  <summary>Details</summary>
Motivation: 生鲜食品的需求预测对于减少食物浪费至关重要，但由于零售商间的数据隐私担忧阻碍了协作，限制了预测准确性的提升潜力。

Method: 首先开发了单个零售商场景下的需求预测和浪费评估基线模型，然后引入了基于区块链的联邦学习模型，允许多个零售商在不直接共享数据的情况下进行协作训练。

Result: 初步结果显示联邦学习模型的性能几乎等同于理想状态下各方共享数据的情况，并且明显优于各参与方不共享数据单独构建的模型，能够显著减少浪费并提高效率。

Conclusion: 联邦学习结合区块链技术能够在保护数据隐私的同时，通过零售商间的协作显著提升需求预测准确性，有效减少食物浪费，为可持续供应链管理提供了创新的解决方案。
...
### [175] [On the use of LLMs to generate a dataset of Neural Networks](https://arxiv.org/abs/2602.04388)
...
### [176] [LoRDO: Distributed Low-Rank Optimization with Infrequent Communication](https://arxiv.org/abs/2602.04396)
...
### [177] [Theory of Speciation Transitions in Diffusion Models with General Class Structure](https://arxiv.org/abs/2602.04404)
...
### [178] [Separation-Utility Pareto Frontier: An Information-Theoretic Characterization](https://arxiv.org/abs/2602.04408)
*Shizhou Xu*

Main category: cs.LG

TL;DR: 本文研究了效用与分离（separation）这一公平性标准的Pareto前沿（最优权衡），提供了信息论分析，开发了基于条件互信息的正则化方法，并在多个数据集上验证了其效果。


<details>
  <summary>Details</summary>
Motivation: 机器学习中的公平性问题日益重要，其中分离准则要求预测结果在真实结果条件下独立于敏感属性。现有的方法往往缺乏对效用与分离之间权衡的理论理解和可证明保证，导致实际应用中的选择困难。

Method: 1) 通过信息论视角分析效用-分离Pareto前沿，证明其性质和严格权衡的条件；2) 基于理论分析开发条件互信息（CMI）正则化器，可兼容任何基于梯度优化的深度模型；3) 在COMPAS、UCI Adult、UCI Bank和CelebA等数据集上进行数值实验验证。

Result: 理论方面：证明了效用-分离Pareto前沿的特性，确立了分离边际成本递增的规律。实验方面：在多个真实数据集上，提出的方法显著减少了分离违规，同时匹配或超越了基准方法的效用。

Conclusion: 本研究提供了一个可证明、稳定且灵活的方法来在深度学习中强制执行分离准则，为实践中权衡选择提供了理论指导，并通过条件互信息正则化器实现了有效的公平性约束。
...
### [179] [MaMa: A Game-Theoretic Approach for Designing Safe Agentic Systems](https://arxiv.org/abs/2602.04431)
...
### [180] [Greedy-Gnorm: A Gradient Matrix Norm-Based Alternative to Attention Entropy for Head Pruning](https://arxiv.org/abs/2602.04491)
...
### [181] [Forget to Generalize: Iterative Adaptation for Generalization in Federated Learning](https://arxiv.org/abs/2602.04536)
...
### [182] [Rethinking the Design Space of Reinforcement Learning for Diffusion Models: On the Importance of Likelihood Estimation Beyond Loss Design](https://arxiv.org/abs/2602.04663)
...
### [183] [Gradient Flow Through Diagram Expansions: Learning Regimes and Explicit Solutions](https://arxiv.org/abs/2602.04548)
*Dmitry Yarotsky,Eugene Golikov,Yaroslav Gusev*

Main category: cs.LG

TL;DR: 本文针对大规模学习问题中的梯度流(GF)提出了一个通用数学框架，通过类似费曼图的图示化展开分析不同的学习机制，并专注于高阶张量的CP分解模型，揭示了不同极端的惰性与丰富GF机制之间的依赖关系。


<details>
  <summary>Details</summary>
Motivation: 旨在分析大规模学习问题中梯度流的缩放机制并揭示不同学习phase的特性，尤其是在张量分解等复杂模型中，理解参数尺度、张量阶数和模型对称性如何影响梯度流行为。

Method: 建立损失函数演化的形式幂级数展开，并借助类似费曼图的图示化系数编码；提出了通用的求和法——将展开约化为偏微分方程（PDE），并在多种情形下通过特征线法求解该一阶PDE。

Result: 理论框架展示了多种极端惰性与丰富GF机制（如自由演化、NTK、欠参与过参数化的平均场机制），这些机制对参数尺度、张量阶数和对称性具有特定且微妙依赖，并与实验数据高度吻合。

Conclusion: 提出的形式展开和PDE缩减方法能有效分析梯度流的缩放机制并得到解析解，为理解大规模张量分解等模型中的梯度流行为和相变现象提供了系统理论工具。
...
### [186] [Let Experts Feel Uncertainty: A Multi-Expert Label Distribution Approach to Probabilistic Time Series Forecasting](https://arxiv.org/abs/2602.04678)
...
### [187] [Probabilistic Label Spreading: Efficient and Consistent Estimation of Soft Labels with Epistemic Uncertainty on Graphs](https://arxiv.org/abs/2602.04574)
*Jonathan Klees,Tobias Riedlinger,Peter Stehr,Bennet Böddecker,Daniel Kondermann,Matthias Rottmann*

Main category: cs.LG

TL;DR: 本文提出了一种概率标签传播方法，用于估计标注中的偶然不确定性和认知不确定性，显著降低了获取高质量标注所需的标注成本。


<details>
  <summary>Details</summary>
Motivation: 感知任务的安全人工智能面临挑战，部分原因是缺乏高质量标注数据。标注本身存在偶然不确定性和认知不确定性，而这些通常被忽视。尽管众包可以收集每个图像的多个标注来估计这些不确定性，但这种方法在大规模应用时因标注工作量过大而不切实际。

Method: 引入一种概率标签传播方法，该方法假设标签在特征空间中是平滑的，通过基于图的扩散方法传播单个标注。证明了即使每个数据点的标注数量收敛为零，标签传播也能产生一致的概率估计器。提出并分析了该方法的可扩展实现。

Result: 实验结果表明，与基线方法相比，该方法显著减少了在常见图像数据集上达到所需标注质量所需的标注预算。在Data-Centric图像分类基准测试上达到了新的最先进水平。

Conclusion: 所提出的概率标签传播方法能够可靠地估计标注中的不确定性，有效降低标注成本，为大规模高质量标注数据获取提供了实用解决方案。
...
### [188] [Stochastic Decision Horizons for Constrained Reinforcement Learning](https://arxiv.org/abs/2602.04599)
*Nikola Milosevic,Leonard Franz,Daniel Haeufle,Georg Martius,Nico Scherf,Pavel Kolev*

Main category: cs.LG

TL;DR: 论文提出了基于随机决策视野的控制即推理框架来处理约束马尔可夫决策过程，通过生存加权目标实现离策略兼容性，提出了两种违规划分方式，在标准基准上展示了更好的样本效率和权衡表现，并成功扩展到高维肌肉骨骼系统。


<details>
  <summary>Details</summary>
Motivation: 传统CMDP方法使用加性成本约束和对偶变量往往阻碍离策略可扩展性，需要一种能保持离策略兼容性的新框架来处理约束强化学习问题。

Method: 基于随机决策视野的控制即推理形式，约束违者会衰减奖励贡献并通过状态行为依赖的继续缩短有效规划视野，产生生存加权目标。提出了两种违规划分语义（吸收终止和虚拟终止），共享相同的生存加权回报但形成不同的优化结构，实现SAC/MPO风格的策略改进。

Result: 实验表明在标准基准上具有改进的样本效率和有利的回报-违规权衡。特别是MPO与虚拟终止（VT-MPO）在高维肌肉骨骼Hyfydy设置中有效扩展。

Conclusion: 提出的控制即推理框架通过生存加权目标解决了CMDP中的离策略兼容性问题，两种违规划分语义为约束强化学习提供了有效的优化结构，在复杂系统中表现出良好的可扩展性。
...
### [189] [Jacobian Regularization Stabilizes Long-Term Integration of Neural Differential Equations](https://arxiv.org/abs/2602.04608)
*Maya Janvier,Julien Salomon,Etienne Meunier*

Main category: cs.LG

TL;DR: 本文提出通过Jacobian正则化改善神经微分方程（NDE）的长时积分稳定性，特别是针对短训练轨迹场景，提出了已知和未知动力学两种正则化方法，显著提升了长期模拟的稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统NDE方法在长时积分中存在稳定性和精度问题，而使用完整轨迹进行训练计算成本过高。如何在短训练轨迹条件下提高长期模拟的稳定性成为关键挑战。

Method: 通过Jacobian的方向导数进行正则化：1）已知动力学场景下直接推导动态的方向导数；2）未知动力学场景下使用有限差分近似。两种方法相比长轨迹训练成本显著降低。

Result: 在多个常微分方程和偏微分方程实验中，两种正则化方法都成功提高了长时模拟的稳定性，证明在短训练轨迹条件下也能获得良好的长期积分效果。

Conclusion: Jacobian正则化为训练适用于大规模系统长时积分的NDE方法提供了有效途径，平衡了计算成本与模拟稳定性，为解决物理系统建模中的长期集成问题提供了新思路。
...
TL;DR: 通过正交化惩罚改善稀疏自编码器特征解耦，减少特征间的干扰和叠加，同时保持性能不变，并提高特征可辨识性、解释距离和模块化干预能力。


<details>
  <summary>Details</summary>
Motivation: 改善语言模型中稀疏自编码器的特征解耦，减少特征间的干扰和叠加问题，提升特征的可识别性和模块化表示以支持因果干预。

Method: 在微调语言模型时，对稀疏自编码器的解码器矩阵施加正交性惩罚，使特征近似正交，同时保持目标任务性能基本不变。

Result: 正交化惩罚减少了特征间的干扰和叠加，提高了特征的可辨识性，增大了特征解释的距离，并允许进行更孤立的干预操作。

Conclusion: 正交化惩罚能够产生更模块化、可识别的特征表示，这些特征更适合因果干预，同时保持了模型的性能不变。
...
### [191] [From Data to Behavior: Predicting Unintended Model Behaviors Before Training](https://arxiv.org/abs/2602.04735)
*Mengru Wang,Zhenqian Xu,Junfeng Fang,Yunzhi Yao,Shumin Deng,Huajun Chen,Ningyu Zhang*

Main category: cs.LG

TL;DR: Data2Behavior任务及MDF方法：通过数据特征操纵预测模型训练前的潜在偏差与安全风险


<details>
  <summary>Details</summary>
Motivation: 大型语言模型即使从看似良性的训练数据中也可能习得意外偏见，现有方法难以在微调前检测此类风险，导致事后评估成本高且低效

Method: 提出Manipulating Data Features（MDF）方法：通过对候选数据的平均表示进行总结，并将其注入基础模型的前向传播过程，让数据中的潜在统计信号影响模型激活，从而在不更新参数的情况下揭示潜在偏见和安全风险

Result: MDF在Qwen3-14B、Qwen2.5-32B-Instruct和Gemma-3-12b-it上的实验证实能有效预测意外行为并洞察预训练漏洞，且仅需约20%的GPU资源（相比微调）

Conclusion: MDF为训练前的风险预测提供了轻量级解决方案，能在资源消耗较少的情况下可靠预测模型可能产生的意外行为，有助于提前发现和改进模型安全性
...
### [192] [QUATRO: Query-Adaptive Trust Region Policy Optimization for LLM Fine-tuning](https://arxiv.org/abs/2602.04620)
...
TL;DR: A2MAML 是一个面向多智能体多模态异构感知系统的主动协作框架，通过显式建模模态特征的不确定性，支持细粒度模态级融合与鲁棒的多智能体协作。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体协同框架多停留在智能体层级的协调，并假设各智能体传感器的同构性与完备性，难以应对传感器异质性和模态间不确定性差异。同时，缺失显式不确定性建模，导致在传感器损坏或信息污染的场景中鲁棒性不足。

Method: A2MAML 为每类模态特征建模带有不确定性预测的随机估计，采用主动选择机制抽取可靠的“智能体-模态”对，最后基于贝叶斯反方差加权整合信息，实现对不对称模态可用性与多模态信息污染的稳健处理。

Result: 在互联自动驾驶协作事故检测实验中，模型持续超越了单智能体与现有协同基线的性能，最高实现了 18.7% 的事故检测率提升。

Conclusion: 该工作展示了显式建模多模态不确定性的重要性，A2MAML 提供了一个理论严谨的模态级协同机制，尤其适合传感器不一致、信息质量不一的复杂异构多智能体场景。
...
### [195] [MTS-JEPA: Multi-Resolution Joint-Embedding Predictive Architecture for Time-Series Anomaly Prediction](https://arxiv.org/abs/2602.04643)
...
### [196] [Team, Then Trim: An Assembly-Line LLM Framework for High-Quality Tabular Data Generation](https://arxiv.org/abs/2602.04785)
*Congjing Zhang,Ryan Feng Lin,Ruoxuan Bao,Shuai Huang*

Main category: cs.LG

TL;DR: 提出T²框架：通过LLM团队协作生成高质量表格数据，再经三阶段质量控制流程优化的方法。


<details>
  <summary>Details</summary>
Motivation: 高质量表格数据获取成本高且稀缺，常存在类别不平衡、选择偏差、低保真度等问题，影响机器学习应用效果。

Method: Team-then-Trim(T²)框架：1) 用专业知识指导的LLM团队顺序生成数据组件；2) 三阶段插件式质量控制管道对合成数据进行系统评估。

Result: 在模拟和真实数据集上的实验表明，T²在生成高质量表格数据方面优于现有最先进方法。

Conclusion: T²框架能生成高质量表格数据，在直接数据收集不可行时有效支持下游模型应用。
...
### [197] [SAFE: Stable Alignment Finetuning with Entropy-Aware Predictive Control for RLHF](https://arxiv.org/abs/2602.04651)
*Dipan Maity*
...
### [198] [Beyond Rewards in Reinforcement Learning for Cyber Defence](https://arxiv.org/abs/2602.04809)
...
### [199] [Safe Urban Traffic Control via Uncertainty-Aware Conformal Prediction and World-Model Reinforcement Learning](https://arxiv.org/abs/2602.04821)
...
TL;DR: 提出了GSBoG框架，用于在任意图上学习可执行的连续时间马尔可夫链策略，解决图传输问题中的可扩展性和约束满足挑战。


<details>
  <summary>Details</summary>
Motivation: 现有图传输方法存在表达性不足、拓扑适应性差、扩展性受限等问题，需要能够处理拓扑约束、状态成本且可执行的策略学习框架。

Method: 基于广义薛定谔桥理论，采用似然优化方法学习轨迹级策略，避免全局求解器，在满足端点边际分布的同时优化中间状态依赖的运行成本。

Result: 在真实世界复杂图拓扑上实验表明，GSBoG能够学习准确且尊重拓扑的策略，同时优化应用特定的中间状态成本，具有良好的可扩展性。

Conclusion: GSBoG为一般图上的成本感知动态传输提供了新颖且广泛适用的解决方案，开辟了新的研究方向。
...
### [201] [REDistill: Robust Estimator Distillation for Balancing Robustness and Efficiency](https://arxiv.org/abs/2602.04677)
...
### [202] [It's not a Lottery, it's a Race: Understanding How Gradient Descent Adapts the Network's Capacity to the Task](https://arxiv.org/abs/2602.04832)
*Hannah Pinson*
...
### [203] [From Evaluation to Design: Using Potential Energy Surface Smoothness Metrics to Guide Machine Learning Interatomic Potential Architectures](https://arxiv.org/abs/2602.04861)
...
### [204] [Subliminal Effects in Your Data: A General Mechanism via Log-Linearity](https://arxiv.org/abs/2602.04863)
*Ishaq Aden-Ali,Noah Golowich,Allen Liu,Abhishek Shetty,Ankur Moitra,Nika Haghtalab*

Main category: cs.LG

TL;DR: 研究发现偏好数据集中存在隐藏信息传递机制，通过Logit-Linear-Selection方法可以从通用数据集中选择特定子集，训练出的模型会表现出原本数据集中不直接可见的特定行为特征。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型训练使用大量算法和数据集，但实验表明数据集可以传递单个数据点无法观察到的信号，这对基于数据集理解LLM训练提出了概念性挑战，需要理解这种隐藏效应的基本机制。

Method: 基于LLMs的线性结构研究，提出了Logit-Linear-Selection (LLS)方法，该方法通过特定方式从通用的偏好数据集中选择子集，从而激发隐藏效应。

Result: 成功发现真实数据集的子集，使用这些子集训练的模型表现出特定偏好、对未见语言的响应能力、不同人格等行为。这些效应在选择的子集中持续存在，且在不同架构模型中都具有普遍性。

Conclusion: 数据集中存在隐藏信息传递的基本机制，LLS方法有效揭示了这一现象，表明数据集的影响比单个数据点的简单聚合更为复杂，对理解LLM训练过程具有重要意义。
...
### [205] [Static and auto-regressive neural emulation of phytoplankton biomass dynamics from physical predictors in the global ocean](https://arxiv.org/abs/2602.04689)
*Mahima Lakra,Ronan Fablet,Lucas Drumetz,Etienne Pauthenet,Elodie Martinez*

Main category: cs.LG

TL;DR: 深度学习模型（特别是UNet架构）结合海洋物理因子，能够有效重建和短期预测全球海洋浮游植物生物量的时空分布，优于传统CNN等模型。


<details>
  <summary>Details</summary>
Motivation: 浮游植物对海洋生态系统和全球生物地球化学循环至关重要，但现有生物地球化学模型因参数化有限、观测数据稀疏及海洋过程复杂等因素，难以准确模拟其动态变化。本文探索如何利用深度学习模型克服传统模型在预测浮游植物生物量时空分布方面的缺陷。

Method: 1. 比较了多种深度学习架构（包括CNN、ConvLSTM、4CastNet以及UNet）在基于卫星观测和环境条件预测浮游植物生物量时空分布方面的表现；2. 发现UNet在重现浮游植物生物量季节和年际模式方面表现最佳；3. 进一步开发了基于自回归的UNet变体，利用模型自身先前预测来预测未来状态，以提升时间序列预测能力。

Result: 1. 使用1-2个月环境数据输入时，UNet表现最佳，但存在对低频变化幅度低估的倾向；2. 自回归UNet在短期预测（最多5个月）中表现良好，但在更长时间尺度上性能下降；3. 将海洋物理预测因子与深度学习相结合，能够有效重建和短期预测浮游植物动态。

Conclusion: 结合海洋物理预测因子与深度学习（特别是UNet架构及其自回归变体）为浮游植物动态的重建和短期预测提供了有效工具。这些模型有望成为监测海洋健康、支持海洋生态系统管理（尤其是在气候变化背景下）的有力手段。
...
### [206] [Rethinking the Trust Region in LLM Reinforcement Learning](https://arxiv.org/abs/2602.04879)
...
### [207] [Bounded-Abstention Multi-horizon Time-series Forecasting](https://arxiv.org/abs/2602.04714)
...
### [208] [Contrastive Continual Learning for Model Adaptability in Internet of Things](https://arxiv.org/abs/2602.04881)
*Ajesh Koyatan Chathoth*

Main category: cs.LG

TL;DR: 论文回顾了在物联网中应用对比持续学习的方法，建立了算法设计与物联网系统现实之间的连接，提出了统一的数学公式、参考架构和评估指南。


<details>
  <summary>Details</summary>
Motivation: 物联网系统运行在非平稳动态环境中，面临传感器漂移、用户行为演变和隐私需求差异等挑战。传统方法难以在适应新数据时避免灾难性遗忘，而对比学习为提升模型鲁棒性和样本效率提供了潜力。本文旨在将两者结合，为物联网场景提供系统性的解决方案。

Method: 提出对比持续学习统一问题公式，融合对比和蒸馏损失目标；设计面向物联网的参考架构，支持设备端、边缘和云端部署；提供评估协议和指标指导。

Result: 构建了连接算法设计（重播、正则化、蒸馏、提示）与物联网现实（TinyML限制、间歇连接、隐私）的系统框架，为物联网中的对比持续学习提供了全面的方法论基础。

Conclusion: 对比持续学习在物联网中具有显著潜力，但仍面临独特挑战，包括处理表格与流数据、概念漂移、联邦学习和能耗感知训练等问题，需要进一步研究解决。
...
### [209] [Protein Autoregressive Modeling via Multiscale Structure Generation](https://arxiv.org/abs/2602.04883)
...
TL;DR: 该研究创建了一个标准化的生理控制基准数据集NBPDB，用于公平评估基于光电容积脉搏波（PPG）的无袖带血压估计模型。研究发现现有模型均未达到临床标准（AAMI/ISO 81060-2），但在加入人口统计学数据后，模型性能显著提升，特别是MInception模型达到了临床精度要求。


<details>
  <summary>Details</summary>
Motivation: 现有基于PPG的血压估计模型缺乏在生理控制条件下的标准化评估，公开数据集异质性强且缺乏公平基准，导致模型临床实用性不足。需要建立标准化基准数据集并评估模型在受控条件下的表现。

Method: 从MIMIC-III和VitalDB数据库中提取数据，构建包含101,453个高质量PPG片段的标准化基准数据集NBPDB。系统评估多个先进PPG模型，并通过添加年龄、性别、BMI等人口统计学数据改进模型性能。

Result: 所有评估的模型均未达到AAMI/ISO 81060-2临床精度标准（平均误差<5 mmHg，标准差<8 mmHg）。经过加入人口统计学数据改进后，所有模型性能均得到提升，其中MInception模型误差降低23%，收缩压平均绝对误差4.75 mmHg，舒张压2.90 mmHg，达到临床标准。

Conclusion: 现有基于PPG的血压估计模型在标准化条件下缺乏临床实用性，但引入人口统计学信息能显著提高模型的准确性和生理有效性，为实现可扩展的心血管健康评估提供了可行路径。
...
### [211] [DMFlow: Disordered Materials Generation by Flow Matching](https://arxiv.org/abs/2602.04734)
...
### [212] [Rationality Measurement and Theory for Reinforcement Learning Agents](https://arxiv.org/abs/2602.04737)
*Kejiang Qian,Amos Storkey,Fengxiang He*

Main category: cs.LG

TL;DR: 本文建立了强化学习智能体的合理性度量及相关理论分析框架，将部署时的理性行为定义为沿真实价值函数最陡方向最大化，提出了期望理性风险和理性风险间隙等概念，并分解为环境转移的外在因素和算法泛化的内在因素，通过实验验证了正则化技术和领域随机化的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习研究对智能体的合理性这一关键属性缺乏系统性探索，需要建立理论框架来量化分析智能体行为的理性程度，从而更好地理解算法在训练和部署阶段的表现差异及其原因。

Method: 提出理性度量和相管理论框架：定义完美理性动作为在部署时沿隐藏真实价值函数最陡方向最大化；提出期望理性风险作为策略行为与理性对应行为在部署轨迹中的期望值差异；定义理性风险间隙并分解为环境转移的外在因素和算法泛化能力的内在因素；分别用训练部署间的1-Wasserstein距离和经验Rademacher复杂度进行上界分析。

Result: 理论分析表明：环境转移导致的外在风险受训练和部署间转移核及初始状态分布的1-Wasserstein距离上界；算法泛化的内在风险受价值函数类的经验Rademacher复杂度上界。实验验证了正则化技术（层归一化、l2正则化、权重归一化）和领域随机化的积极作用，以及环境转移的负面影响。

Conclusion: 该理论框架为强化学习智能体的合理性提供了系统化分析工具，通过分解风险来源帮助理解算法在不同阶段的表现差异，为改进智能体稳健性和泛化能力提供了理论指导和实证支持。
...
### [213] [Decomposing Query-Key Feature Interactions Using Contrastive Covariances](https://arxiv.org/abs/2602.04752)
...
### [214] [A Dual-TransUNet Deep Learning Framework for Multi-Source Precipitation Merging and Improving Seasonal and Extreme Estimates](https://arxiv.org/abs/2602.04757)
...
### [215] [Improved Dimension Dependence for Bandit Convex Optimization with Gradient Variations](https://arxiv.org/abs/2602.04761)
*Hang Yu,Yu-Hu Yan,Peng Zhao*

Main category: cs.LG

TL;DR: 通过改进对非连续梯度变化的分析，显著提升了带两点反馈的强盗凸优化中梯度变化的维度依赖性，并在多种设置下首次实现了梯度变化的遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 传统全信息设置下的梯度变化在线学习已有深入研究，但带匪徒反馈（bandit feedback）的设置尚未充分探索。本文旨在提升两点反馈强盗凸优化中对非连续梯度变化的分析，改进维度依赖性，并扩展到更广泛的应用场景。

Method: 提出一种针对非连续梯度变化的精细化分析方法，改进了两点反馈强盗凸优化中梯度变化的维度依赖性。此外，将该技术推广至单点超矩形域线性优化，首次实现梯度变化边界，并验证其在动态/通用遗憾最小化和强盗博弈等任务中的有效性。

Result: 分析改进后，为凸函数和强凸函数带来了比已知最佳结果更好的维度依赖性。同时，在单点超矩形域线性优化中首次实现了梯度变化的边界。此外，在两点反馈强盗凸优化的动态/通用遗憾最小化及强盗博弈中，首次建立了梯度变化的动态和通用遗憾边界，并实现了快速的收敛速度。

Conclusion: 本文对非连续梯度变化的精细化分析显著提升了强盗凸优化的理论性能，并在多种设置下首次实现了梯度变化的遗憾边界。该技术展现出良好的可扩展性，为在线学习和博弈理论提供了新的理论工具。
...
### [216] [NeuroCanvas: VLLM-Powered Robust Seizure Detection by Reformulating Multichannel EEG as Image](https://arxiv.org/abs/2602.04769)
...
### [217] [Interval-Based AUC (iAUC): Extending ROC Analysis to Uncertainty-Aware Classification](https://arxiv.org/abs/2602.04775)
*Yuqi Li,Matthew M. Engelhard*

Main category: cs.LG

TL;DR: 本文提出了一种针对区间值预测的不确定性感知ROC框架，包括新的AUC_L和AUC_U度量，能够将ROC平面划分为三个区域，支持选择性预测，并能给出理论最优AUC的正式上下界。


<details>
  <summary>Details</summary>
Motivation: 在高风险风险预测中，通过区间值预测量化不确定性对可靠决策至关重要。但标准的ROC曲线和AUC等评估工具仅适用于点预测得分，无法捕捉预测不确定性对排序性能的影响。

Method: 提出了专门针对区间值预测的不确定性感知ROC框架，引入了两个新度量AUC_L和AUC_U。该框架能够将ROC平面信息性地划分为三个区域：正确排序、错误排序和不确定排序。这种方法自然支持选择性预测，允许模型避免为重叠区间的情况进行排序，从而优化弃权率与判别可靠性之间的权衡。

Result: 证明了在有效的类别条件覆盖下，AUC_L和AUC_U为理论最优AUC（AUC*）提供了正式的下界和上界，刻画了可达判别性的物理极限。在真实世界基准数据集上的实验验证了该框架的正确性，并展示了其在不确定性感知评估和决策中的实用价值。

Conclusion: 所提出的框架广泛适用于区间值预测模型，无论区间构建方法如何。它为高风险风险预测中的不确定性量化提供了有效的评估工具，能够在保持决策可靠性的同时支持选择性预测。
...
### [218] [Dynamical Regimes of Multimodal Diffusion Models](https://arxiv.org/abs/2602.04780)
...
### [219] [Legendre Memory Unit with A Multi-Slice Compensation Model for Short-Term Wind Speed Forecasting Based on Wind Farm Cluster Data](https://arxiv.org/abs/2602.04782)
*Mumin Zhang,Haochen Zhang,Xin Zhi Khoo,Yilin Zhang,Nuo Chen,Ting Zhang,Junjie Tang*

Main category: cs.LG

TL;DR: 提出WMF-CPK-MSLMU集成模型，用于风电场集群短期风速预测，结合加权均值滤波去噪、Legendre记忆单元和多切片补偿机制


<details>
  <summary>Details</summary>
Motivation: 风电场集群化并网对电力系统稳定运行至关重要，但现有预测方法在准确性、速度和鲁棒性方面存在不足，需要充分利用集群数据的时空相关性来提高预测性能

Method: 采用三阶段方法：1)加权均值滤波(WMF)进行单场数据去噪；2)创新的多切片LMU(MSLMU)架构，结合基于Kendall秩相关系数(CPK)的补偿参数进行建模；3)集成模型WMF-CPK-MSLMU包含数据预处理、预测和多切片补偿三个关键模块

Result: 在不同风电场集群上的测试结果表明，WMF-CPK-MSLMU模型在短期预测方面相比现有模型具有显著的有效性和优越性

Conclusion: 所提出的集成模型通过充分利用风电场集群的时空相关性，实现了准确、快速且鲁棒的短期风速预测，为风电场集群的有效并网和电力系统稳定运行提供了可靠的技术支持
...
### [220] [From independent patches to coordinated attention: Controlling information flow in vision transformers](https://arxiv.org/abs/2602.04784)
*Kieran A. Murphy*

Main category: cs.LG

TL;DR: 通过在视觉Transformer的注意力机制中插入变分信息瓶颈，使注意力传递的信息量变得显式可测量，从而能够训练具有明确信息成本的模型，并实现从局部处理到全局注意力的可控谱系


<details>
  <summary>Details</summary>
Motivation: 使注意力机制中传输的信息成为显式、可测量的量，从而获得更易于机制分析和控制的可解释模型

Method: 在所有注意力向残差流的写入处插入变分信息瓶颈，不改变其他架构，通过信息成本控制训练模型

Result: 在ImageNet-100上展示了分类行为和信息路由在谱系上的演变，分析了首个传递信息的注意力头，提供了全局视觉表征如何从局部补丁处理中涌现的初步洞见

Conclusion: 通过约束内部通信的方向进行学习偏向，获得了更易于机制分析和控制的模型
...
### [221] [Maximum-Volume Nonnegative Matrix Factorization](https://arxiv.org/abs/2602.04795)
*Olivier Vu Thanh,Nicolas Gillis*

Main category: cs.LG

TL;DR: 本文提出逆非负矩阵分解（MinVol NMF）的逆向方法——最大体积非负矩阵分解（MaxVol NMF），该方法最大化H矩阵的体积而非最小化W矩阵的体积。MaxVol NMF在噪声条件下表现更佳，能避免秩亏损并提取稀疏分解。


<details>
  <summary>Details</summary>
Motivation: MinVol NMF虽然能提高可解释性和唯一性，但在噪声条件下可能出现秩亏损问题。MaxVol NMF通过最大化因子矩阵H的体积来克服这些限制，在实际应用中特别是噪声条件下表现更好。

Method: 提出两种MaxVol NMF算法：1）标准MaxVol NMF算法，最大化H矩阵体积；2）归一化MaxVol NMF变体，结合标准NMF和正交NMF特点。所有方法均保持了非负性约束。

Result: 理论证明：最大体积解的MaxVol NMF能将X的列聚类为不相交簇，而最小体积解的MinVol NMF会产生秩亏损解。实验显示MaxVol NMF在噪声条件下更有效，能提取稀疏分解且不产生秩亏损。归一化变体性能优于MinVol NMF和标准MaxVol NMF。

Conclusion: MaxVol NMF是MinVol NMF的有效对偶方法，在噪声环境下具有更好的稀疏分解特性和避免秩亏损的优势。归一化MaxVol NMF进一步提升了性能，并能在标准NMF和正交NMF之间提供连续过渡。在高光谱解混场景中验证了其有效性。
...
### [222] [Evolving Afferent Architectures: Biologically-inspired Models for Damage-Avoidance Learning](https://arxiv.org/abs/2602.04807)
...
### [223] [Robust Generalizable Heterogeneous Legal Link Prediction](https://arxiv.org/abs/2602.04812)
...
### [224] [The Key to State Reduction in Linear Attention: A Rank-based Perspective](https://arxiv.org/abs/2602.04852)
*Philipp Nazari,T. Konstantin Rusch*

Main category: cs.LG

TL;DR: 这篇论文分析了线性注意力模型中普遍存在的低秩状态问题，提出通过结构化剪枝减少状态大小的方法来提升效率


<details>
  <summary>Details</summary>
Motivation: 线性注意力虽然计算高效，但实际训练后的模型状态常呈低秩结构，这表明模型未能充分利用其容量。低有效秩会放大查询噪声，影响检索准确性

Method: 提出一种硬件感知的结构化剪枝框架，专门针对键和查询矩阵进行剪枝，保持与现有CUDA核的兼容性。基于理论分析，提出一种基于秩揭示QR分解的新型结构化剪枝方法

Result: 在多个不同规模模型和各种下游任务上的实验表明，该框架能有效减少状态大小。特别是可以移除50%的查询和键通道，仅导致困惑度轻微增加

Conclusion: 论文证明线性注意力模型在训练后可通过最小化性能损失显著减少低秩状态，得到更快且内存效率更高的模型，为线性注意力的实际部署提供了有效工具
...
### [225] [Multi-Head LatentMoE and Head Parallel: Communication-Efficient and Deterministic MoE Parallelism](https://arxiv.org/abs/2602.04870)
