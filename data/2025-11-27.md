...
TL;DR: 该论文研究了针对多模态视觉语言模型的黑盒隐私攻击（成员推断攻击），并提出了一种神经科学启发的拓扑正则化框架来增强模型对隐私攻击的抵御能力。


<details>
  <summary>Details</summary>
Motivation: 多模态模型部署增加带来了新的隐私泄露风险，现有研究主要关注单模态AI系统的隐私攻击，而多模态模型对隐私攻击的脆弱性和神经启发模型在隐私保护中的作用尚未充分探索。

Method: 提出神经科学启发的拓扑正则化(τ)框架，在BLIP、PaliGemma 2和ViT-GPT2三种视觉语言模型上评估，使用COCO、CC3M和NoCaps三个基准数据集，比较基线模型和神经模型（带拓扑正则化）的抗攻击能力。

Result: 在BLIP模型和COCO数据集上，神经VLM使MIA攻击成功率平均ROC-AUC下降24%，同时保持相似的模型效用（基于MPNet和ROUGE-2指标）。其他模型在更多数据集上的评估进一步验证了结果的稳定性。

Conclusion: 神经启发的多模态视觉语言模型相比基线模型对隐私攻击更具抵御能力，且不会显著损害模型效用，为理解多模态模型隐私风险和构建更安全的AI系统提供了证据。
...
TL;DR: Inferix是一个专为世界模型设计的下一代推理引擎，通过优化的半自回归解码技术实现沉浸式世界合成，区别于传统视频扩散模型和高并发系统，支持交互式视频流和精细基准测试。


<details>
  <summary>Details</summary>
Motivation: 世界模型是智能体AI、具身AI和游戏等领域的核心模拟器，能够生成长时间、物理真实且交互的高质量视频。扩展这些模型可能解锁视觉感知、理解和推理的新兴能力，超越当前以LLM为中心的视觉基础模型范式。

Method: 采用半自回归（块扩散）解码范式，结合扩散和自回归方法的优势，在每个块内应用扩散生成视频令牌，同时以前面块为条件生成更连贯稳定的视频序列。通过LLM风格的KV缓存管理克服标准视频扩散的限制，实现高效、可变长度和高质量生成。

Result: Inferix作为专门优化半自回归解码过程的推理引擎，提供交互式视频流和性能分析功能，支持实时交互和真实模拟以准确建模世界动态。此外，通过无缝集成LV-Bench实现高效基准测试。

Conclusion: Inferix为世界模型探索提供了专门的推理引擎支持，希望社区共同推进该工具，促进世界模型的发展和应用。
...
TL;DR: DeeAD框架无需训练即可实现VLA规划的早期退出，通过在中间轨迹满足物理可行性时终止推理，显著降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 视觉语言动作模型（VLA）在自动驾驶中存在显着的推理延迟问题，主要源于深度Transformer堆栈的计算开销，需要在保持规划质量的同时降低延迟。

Method: 提出基于动作引导的早期退出框架，评估中间轨迹的物理可行性，采用多跳控制器自适应跳过冗余层，无需重新训练即可整合到现有VLA模型中。

Result: 在Bench2Drive基准测试中，实现了高达28%的Transformer层稀疏化和29%的延迟降低，同时保持了规划质量和安全性。

Conclusion: DeeAD证明了通过物理可行性评估实现训练自由早期退出的有效性，为VLA模型推理加速提供了实用解决方案。
...
TL;DR: DinoLizer基于DINOv2的视觉Transformer模型，通过在补丁嵌入上添加线性分类头来定位生成性修复图像中的篡改区域，使用滑动窗口策略处理大图像，后在多个数据集上优于最先进的局部篡改检测器。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够准确定位生成性修复图像中语义篡改区域的方法，将非语义编辑视为原始内容的一部分，提高篡改检测的准确性和鲁棒性。

Method: 基于DINOv2预训练模型，在其视觉Transformer的补丁嵌入上添加线性分类头，以14×14补丁分辨率预测篡改区域；采用滑动窗口策略处理大图像，并通过后处理优化二值篡改掩码。

Result: DinoLizer在多个生成性修复数据集上超越最先进的局部篡改检测器，平均IoU比次优模型高12%，对尺寸调整、噪声添加和JPEG压缩等后处理操作具有鲁棒性。

Conclusion: DinoLizer展示了视觉Transformer在生成图像篡改检测任务中的强大表示能力，通过综合消融研究确认了其相对于DINOv3的优越性。
...
TL;DR: CANVAS是为VLMs在基于工具的用户界面设计方面推出的首个基准测试，包含598项设计任务，用于评估模型通过工具调用来编辑UI设计的能力。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估基于工具的设计性能的基准测试，无法了解VLMs在常规设计软件中进行UI设计迭代的能力。

Method: 包含复制整个UI屏幕和修改现有屏幕特定部分两种任务类型，要求模型通过上下文工具调用来逐步更新设计。

Result: 领先模型表现出更具策略性的工具调用，提升了设计质量，同时识别出模型常见的错误模式。

Conclusion: CANVAS基准为未来增强基于工具的设计能力提供了指导方向，凸显了VLMs在设计软件中与设计师协作的潜力。
...
### [6] [MODEST: Multi-Optics Depth-of-Field Stereo Dataset](https://arxiv.org/abs/2511.20853)
*Nisarg K. Trivedi,Vinayak A. Belludi,Li-Yun Wang,Pardis Taghavi,Dante Lok*

Main category: cs.CV

TL;DR: 提出了首个高分辨率（5472×3648像素）立体DSLR数据集，包含18000张图像，系统性地在不同真实场景中改变焦距和光圈参数，填补了真实世界相机光学数据集的空白。


<details>
  <summary>Details</summary>
Motivation: 解决当前深度估计研究中缺乏大规模、高保真真实立体DSLR数据集的限制，这类数据集对于评估合成数据训练的模型在真实世界的泛化能力至关重要。

Method: 在9个复杂场景中使用两个相同的相机组件，系统采集10个焦距（28-70mm）和5个光圈（f/2.8-f/22）组合的50种光学配置图像，每场景2000张图像，包含专用校准图像集。

Result: 创建了首个覆盖全范围光学参数的立体数据集，支持几何和光学效应的控制分析，包括深度估计、浅景深渲染、去模糊、3D场景重建和新视角合成等任务。

Conclusion: 该数据集在真实相机光学和合成训练数据之间架起桥梁，揭示了当前最先进单目和立体深度估计方法面临的挑战，并支持可复现的真实世界光学泛化研究。
...
TL;DR: 提出了一种基于文本指导的语义图像编码器（TIE），能够根据输入文本查询生成图像表示，取代传统的与查询无关的图像编码器。该方法在多个基准测试中显著提升了性能，同时减少了图像标记数量，提高了推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型（VLM）中的图像编码器通常独立预训练，然后与语言模型对齐，导致其处理图像时与下游任务或文本查询无关，存在性能局限。

Method: 提出了文本指导的语义图像编码器（TIE），通过文本条件训练使图像编码器能够根据输入文本查询生成图像表示，关注与查询相关的视觉特征。

Result: 包含TIE的VLM在9个图像到文本基准测试中平均提升1.5和1.3个百分点（1B和3B规模），在DocVQA和InfoVQA等任务上提升高达6个百分点，同时仅使用一半图像标记，推理效率显著提高。

Conclusion: TIE方法能够有效优化图像编码器捕获关键视觉特征，实现更好的查询特定定位和可解释性，为视觉语言模型提供了更高效的图像编码方案。
...
TL;DR: 该论文提出了Δ-NeRF框架，解决NeRF在增量数据场景下的训练问题，通过模块化残差设计避免了灾难性遗忘，并在卫星图像数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF模型在引入新视角时需要完全重新训练，这限制了其在卫星地形分析等增量数据场景的应用。传统增量方法存在灾难性遗忘问题，且难以在无历史数据的情况下实现精炼。

Method: 提出Δ-NeRF框架，包括：残差控制器向冻结的基础NeRF注入逐层修正；不确定性感知门控机制自适应结合基础与精炼预测；视图选择策略减少47%训练数据；知识蒸馏将增强模型压缩至原始大小的20%。

Result: 在卫星图像实验中，Δ-NeRF达到与联合训练相当的性能，同时减少30-42%训练时间。相比朴素微调提升43.5% PSNR，部分指标优于联合训练。

Conclusion: Δ-NeRF为解决NeRF的增量学习问题提供有效方案，在保持性能的同时显著提升效率，适用于卫星地形监测等实际应用场景。
...
TL;DR: Split-then-Merge (StM)框架通过将未标注视频分为动态前景和背景层，然后自合成学习动态主体与场景交互，解决生成视频组合的控制和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决生成视频组合中的控制问题以及数据稀缺问题，传统方法依赖于标注数据集或手工规则，难以学习复杂的组合动态。

Method: 将大量未标注视频分割为动态前景和背景层，通过自合成学习主体与场景的交互；引入变换感知训练流程，使用多层融合和增强实现可供性感知组合，结合身份保持损失保持前景保真度。

Result: 实验表明StM在定量基准测试和人类/VLLM定性评估中均优于当前最先进方法。

Conclusion: StM框架能够有效学习复杂的组合动态，为生成视频组合提供了增强的控制能力，并通过自监督方式解决了数据稀缺问题。
...
TL;DR: Sphinx是一个针对认知基本原语的视觉感知推理合成环境，通过程序化生成包含多种元素的可验证谜题，用于评估大视觉语言模型，当前最优模型准确率仅51.1%，远低于人类表现，研究表明强化学习能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 开发和验证能够理解和推理视觉场景的核心认知能力的AI系统需要能够准确评估这些能力的测试平台，现有基准存在解决方案偏差且评估不够精确的问题。

Method: 构建Sphinx合成环境，程序化生成包含motifs、tiles、charts、icons和几何基本元素的谜题，提供可验证的ground-truth解决方案，涵盖25种任务类型，包括对称检测、几何变换、空间推理、图表解读和序列预测。

Result: 评估现有大视觉语言模型显示，即使是目前最先进的GPT-5模型准确率也仅为51.1%，远低于人类表现。强化学习与可验证奖励（RLVR）能显著提高模型在任务中的准确率，并在外部视觉推理基准上获得提升。

Conclusion: Sphinx为评估视觉感知和推理能力提供了精确的基准，现有模型仍有很大改进空间，强化学习方法显示出在推进多模态推理能力方面的巨大潜力。
...
TL;DR: RefTr是一个用于血管树中心线生成的3D图像到图模型，通过基于Transformer解码器的生成器-优化器架构，采用汇流轨迹的循环精炼方法，在保持正确树形拓扑的同时提高召回率。


<details>
  <summary>Details</summary>
Motivation: 人体内的管状结构（如血管和气道）对临床诊断至关重要，保持高召回率是关键，因为遗漏小分支可能导致致命错误。需要开发准确检测中心线且保持正确树形拓扑的方法。

Method: 采用生成器-优化器架构，生成器提出初始汇流轨迹，优化器通过循环精炼产生最终轨迹形成中心线图。使用汇流轨迹表示法确保有效树形拓扑，并引入高效的非极大值抑制算法合并重复分支。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前最先进方法的召回率和相当的精度，同时提供更快的推理速度和显著减少的参数数量（解码器参数减少2.4倍）。

Conclusion: RefTr展示出作为3D医学影像中血管树分析的新型最先进框架的潜力，通过循环精炼方案在保持高召回率的同时减少了模型复杂性。
...
TL;DR: 该论文提出首个大规模无监督可视化记忆数据集VL-ToT，包含超过82,000个视频及其描述性回忆数据，通过利用Reddit等平台的舌尖现象查询构建，在回忆生成和ToT检索任务上表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 视觉内容记忆性研究面临标注成本昂贵的问题，现有数据集仅收集聚合记忆分数，无法捕捉自然开放式回忆中的细微记忆信号，限制了该领域的发展。

Method: 利用Reddit等在线平台的舌尖现象检索查询构建大规模无监督数据集，采用对比训练策略创建多模态ToT检索模型，在数据集上微调大型视觉语言模型。

Result: 基于该数据集微调的视觉语言模型在生成视觉内容开放式记忆描述方面超越GPT-4o等最先进模型，并成功创建首个能够执行多模态ToT检索的模型。

Conclusion: 提出的数据集和模型为视觉内容记忆性研究开辟了新方向，解决了数据多样性不足的挑战，推动了该领域的进展。
...
TL;DR: 提出了一种同时估计雾模型参数的方法，可动态更新参数，避免误差传播。创建了首个包含真实雾天道路场景的立体数据集SDIRF。


<details>
  <summary>Details</summary>
Motivation: 传统方法顺序估计雾参数容易导致误差传播，且真实雾通常是全局不均匀的。需要一种能处理真实雾天情况的参数估计方法。

Method: 通过求解新颖的优化问题同时估计所有雾参数，假设雾是局部均匀的以处理全局不均匀的真实雾。可作为附加模块集成到现有SLAM或里程计系统中。

Result: 在合成雾数据和真实SDIRF数据集上的实验表明，该方法比现有方法更准确，能更好适应真实雾环境。

Conclusion: 该方法在雾参数估计方面优于现有方法，创建的SDIRF数据集将推动雾天视觉感知研究。代码和数据集已开源。
...
TL;DR: V^2-SAM是一个统一的跨视角物体对应框架，通过双提示生成器和多专家设计，将SAM2从单视角分割扩展到跨视角对应，在多个基准测试中取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 由于视角和外观的巨大变化，现有的分割模型如SAM2难以直接应用于跨视角物体对应任务，特别是自我视角和外部视角的对应问题。

Method: 通过两个互补的提示生成器：基于DINOv3特征的跨视角锚点提示生成器实现几何感知对应，跨视角视觉提示生成器通过特征和结构对齐增强外观引导；采用多专家设计和后验循环一致性选择器自适应选择最可靠的专家。

Result: 在Ego-Exo4D、DAVIS-2017和HANDAL-X三个基准测试中都取得了新的最优性能。

Conclusion: V^2-SAM成功地将SAM2适应到跨视角物体对应任务中，证明了所提方法的有效性和泛化能力。
...
TL;DR: 该论文提出Null-Text Test-Time Alignment (Null-TTA)方法，通过在分类器自由引导中优化无条件嵌入来实现扩散模型的测试时对齐，避免了现有方法在目标奖励函数上的欠优化或过度优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时对齐方法倾向于对目标奖励函数欠优化或过度优化（奖励欺诈），无法在语义层面有效对齐模型。

Method: 通过优化分类器自由引导中的无条件嵌入，而不是操纵潜在变量或噪声变量，使对齐在语义一致的流形上进行，防止非语义噪声模式的奖励欺诈。

Result: Null-TTA实现了最先进的测试时对齐性能，同时在跨奖励泛化方面保持强大能力。

Conclusion: 该研究确立了语义空间优化作为一种有效且原则性的测试时对齐新范式。
...
TL;DR: 使用深度学习模型降低小脑桥脑角增强T1加权MRI造影剂剂量的研究评估，证明在10%-30%标准剂量下可实现优质图像质量和诊断表征


<details>
  <summary>Details</summary>
Motivation: 降低增强MRI检查中的造影剂剂量，减少患者暴露于造影剂的风险，同时保持图像质量和诊断准确性

Method: 采用多中心回顾性研究，利用前庭神经鞘瘤患者的T1和增强T1加权图像模拟低剂量图像，训练深度学习模型从低剂量模拟图像恢复标准剂量图像质量

Result: 随着输入剂量增加，恢复图像的结构相似性指数从0.639提高到0.993，峰值信噪比从21.6dB增加到41.4dB。在10%输入剂量下，分割性能显著改善，Dice系数从0.673提升至0.734

Conclusion: 深度学习模型显著改善了低剂量小脑桥脑角MRI的图像质量，使用10%-30%标准剂量即可实现病灶检测和诊断表征
...
TL;DR: 提出一种平滑正则化技术，通过在轻量级视频识别模型中强制嵌入平滑性，利用高斯随机行走建模帧间变化，提升模型在Kinetics-600数据集上的准确率3.8%-6.4%


<details>
  <summary>Details</summary>
Motivation: 为轻量级视频识别模型注入强时间归纳偏置，使其更符合视频固有的时间连贯性特性

Method: 使用高斯随机行走（GRW）建模连续帧中间层嵌入的变化，惩罚表示突变，促进低加速度的平滑解

Result: MoViNets模型家族准确率提升3.8%-6.1%，MobileNetV3和MoViNets-Stream家族准确率提升4.9%-6.4%，在各自FLOP约束下达到新的最佳性能

Conclusion: 提出的平滑正则化技术能显著提升轻量级视频识别模型的性能，特别是在复杂时序动态捕捉方面表现优异
...
TL;DR: 本研究介绍了UruDendro4数据集，包含102个火炬松横截面图像和手工标注的年轮数据，支持年轮检测算法的自动训练和评估，其中DeepCS-TRD方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统人工测量树木年轮生长的方法耗时且不准确，缺乏足够的木材横截面数据集来支持自动化年轮检测算法的研发。

Method: 收集了102个不同树干高度的火炬松横截面图像样本，并进行手工年轮标注，使用深度学习方法进行自动年轮检测性能基准测试。

Result: DeepCS-TRD方法取得最佳性能：平均精度0.838，平均召回率0.782，适应兰德误差0.084；包含该数据集的训练可提升模型泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据稀缺的空白，支持树木年轮体积建模和自动检测算法开发，为林业研究提供了重要工具。
...
TL;DR: 本文提出了表达性图像合成任务和StickerNet框架，不再追求真实感，而是基于真实用户编辑行为学习具有表现力的贴纸放置策略。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究注重真实感，但现代内容创作平台上的用户更追求艺术性、趣味性和社交参与度，需要新的非现实导向的合成方法。

Method: 提出两阶段框架StickerNet，首先判断合成类型，然后预测透明度、遮罩、位置和缩放等放置参数。基于180万真实编辑行为构建数据集。

Result: 用户研究和量化评估显示，StickerNet优于常见基线方法，更接近人类放置行为，证明了从真实编辑模式学习的有效性。

Conclusion: 这项工作为视觉理解引入了新方向，强调表达性和用户意图而非真实感，反映了现实创意平台上的编辑实践。
...
TL;DR: TrafficLens 是一种针对多摄像头交通路口的优化算法，通过顺序处理摄像头重叠覆盖区域，利用不同token限制的VLM进行迭代处理，将前一个摄像头的输出作为后续摄像头的提示，从而在保持信息准确性的同时，将视频到文本的转换时间减少了4倍。


<details>
  <summary>Details</summary>
Motivation: 为了解决多摄像头交通视频数据量庞大、分析效率低下的问题，传统方法需要将视频数据转换为文本，过程耗时且延迟了及时获取交通洞见和调查事件的时效性。

Method: 采用顺序处理方法，利用摄像头重叠覆盖区域，迭代使用不同token限制的视觉语言模型(VLM)，将前一个摄像头的输出作为后续摄像头的提示，并通过对象级别相似性检测器智能绕过冗余的VLM调用。

Result: 实验结果显示，TrafficLens能够在保持信息准确性的同时，将视频到文本的转换时间减少高达4倍。

Conclusion: TrafficLens提供了一种高效的解决方案，显著提升了多摄像头交通视频的分析效率，在智能交通系统中具有重要应用价值。
...
TL;DR: 该论文提出了一个结合视觉变换器和同态加密的隐私保护联邦学习框架，用于安全的多机构病理学分类，既减少通信成本又防止模型梯度泄露风险。


<details>
  <summary>Details</summary>
Motivation: 医疗领域的协作机器学习面临隐私法规限制，传统联邦学习的模型梯度容易遭受重建攻击，可能暴露敏感医疗信息，需要更安全的隐私保护方案。

Method: 采用Vision Transformers和同态加密技术，使用ViT CLS令牌作为紧凑特征表示，通过CKKS同态加密保护这些令牌传输，实现安全聚合。

Result: CLS令牌加密相比梯度加密减少30倍通信量，有效防止模型反转攻击，加密域分类准确率达90.02%，未加密域达96.12%。

Conclusion: 该框架成功实现了隐私保护与模型性能的平衡，为医疗协作机器学习提供了安全高效的解决方案。
...
TL;DR: 提出一种基于双整流流的无反转风格迁移框架，通过并行预测内容和风格轨迹，使用动态中点插值融合，无需反向过程即可实现高效高质的风格迁移。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于扩散模型的方法依赖计算密集型反转过程导致效率低下和视觉失真的问题，寻求无需反转的前向风格迁移方案。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合两者的速度场，结合注意力注入引导风格集成，仅需前向传播。

Result: 实验表明该框架在多样化风格和内容上具有良好泛化能力，提供高效且有效的风格迁移管线，避免了传统方法的视觉失真问题。

Conclusion: 所提出的双整流流无反转框架为风格迁移任务提供了一种更高效、质量更好的解决方案，显著改进了现有方法的局限性。
...
TL;DR: 提出无需推理阶段参考图像的伪装目标检测框架，通过类原型记忆和查询条件原型混合实现参考向量的合成


<details>
  <summary>Details</summary>
Motivation: 现有参考伪装目标检测系统需要测试时参考图像，限制了部署效率并增加了数据收集负担

Method: 训练时将参考图像蒸馏到类原型记忆，推理时通过查询条件原型混合生成参考向量，提出双向注意力对齐模块弥合表示差异

Result: 在大规模R2C7K基准测试中表现出与最先进方法相当或更好的性能

Conclusion: 该方法为Ref-COD提供了一种简单高效的路径，无需强制使用参考图像
...
TL;DR: MLRM在视觉语言任务中生成的中间推理过程可能含有不安全内容，而现有安全防护方法仅评估最终答案。本文提出GuardTrace-VL系统，通过联合图像文本分析监控完整推理流程，并构建GuardTrace数据集和三阶段训练方案用于检测推理阶段的危险内容。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型的安全防护方法主要关注输入问题和最终答案，忽视了中间推理过程中可能出现的有害内容，这导致偏见推断或违规使用视觉上下文等风险无法被检测。

Method: 提出GuardTrace-VL安全审计器，通过联合图像文本分析监控完整的Question-Thinking-Answer流程；构建GuardTrace数据集，并采用三阶段渐进训练方案结合数据精炼流程。

Result: 在覆盖领域内和领域外场景的测试集上，GuardTrace-VL在不安全推理检测任务中达到93.1%的F1分数，相比之前最强多模态安全防御方法提升了13.5%。

Conclusion: GuardTrace-VL能够有效检测多模态大模型推理过程中出现的不安全内容，弥补了现有安全防护方法的不足，显著提升了不安全推理识别能力。
...
TL;DR: 该论文提出了一种新的图像分层分解方法，通过轻量级微调将基于扩散的修复模型适应于分层分解任务，并引入了多模态上下文融合模块来保护细节，在物体移除和遮挡恢复方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 图像可以看作是由前景和背景组成的分层结构，这种表示能够支持元素的独立编辑，为内容创作提供更大灵活性。尽管大型生成模型取得了进展，但由于方法和数据有限，从单张图像中分解出各层仍具有挑战性。观察到了分层分解与图像修复任务之间的强关联。

Method: 1. 通过轻量级微调将基于扩散的修复模型适配到分层分解任务；2. 引入了具有线性注意力复杂度的多模态上下文融合模块，以在潜在空间中保留细节；3. 仅使用开源资源构建的合成数据集进行训练。

Result: 该方法在物体移除和遮挡恢复方面实现了卓越性能，为下游编辑和创意应用开辟了新的可能性。

Conclusion: 该方法成功地建立了分层分解与修复任务之间的联系，通过创新的多模态上下文融合和轻量级微调策略，解决了单图像分层分解的挑战，并在实际应用中展现出强大的编辑能力。
...
TL;DR: MERGE是一个多模态实体感知检索增强生成框架，专门解决新闻图像描述中的信息覆盖不全、跨模态对齐不足和视觉实体定位欠佳三大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法存在三个关键问题：信息覆盖不完整、跨模态对齐较弱以及视觉实体定位欠佳，这限制了生成描述的新闻价值和信息准确性。

Method: 构建以实体为中心的多模态知识库，结合文本、视觉和结构化知识实现背景信息丰富检索；采用多阶段假设-描述策略改善跨模态对齐，通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上显著优于现有基准方法：CIDEr分别提升+6.84和+1.16，命名实体识别F1分数分别提升+4.14和+2.64。在未见过的Visual News数据集上表现优异，CIDEr提升+20.17，F1提升+6.22。

Conclusion: MERGE框架有效解决了新闻图像描述的核心挑战，展现出强大的生成效果、鲁棒性和领域适应性，为多模态新闻理解提供了新的解决方案。
...
TL;DR: MetaRank框架通过元学习自动选择最适合目标数据集的模型可迁移性评估指标，解决当前指标选择随意性问题


<details>
  <summary>Details</summary>
Motivation: 现有模型可迁移性评估指标选择方法随意，缺乏任务感知能力，不同指标在不同数据集上表现差异显著，尚无统一最优指标

Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和指标的文本描述，在共享语义空间中训练元预测器，采用列表排序目标优化

Result: 在11个预训练模型和11个目标数据集上的实验表明，MetaRank能有效为未见目标数据集选择最合适的评估指标

Conclusion: MetaRank为实践者提供了一种任务感知的自动化指标选择方案，显著提升了模型可迁移性评估的效率和准确性
...
TL;DR: 该文提出了一种基于原型的多视图分类框架，通过简化视图内邻域关系学习并实现视图内外结构动态对齐，来解决现有方法计算成本高、跨视图一致性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有可信多视图分类方法依赖全局密集邻域关系建模视图内依赖，导致计算成本高且无法保证跨视图关系一致性，同时缺乏对学习到的多视图邻域结构在类别空间内一致性的保证。

Method: 引入原型来表示每个视图的邻域结构，通过简化视图内邻域关系学习并实现视图内外结构动态对齐，促进跨视图共识的高效一致性发现。

Result: 在多个公开多视图数据集上的实验表明，该方法在下游性能和鲁棒性方面优于主流可信多视图分类方法。

Conclusion: 所提出的基于原型的TMVC框架能够更高效和一致地发现跨视图共识，在保持竞争力的同时显著提升计算效率和结构一致性。
...
TL;DR: CameraMaster：一个统一相机感知的图像润色框架，通过显式解耦相机指令和解码参数，实现精确的物理一致图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖模糊的文本提示阻碍精确相机控制，要么需要为参数调整单独训练权重，影响可扩展性和多参数组合能力。

Method: 提出CameraMaster框架，将相机指令和参数嵌入解耦融合：使用相机参数嵌入调制指令和内容语义，通过跨注意力注入调节内容特征，并作为条件信号注入时间嵌入实现统一层调制。

Result: 在构建的78K图像数据上进行实验，CameraMaster对参数变化产生单调近似线性响应，支持无缝多参数组合，显著优于现有方法。

Conclusion: 该方法实现了精确的物理一致图像润色，解决了参数控制的精确性和可组合性问题。
...
TL;DR: 提出了一个基于实用性的图像描述评估基准CaptionQA，通过下游任务效用衡量描述质量，在4个领域中构建了33,027个多选题，评估发现现有模型在描述效用上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法未能回答一个基本问题：图像描述是否能在实际下游任务中替代图像？需要一种基于任务效用的基准来评估模型生成的描述质量。

Method: 构建CaptionQA基准，覆盖自然、文档、电商和具身AI四个领域，包含25个一级类别和69个子类别的精细分类，创建33,027个密集标注的选择题（平均每个图像50.3题），通过LLM仅使用描述回答问题来直接衡量描述效用。

Result: 评估最先进的多模态大模型显示图像与其描述效用之间存在显著差距。在传统图像QA基准上表现相近的模型，在描述效用上可能下降高达32%。

Conclusion: CaptionQA有效揭示了当前图像描述模型的局限性，传统评估指标无法充分衡量描述在下游任务中的实际效用，需要更加关注描述的实用价值。
...
TL;DR: LungNoduleAgent是一个用于CT扫描分析的协作多智能体系统，通过分模块处理实现精准的肺结节描述和恶性分级。


<details>
  <summary>Details</summary>
Motivation: 现有模型在描述肺结节形态和融入医学专业知识方面存在挑战，影响临床可靠性。多智能体系统在病理学中的应用潜力尚未充分探索。

Method: 系统包含三个模块：Nodule Spotter协调检测模型识别结节；Radiologist整合图像描述技术生成CT报告；Doctor Agent System使用图像和CT报告进行恶性推理。
...
TL;DR: 提出了一种结合模型驱动和生成模型的新方法，通过密集物理约束来控制强大的生成先验，有效解决空间变化图像去模糊问题。


<details>
  <summary>Details</summary>
Motivation: 当前学习方法的局限性：模型驱动方法虽然物理约束强但会产生过度平滑和伪影，而生成模型感知质量好但物理约束弱导致细节虚构。

Method: 将退化场建模为高维压缩核的密集连续体，捕获运动和退化细微变化，并利用ControlNet架构配合这些丰富描述符条件化扩散采样过程。

Result: 广泛实验表明该方法有效弥合物理准确性和感知真实性之间的差距，在严重模糊场景中优于最先进的模型方法和生成基线。

Conclusion: 该方法独特地调和了两种范式，通过在物理约束下驯服强大的生成先验，实现了空间变化图像去模糊的重大改进。
...
TL;DR: MUSE 推出了首个统一的图像情感合成框架，既支持情感生成也支持情感编辑，避免了现有方法将二者人为分离的局限性，通过借鉴测试时间缩放策略实现了高效的端到端情感控制。


<details>
  <summary>Details</summary>
Motivation: 当前图像情感合成方法将生成和编辑任务人为分离，造成效率低下且限制在自然交织的场景中的应用，如心理治疗或叙事创作。

Method: 采用测试时间缩放策略，无需更新扩散模型；通过三个关键技术问题：梯度优化情感token、语义相似度确定引导时机、多情感损失减少干扰。

Result: 实验显示MUSE在所有方法中表现优异，同时提升了情感准确性和语义多样性，在内容保真度、文本一致性和情感表达间达到最佳平衡。

Conclusion: MUSE为情感合成建立了新范式，实现了统一的生成与编辑功能，推进了图像情感合成技术的发展。
...
TL;DR: 该研究提出T-NIG模型，通过将时间参数整合到正态逆伽马分布中，解决了阿尔茨海默病长期预测中不规则时间间隔导致的疾病特征保持问题，能够在序列数据分布不均衡时生成保持疾病特征的大脑图像并进行疾病预测。


<details>
  <summary>Details</summary>
Motivation: 针对阿尔茨海默病长期预测中，面对序列数据中不规则时间间隔时难以保持疾病相关特征的问题。由于时间相关特征分布能够反映图像分布不均时疾病特征的变化，需要一种能够处理这种时间分布不规律性的模型。

Method: 提出T-NIG模型，使用两个不同时间点的大脑图像创建中间图像、预测未来图像和进行疾病预测。模型利用坐标邻域识别特征，将时间参数整合到正态逆伽马分布中，学习具有不同时间间隔的脑成像序列中特征的变化方式，并使用不确定性估计减少因时间数据不足引起的模型不确定性。

Result: T-NIG模型在数据集中的短期和长期预测任务均展现出最先进的性能。实验结果表明，即使在面对不规则的时间数据分布时，T-NIG也能保持疾病相关特征，熟练预测疾病进展。

Conclusion: T-NIG模型通过整合时间参数的正态逆伽马分布和不确定性估计，有效解决了阿尔茨海默病长期预测中面对不规则时间间隔时的疾病特征保持问题，在短期和长期预测任务中均表现出卓越性能。
...
TL;DR: 该论文提出了一种新颖的基于3D-CNN的虹膜认证流程，通过学习丰富的时空表征来解决现有方法在处理旋转、尺度、反光和散焦模糊等问题时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有虹膜认证算法在面对旋转、尺度、反光和散焦模糊等变化时鲁棒性不足，且大多采用简单的点对点比较方法，未能有效利用虹膜模式的时空结构信息。

Method: 将虹膜图像沿一个维度分割成子图像序列，作为3D-CNN的输入来捕捉时空特征。采用课程学习方式训练模型，使用三重损失和ArcFace损失进行端到端训练。

Result: 该方法能够将时间依赖性直接嵌入到特征空间中，在深度度量域中提升区分性，对旋转、尺度、反光和模糊等挑战具有鲁棒性。

Conclusion: 提出的框架为虹膜认证提供了一个鲁棒且可推广的解决方案，增强了在现实应用场景中的性能表现。
...
TL;DR: RadarFM是一个雷达基础模型，通过学习结构化空间语言监督生成统一场景级表征，解决现有雷达方法的碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 雷达传感器在不同天气、光照和远距离条件下提供可靠感知，但现有雷达方法分散且任务特定，无法实现任务间迁移，同时基础模型在雷达领域的应用探索不足。

Method: 提出了结构化标注框架将车辆分布编码为雷达坐标系表示，以及哈希感知对比学习目标来量化连续场景相似性；利用CARLA模拟器生成大规模带标注雷达数据集。

Result: 开发了能够学习统一表征的雷达基础模型，并提出定位感知的评估指标来评估空间准确性。

Conclusion: RadarFM通过结构化语言监督实现了雷达感知的任务统一，为雷达基础模型的发展提供了新方法。
...
TL;DR: 本文提出EM-KD(一种增强高效多模态大语言模型知识蒸馏的新范式)，通过解决学生和教师模型间不平衡视觉标记的问题，并引入两种蒸馏策略来提升模型性能


<details>
  <summary>Details</summary>
Motivation: 现有方法通过压缩视觉标记来减少资源消耗，但视觉信息丢失会降低理解能力。先前工作虽然引入知识蒸馏，却忽略了高效学生模型和普通教师模型之间由于不平衡视觉标记导致的理解差异

Method: 1)用曼哈顿距离和匈牙利匹配算法对齐师生视觉标记；2)提出视觉-语言亲和力蒸馏(VLAD)，最小化师生亲和度矩阵的平滑L1距离；3)提出视觉语义蒸馏(VSD)，用反向KL散度测量对齐后视觉标记在词汇空间的离散概率分布

Result: 在多样化基准评估中，EM-KD训练模型在准确性和效率上都显著优于先前的高效MLLMs，相比采用相同视觉标记匹配策略的先前蒸馏方法也表现更优

Conclusion: EM-KD通过有效对齐师生视觉标记并实施两种蒸馏策略，成功提升了高效多模态大语言模型的性能，在精度和效率方面都取得显著提升
...
TL;DR: FaithFusion是一个结合3DGS和扩散模型的框架，通过期望信息增益统一指导空间合成和时间编辑，在保持几何保真度的同时解决大视点变化下的外观合成挑战。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在保持几何保真度的同时合成视觉上合理的外观，特别是在大视点变化下。当前3DGS与扩散模型融合缺乏像素级、3D一致的编辑标准，导致过度恢复和几何漂移问题。

Method: 提出基于期望信息增益的3DGS-扩散融合框架：EIG作为空间先验优化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS。这是一个即插即用的系统，无需额外先验条件和结构修改。

Result: 在Waymo数据集上的实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到SOTA性能，即便在6米车道偏移下仍能保持107.47的FID。

Conclusion: FaithFusion通过EIG驱动的统一策略实现了连贯的时空合成，有效解决了3D场景生成中的几何保真度和外观合成挑战，在自动驾驶场景重建中展现了突出优势。
...
### [39] [Which Layer Causes Distribution Deviation? Entropy-Guided Adaptive Pruning for Diffusion and Flow Models](https://arxiv.org/abs/2511.21122)
*Changlin Li,Jiawei Zhang,Zeyi Shi,Zongxin Yang,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

TL;DR: 提出EntPruner框架——基于熵引导的自动渐进剪枝方法，用于减少扩散模型和流模型在下游任务中的参数冗余，实现2.22倍推理加速同时保持生成质量


<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型在下游任务中存在显着的参数冗余，需要有效的剪枝方法。生成模型与判别模型不同，需要保持输出分布的多样性和条件保真度

Method: 1. 引入熵引导剪枝：基于条件熵偏差(CED)评估块级重要性；2. 提出零样本自适应剪枝框架：动态确定剪枝时机和程度，避免一次性剪枝的问题

Result: 在DiT和SiT模型上的实验显示，EntPruner实现了最高2.22倍推理加速，在ImageNet和三个下游数据集上保持竞争力的生成质量
...
### [40] [CtrlVDiff: Controllable Video Generation via Unified Multimodal Video Diffusion](https://arxiv.org/abs/2511.21129)
*Dianbing Xi,Jiepeng Wang,Yuanzhi Liang,Xi Qiu,Jialun Liu,Hao Pan,Yuchi Huo,Rui Wang,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: CtrlVDiff是一个统一的扩散模型，通过融合深度、法线、分割、边缘和图形本征等多模态控制信号，实现视频理解和可控视频生成的统一框架，提供强大的时间一致性和分层编辑能力。


<details>
  <summary>Details</summary>
Motivation: 解决视频理解和可控生成的双重挑战，传统几何线索不足以约束外观、材质和光照，需要图形化模态提供互补约束以实现精确可预测的控制。

Method: 提出CtrlVDiff扩散模型，采用混合模态控制策略融合多种控制信号，构建MMVideo混合数据集提供多模态对齐监督。

Result: 在理解和生成基准测试中表现优异，实现分层编辑（重光照、材质调整、对象插入），优于现有基线方法且在模态缺失时保持鲁棒性。

Conclusion: 图形本征与语义的融合显著提升了视频生成的可控性，统一的扩散框架能够有效处理异构模态并保持时间一致性。
...
### [41] [Efficient Training for Human Video Generation with Entropy-Guided Prioritized Progressive Learning](https://arxiv.org/abs/2511.21136)
*Changlin Li,Jiawei Zhang,Shuhao Liu,Sihao Lin,Zeyi Shi,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

TL;DR: 提出了Ent-Prog框架，通过条件熵膨胀评估模型组件重要性实现优先级训练，结合自适应渐进式调度，在保持性能的同时提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 人类视频生成模型训练存在高计算成本和内存消耗的挑战

Method: 使用条件熵膨胀（CEI）评估模型组件重要性，结合自适应渐进式调度动态增加计算复杂度

Result: 在三个数据集上实现2.2倍训练加速和2.4倍GPU内存减少，且不损失生成性能

Conclusion: Ent-Prog框架有效解决了人类视频生成中训练效率问题
...
TL;DR: 针对文生视频模型的安全隐患，提出一种面向时间动态性的自动红队框架TEAR，能够成功攻击开源和商业系统，攻击成功率超过80%。


<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要关注静态图像和文本生成，无法有效捕捉视频生成中复杂的时间动态特性，因此需要专门针对文生视频模型安全隐患的评估框架。

Method: 使用时间感知的测试生成器，通过两阶段优化（初始生成器训练和时间感知在线偏好学习），生成看似无害但能利用时间动态性诱导违规视频的文本提示，同时采用精炼模型循环改进提示的隐蔽性和对抗效果。

Result: 在开源和商业文生视频系统上的实验验证，TEAR的攻击成功率超过80%，较之前57%的最佳结果有显著提升。

Conclusion: TEAR框架成功揭示了文生视频模型在时间动态性方面的安全隐患，为视频生成安全评估提供了有效工具。
...
TL;DR: 论文提出LLaVA-UHD v3，通过渐进式视觉压缩方法解决全局原生分辨率视觉编码计算量大的问题，在保持性能的同时显著减少时间到第一token


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs越来越多采用全局原生分辨率视觉编码而非基于切片的方法，这提升了整体能力但带来更大计算开销，需要解决此效率问题

Method: 提出渐进式视觉压缩方法，包含两个关键模块：细化补丁嵌入支持灵活补丁尺寸缩放、窗口化token压缩在ViT层间分层部署以逐步聚合局部token表示

Result: 转换后的ViT-UHD在相同MLLM架构下性能与MoonViT相当，但TTFT减少2.4倍；LLaVA-UHD v3性能与Qwen2-VL相当，TTFT进一步减少1.9倍

Conclusion: 渐进式视觉压缩方法可将预训练ViT重构为高效架构，在保持通用性的同时显著提升MLLMs的效率，为高效MLLMs研究提供了新思路
...
TL;DR: GridAR是一个测试时扩展框架，通过网格分区渐进生成和布局指定提示重构策略，提升视觉自回归模型的生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 当前测试时计算扩展策略在视觉自回归模型中应用效果不佳，存在消耗计算资源在错误生成轨迹上，以及缺乏整个画布蓝图的问题。

Method: GridAR采用网格分区渐进生成方案，在同一位置生成多个部分候选，早期剪枝不可行候选，并用可行候选作为锚点指导后续解码。结合布局指定提示重构策略，从部分视图推断可行布局。

Result: 在N=4时，GridAR在T2I-CompBench++上比Best-of-N（N=8）性能提升14.4%，成本降低25.6%。在图像编辑任务中，PIE-Bench上语义保存度提升13.9%。

Conclusion: GridAR框架有效解决了视觉自回归模型测试时扩展的挑战，在有限扩展下实现更高质量的生成结果，并适用于图像编辑任务。
...
TL;DR: AnchorOPT是一个动态锚点提示学习框架，解决现有方法中锚点静态固定的问题，通过动态学习锚点值和自适应优化锚点位置来提升CLIP模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的提示学习方法使用静态锚点（文本标记）来引导可学习的软标记，但锚点的值和位置固定不变，缺乏跨任务和阶段自适应的灵活性。

Method: 引入动态锚点机制：锚点值从任务特定数据中动态学习而非手工设计，锚点与软标记的位置关系通过可学习的位置矩阵自适应优化。采用两阶段训练策略。

Result: 仅使用简单的可学习锚点和位置矩阵就能达到或超过一些包含额外可学习模块或正则化技术的复杂方法的性能。作为即插即用模块，在多个数据集上都能带来一致的性能提升。

Conclusion: AnchorOPT通过动态锚点机制有效提升了提示学习的灵活性，实现与复杂方法相当甚至超越的性能，具有良好的可集成性和泛化能力。
...
### [46] [Scenes as Tokens: Multi-Scale Normal Distributions Transform Tokenizer for General 3D Vision-Language Understanding](https://arxiv.org/abs/2511.21191)
*Yutao Tang,Cheng Zhao,Gaurav Mittal,Rohith Kukkala,Rama Chellappa,Cheng Peng,Mei Chen*

Main category: cs.CV

TL;DR: NDTokenizer3D是一个通用3D视觉语言模型，使用基于多尺度正态分布变换(NDT)的三阶段场景token化流程，将3D点云转换为可由LLM处理的整体场景token，统一支持3D参考分割、视觉问答和密集描述等多种任务。


<details>
  <summary>Details</summary>
Motivation: 有效Token化3D场景并利用这些token处理各种3D理解任务具有挑战性，需要将语言推理与3D空间理解相结合。

Method: 构建多尺度NDT表示，保留全局上下文和细粒度几何细节；使用多尺度NDT解码器融合跨尺度特征生成场景token；该解码器还可作为通用接口支持人机交互提示和分割掩码解码。

Result: 在3D参考分割、3D视觉问答和3D密集描述任务中取得了显著改进。

Conclusion: NDTokenizer3D提供了一个紧凑统一的通用3D VLM架构，能够有效桥接语言推理与3D空间理解。
...
### [47] [When Robots Obey the Patch: Universal Transferable Patch Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21192)
*Hui Lu,Yi Yu,Yiming Yang,Chenyu Yi,Qixin Zhang,Bingquan Shen,Alex C. Kot,Xudong Jiang*

Main category: cs.CV

TL;DR: 提出UPA-RFAS框架，一种针对视觉-语言-动作模型的通用可迁移对抗攻击方法，能够在未知架构、微调变体和仿真到真实场景中进行有效攻击。


<details>
  <summary>Details</summary>
Motivation: 当前大多数对手补丁方法过拟合于单一模型，在未知模型的黑盒设置下效果不佳，需要研究通用可迁移的攻击方法。

Method: UPA-RFAS采用共享特征空间统一框架，结合特征空间优化、鲁棒增强的双阶段极小极大过程、以及特定的注意力劫持和语义错配损失函数。

Result: 在多种VLA模型、操作环境和物理执行中显示出持续的跨模型、跨任务和跨视角迁移能力。

Conclusion: 该方法暴露了基于补丁的实用攻击面，为未来防御研究建立了强基线。
...
### [48] [BotaCLIP: Contrastive Learning for Botany-Aware Representation of Earth Observation Data](https://arxiv.org/abs/2511.21194)
*Selene Cerna,Sara Si-Moussi,Wilfried Thuiller,Hadrien Hendrikx,Vincent Miele*

Main category: cs.CV

TL;DR: BotaCLIP是一个轻量级多模态对比学习框架，通过对齐高分辨率航拍图像与植物样方数据来调整预训练的地球观测基础模型DOFA，以注入领域知识，适用于稀缺数据场景。


<details>
  <summary>Details</summary>
Motivation: 解决在不重新训练或高计算成本下，为预训练基础模型注入领域特定知识的挑战，特别是在生物多样性建模等数据稀缺的实际应用中。

Method: 引入BotaCLIP框架，采用对比学习对齐航拍图像和植物样方数据，并通过正则化策略缓解灾难性遗忘，生成可迁移的下游表示。

Result: 在植物存在预测、蝴蝶出现建模和土壤营养组丰度估计三个生态任务中，BotaCLIP表示相比DOFA和监督基线模型均取得一致改进。

Conclusion: 这项工作展示了领域感知的基础模型调整能够为数据稀缺环境注入专家知识，实现节约的表示学习。
...
### [49] [Towards an Effective Action-Region Tracking Framework for Fine-grained Video Action Recognition](https://arxiv.org/abs/2511.21202)
*Baoli Sun,Yihan Wang,Xinzhu Ma,Zhihui Wang,Kun Lu,Zhiyong Wang*

Main category: cs.CV

TL;DR: 提出了Action-Region Tracking (ART)框架，通过查询响应机制发现并跟踪细粒度动作中局部细节的动态变化，解决了现有方法难以捕捉局部细微区域随时间演化细节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度动作识别方法往往只能捕捉粗粒度的运动模式，但难以识别随时间演化局部区域的细微细节，需要更好地区分相似动作。

Method: ART框架使用查询响应机制，通过区域特定语义激活模块捕捉与动作最相关的区域响应，组织成动作轨迹，并设计多层次轨迹对比约束。采用任务特定的微调机制优化文本语义。

Result: 在广泛使用的动作识别基准测试上进行综合实验，证明该方法优于先前最先进的基线方法。

Conclusion: 提出的ART框架通过发现和跟踪局部细节的动态变化，能够有效区分相似的细粒度动作，显著提升了细粒度动作识别性能。
...
### [50] [From Diffusion to One-Step Generation: A Comparative Study of Flow-Based Models with Application to Image Inpainting](https://arxiv.org/abs/2511.21215)
*Umang Agarwal,Rudraksh Sangore,Sumit Laddha*

Main category: cs.CV

TL;DR: 比较DDPM、CFM和MeanFlow三种生成模型，CFM在50步采样下FID为24.15优于DDPM(402.98)，MeanFlow单步采样FID为29.15且推理时间减少50倍。CFM扩展至图像修复，多种掩码类型下取得显著改进：PSNR从4.95增至8.57dB(+73%)，SSIM从0.289增至0.418(+45%)。


<details>
  <summary>Details</summary>
Motivation: 对三种生成建模范式进行全面比较研究：DDPM、CFM和MeanFlow，探索迭代采样与直接一步生成的性能差异，并验证inpainting感知训练的有效性。

Method: 使用统一TinyUNet架构(<1.5M参数)在CIFAR-10上实现三种方法，CFM采用条件流匹配，MeanFlow通过建模时间间隔内的平均速度实现一步生成，并扩展CFM至图像修复，实现四种掩码类型(中心、随机边界框、不规则、半图像)的掩码引导采样。

Result: CFM在50步采样下FID为24.15，显著优于DDPM的402.98；MeanFlow单步采样FID为29.15，推理时间减少50倍；图像修复模型在中心掩码下PSNR从4.95增至8.57dB(+73%)，SSIM从0.289增至0.418(+45%)。

Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow在保持竞争力的生成质量下实现50倍推理加速，inpainting感知训练能显著提升图像修复性能，验证了条件流匹配在生成任务中的有效性。
...
TL;DR: FIELDS使用双重监督策略从单张图像重建3D人脸，准确捕捉微妙的情感细节。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法依赖2D监督且缺乏3D真值，导致无法准确捕捉微妙的情感表达细节。

Method: 结合自监督2D图像一致性、直接3D表情参数监督和辅助情感识别分支，采用双重监督策略减少2D/3D领域差异和表情强度偏差。

Result: 生成的情感丰富3D人脸模型具有高度真实的表情，显著提升野生环境下的人脸表情识别性能且不牺牲自然度。

Conclusion: FIELDS成功弥合了2D/3D领域差异，实现了保留微妙情感线索的高保真3D重建。
...
### [52] [AVFakeBench: A Comprehensive Audio-Video Forgery Detection Benchmark for AV-LMMs](https://arxiv.org/abs/2511.21251)
*Shuhan Xia,Peipei Li,Xuannan Liu,Dongsen Zhang,Xinyu Guo,Zekun Li*

Main category: cs.CV

TL;DR: AVFakeBench是一个全面的音频视频伪造检测基准，包含12K个精心策划的问题，涵盖7种伪造类型和4级注释，专门针对人类主题和通用主题的多样化伪造场景。


<details>
  <summary>Details</summary>
Motivation: 现有的伪造检测基准主要局限于DeepFake伪造和单粒度注释，无法捕捉现实世界伪造场景的多样性和复杂性，因此需要更全面的基准来评估伪造检测能力。

Method: 提出了多阶段混合伪造框架，集成了用于任务规划的专属模型和用于精确操作的专家生成模型。建立了多任务评估框架，涵盖二元判断、伪造类型分类、伪造细节选择和解释性推理。

Result: 评估了11个音频视频大语言模型和2种流行检测方法，证明AV-LMMs作为新兴伪造检测器的潜力，但揭示了它们在细粒度感知和推理方面的显著弱点。

Conclusion: AVFakeBench为音频视频伪造检测提供了全面的基准，揭示了现有检测方法特别是AV-LMMs在复杂伪造场景下的局限性和改进空间。
...
TL;DR: LaGen是首个能够逐帧自回归生成长时序LiDAR场景的框架，可通过单帧LiDAR输入和包围框条件生成高保真4D场景点云。


<details>
  <summary>Details</summary>
Motivation: 当前LiDAR数据生成方法仅支持单帧生成，而预测方法需要多帧历史输入且只能确定性预测多帧，缺乏交互性，两者都不支持长时序交互生成。

Method: LaGen框架包含场景解耦估计模块和噪声调制模块，支持单帧输入和包围框条件进行逐帧自回归生成，增强了对象级内容的交互生成能力并减轻长时序生成中的误差累积。

Result: 在基于nuScenes构建的长时序LiDAR场景生成评估协议下，LaGen超越现有最先进的LiDAR生成和预测模型，尤其在后续帧上表现优异。

Conclusion: LaGen是首个支持逐帧自回归生成长时序LiDAR场景的框架，在长时序生成任务中显著优于现有方法。
...
### [54] [Unlocking Zero-shot Potential of Semi-dense Image Matching via Gaussian Splatting](https://arxiv.org/abs/2511.21265)
*Juncheng Chen,Chao Xu,Yanjun Cao*

Main category: cs.CV

TL;DR: MatchGS是一个利用精化3D高斯泼溅生成高质量图像匹配数据的框架，通过几何校正和2D-3D表示对齐，提升零样本图像匹配性能达17.7%。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅在图像匹配应用中存在的几何不准确性和深度渲染偏差问题，为学习型图像匹配提供大规模、多样且几何精确的训练数据。

Method: 采用两阶段方法：1）几何精确的数据生成流程，精化3DGS几何结构生成精确对应标签；2）2D-3D表示对齐策略，将3DGS的显式3D知识注入2D匹配器。

Result: 生成的地面真值对应关系将极线误差降低40倍，在公开基准测试中使零样本性能提升高达17.7%。

Conclusion: 证明经过适当几何精化的3DGS可作为可扩展、高保真且结构丰富的数据源，为新一代鲁棒零样本图像匹配器开辟道路。
...
### [55] [Co-Training Vision Language Models for Remote Sensing Multi-task Learning](https://arxiv.org/abs/2511.21272)
...
TL;DR: 提出RSCoVLM，一种用于遥感多任务学习的视觉语言模型基线，通过数据引擎、动态分辨率策略和Zoom-in Chain机制解决遥感数据处理挑战，在多个任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在单项遥感任务表现优异，但需要一个能在多任务学习中表现优秀的统一模型。多任务学习相比单任务方法具有更好的泛化性、可扩展性和实用性。

Method: 开发数据管理引擎处理复杂遥感数据环境；提出统一动态分辨率策略应对不同图像尺度；针对超高清图像引入Zoom-in Chain机制；增强目标检测能力；建立新的评估协议

Result: RSCoVLM在多样化任务上达到最先进的性能，超越了现有的遥感视觉语言模型，甚至与专门的专家模型相媲美

Conclusion: 该基线模型为通用遥感模型的进一步发展提供了支持，所有训练评估工具、模型权重和数据集均已开源以确保可复现性
...
### [56] [HTTM: Head-wise Temporal Token Merging for Faster VGGT](https://arxiv.org/abs/2511.21317)
*Weitian Wang,Lukas Meiner,Rai Shubham,Cecilia De La Parra,Akash Kumar*

Main category: cs.CV

TL;DR: 提出HTTM方法解决大场景3D重建中的延迟瓶颈，通过多头时间合并技术实现训练免费加速，在GPU推理中达到7倍加速且性能损失可忽略


<details>
  <summary>Details</summary>
Motivation: VGGT模型进行3D场景重建时，由于全局面注意力计算导致大场景重建出现显著延迟瓶颈

Method: HTTM（多头时间合并）方法，在多头粒度合并token，利用空间局部性和时间对应关系，在头级别实现更高合并比和更低合并成本

Result: 在GPU推理中实现高达7倍加速，性能损失可忽略不计

Conclusion: HTTM是一种高效的无训练3D token合并方法，显著加速VGGT模型，同时保持模型表示能力
...
TL;DR: ConFu框架通过对比学习联合嵌入单个模态及其融合组合，捕获多模态间的高阶依赖关系，同时保持强成对对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要集中在成对模态对齐，但忽视了多模态间的高阶交互，且无法有效保留对单模态任务的成对关系。

Method: 引入对比融合(ConFu)框架，在传统成对对比目标基础上增加融合模态对比项，将模态及其融合对应对齐到统一表示空间。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对多检索。

Conclusion: ConFu能够捕获仅靠成对对齐无法恢复的高阶依赖关系，同时在单模态任务中保持强成对对应。
...
### [58] [Hybrid SIFT-SNN for Efficient Anomaly Detection of Traffic Flow-Control Infrastructure](https://arxiv.org/abs/2511.21337)
*Munish Rathee,Boris Bačić,Maryam Doborjeh*

Main category: cs.CV

TL;DR: SIFT-SNN框架结合SIFT空间特征编码和SNN，实现实时交通基础设施异常检测，准确率92.3%，延迟9.5ms，适用于嵌入式边缘部署。


<details>
  <summary>Details</summary>
Motivation: 开发低延迟、可解释的神经形态信号处理系统，用于交通基础设施结构异常的实时监测，解决传统CNN方法缺乏空间特征定位的问题。

Method: 集成尺度不变特征变换(SIFT)进行空间特征编码，结合延迟驱动的脉冲转换层和LIF SNN进行分类。

Result: 在奥克兰海港大桥数据集上实现92.3%分类准确率，每帧推理时间9.5毫秒，脉冲活动稀疏度为8.1%。

Conclusion: SIFT-SNN框架在保持空间特征定位的同时支持实时低功耗部署，增强可解释性，但到未见现场条件的泛化能力仍需验证。
...
TL;DR: SurgMLLMBench是一个统一的多模态基准测试平台，专门用于开发和评估手术场景理解中的交互式多模态大语言模型，包括新收集的MAVIS数据集


<details>
  <summary>Details</summary>
Motivation: 现有手术数据集主要采用视觉问答格式，存在分类学异构性且缺乏像素级分割支持，限制了评估的一致性和适用性

Method: 整合了像素级器械分割掩码和结构化VQA标注，涵盖腹腔镜、机器人辅助和显微手术领域，在统一分类学下实现丰富视觉对话交互

Result: 在SurgMLLMBench上训练的单一模型在多个领域表现一致，并能有效泛化到未见过的数据集

Conclusion: SurgMLLMBench将作为公开资源推进多模态手术AI研究，支持交互式手术推理模型的可重现评估和开发
...
TL;DR: 提出了Endo-G²T方法，通过几何引导和时间感知训练解决内窥镜视频中的几何漂移问题，结合单目深度先验、时间嵌入高斯场和关键帧约束流式处理，实现动态场景的4D重建。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈视角依赖性效应，纯光度监督会导致几何漂移，需要早期锚定几何形状并保持时间一致性和效率。

Method: 1) 几何引导先验蒸馏转换置信度门控的单目深度为监督；2) 时间嵌入高斯场在XYZT空间中表示动态；3) 关键帧约束流式处理在最大点数预算下优化关键帧。

Result: 在EndoNeRF和StereoMIS-P1数据集上实现了单目重建基线中的最先进结果。

Conclusion: Endo-G²T方法成功解决了内窥镜动态场景中的几何漂移问题，实现了高质量的时间一致性重建。
...
TL;DR: Monet训练框架使多模态大语言模型能在潜在视觉空间直接推理，通过生成连续嵌入作为中间视觉思考，显著提升视觉推理能力并在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有方法由于依赖外部工具，在灵活性上受到限制，无法实现类似人类的抽象视觉推理。作者希望开发能够直接在潜在视觉空间进行推理的模型。

Method: 提出三阶段蒸馏监督微调流程（SFT），包括图像-语言蒸馏、混合SFT和视觉思考SFT。同时提出VLPO强化学习方法，明确将潜在嵌入纳入策略梯度更新。

Result: Monet-7B模型在真实世界感知和推理基准上取得一致改善，在具有挑战性的抽象视觉推理任务上展现出强大的分布外泛化能力。

Conclusion: 该工作展示了让MLLMs在潜在视觉空间直接推理的可行性，通过精心设计的训练策略显著提升了视觉推理性能，并为未来的视觉潜在推理研究提供了方向。
...
TL;DR: 研究表明视觉干扰物与文本干扰物存在根本差异：虽然都会导致准确率下降，但视觉干扰物不会增加推理长度。


<details>
  <summary>Details</summary>
Motivation: 探究不相关信息（干扰物）如何影响视觉-语言模型在测试时的扩展效应，特别是比较视觉干扰物与文本干扰物的不同影响

Method: 引入Idis数据集，系统性地改变语义、数值和空间维度的视觉干扰物，分析推理痕迹中的属性计数

Result: 发现视觉干扰物存在反向扩展效应（准确率下降但推理长度不增加），这些趋势也适用于Waterbirds等视觉偏见基准测试

Conclusion: 提出简单的提示策略来减轻推理模型中的偏见驱动预测，强调了视觉干扰物与文本干扰物的本质差异
...
TL;DR: 提出了DiverseVAR框架，通过文本嵌入噪声注入和尺度穿梭精炼技术，在不重新训练的情况下提升VAR模型的生成多样性，同时保持图像质量


<details>
  <summary>Details</summary>
Motivation: 现有的文本条件视觉自回归模型存在多样性不足的问题，即使对于简单提示也会产生几乎相同的图像。这一问题在目前主要关注图像质量的背景下被忽视

Method: 采用两阶段方法：1）在文本嵌入中注入噪声以增强多样性，2）提出尺度穿梭精炼技术，利用多尺度自编码器提取粗尺度tokens在中间阶段恢复生成，以维持图像质量

Result: 实验表明，文本嵌入噪声注入与尺度穿梭精炼相结合，能显著提高多样性同时最小化图像质量下降，在多样性-质量权衡中达到新的帕累托前沿

Conclusion: DiverseVAR框架有效解决了VAR模型的多样性限制问题，无需重新训练或微调，在保持生成质量的同时实现了更好的多样性
...
TL;DR: 该论文提出了一种基于SAM基础模型的遥感变化描述方法，通过提取语义和运动级变化区域，结合知识图谱增强区域关注度和时序对准能力。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化描述方法在区域感知和时间对齐方面存在局限，本文探索利用SAM基础模型提取区域级表示，向标题生成框架注入感兴趣区域知识。

Method: 使用CNN/Transformer提取全局视觉特征，利用SAM基础模型勾勒语义和运动级变化区域，构建专用知识图谱提供感兴趣对象信息，通过交叉注意力融合异质信息，最后用Transformer解码器生成自然语言描述。

Result: 在多个广泛使用的基准数据集上的实验结果表明，该方法达到了最先进的性能水平。

Conclusion: 通过引入SAM基础模型提取区域级表示和注入感兴趣区域知识，有效提升了遥感变化描述的性能，源代码将在GitHub上发布。
...
TL;DR: 该论文提出了一种名为E-M3RF的等变多模态3D重组框架，通过结合几何和颜色特征来解决传统方法在几何特征不足时的重组问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的3D重组方法主要依赖几何特征，当几何特征不足以区分片段或存在对称性时效果不佳，且缺乏物理约束防止重叠组装。

Method: 使用SE(3)流匹配预测变换，通过旋转等变编码器提取几何特征，transformer提取颜色特征，然后将两者结合形成多模态表示。

Result: 在四个数据集上测试，RePAIR数据集上相比竞争方法旋转误差减少23.1%，平移误差减少13.2%，Chamfer距离降低18.4%。

Conclusion: E-M3RF通过结合多模态特征显著提升了3D重组性能，特别是在几何特征模糊或碎片较小的情况下。
...
TL;DR: 提出一种无监督框架，从工业视频流中自动提取结构化的视觉-语言-动作预训练数据，包括运动编码和动作分割模块。


<details>
  <summary>Details</summary>
Motivation: 利用工业环境中大量未标注的人类演示视频，为VLA模型提供可扩展的预训练数据来源。

Method: 先训练轻量级运动分词器编码运动动态，再使用基于"隐动作能量"度量的无监督动作分割器发现语义连贯的动作基元。

Result: 在公共基准和电机组装数据集上验证了有效分割人类工作动作的能力，VLM评估确认动作基元的语义一致性。

Conclusion: 这是首个从非结构化工业视频中端到端自动提取VLA预训练数据的系统，为制造业中的具身AI集成提供可扩展方案。
...
TL;DR: 提出了一种基于超图的时空事件流补全机制，解决了事件相机数据空间稀疏性导致的欠采样问题，支持多模态信息融合。


<details>
  <summary>Details</summary>
Motivation: 为解决事件相机异步事件流中因空间稀疏性导致的欠采样问题，提升事件识别性能。

Method: 通过超图连接不同时空位置的事件标记，利用上下文信息传递补全稀疏事件，并可灵活集成RGB标记实现多模态信息补全，通过自注意力聚合不同时间步的超图节点信息。

Result: 在单标签和多标签事件分类任务上的广泛实验充分验证了所提框架的有效性。

Conclusion: 该方法能有效解决事件稀疏性问题，实现多模态特征的有效学习和融合，性能优于现有方法。
...
TL;DR: 提出了一个270M参数的轻量级扩散模型MobileI2V，可在移动设备上实现实时高清图像到视频生成，通过线性混合架构、时间步长蒸馏和移动端注意力优化，将生成速度提升10倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在移动设备上存在计算复杂度高和生成速度慢的问题，难以在资源受限的移动设备上实现实时高清视频生成。

Method: 分析了线性注意力和softmax注意力模块在移动设备上的性能，提出线性混合架构去噪器；设计时间步长蒸馏策略，将采样步骤从20+压缩到2步；应用移动端特定的注意力优化。

Result: 实现了在移动设备上首次快速720p图像到视频生成，质量与现有模型相当，单步条件下每帧720p视频生成时间小于100毫秒。

Conclusion: MobileI2V为移动设备上的实时高清视频生成提供了可行解决方案，在保持质量的同时大幅提升了生成效率。
...
TL;DR: 本文提出了一种基于频率感知的token缩减策略，通过将token划分为高频和低频分量，选择性地保留高频token并将低频token聚合为紧凑的DC token，从而提高计算效率并缓解rank collapsing问题。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer在计算机视觉任务中表现出色，但其关于token长度的二次计算复杂度仍然是重大挑战。现有token缩减方法往往忽视了自注意力的频率特性，如rank collapsing和过平滑现象。

Method: 提出频率感知token缩减策略：将token分割为高频token和低频token，选择性保留高频token，将低频token聚合为紧凑的DC token以保留必要的低频分量。

Result: 通过广泛实验分析，该方法在减少计算开销的同时显著提高准确性，并有效缓解rank collapsing和过平滑问题。

Conclusion: 提出的频率感知token缩减方法在保持性能的同时提升计算效率，并对先前方法的隐含频率特性和局限性进行了分析。
...
TL;DR: 本文提出了一种名为Merge-and-Bound (M&B)的新型类增量学习方法，通过在参数空间中直接操作模型权重进行优化，包含任务间和任务内权重融合两种方式，结合边界更新技术来缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 针对类增量学习中的灾难性遗忘问题，提出直接优化模型权重的训练方法，探索在旧模型附近有效获取新模型的可能性。

Method: 该方法包含两种权重融合：任务间权重融合通过平均所有先前阶段模型的权重来统一历史模型；任务内权重融合在当前阶段内结合模型参数促进学习。结合边界更新技术，以最小累积更新优化目标模型。

Result: 在标准CIL基准测试上进行广泛评估，相比最先进方法展示了优越性能。

Conclusion: M&B方法无需修改架构组件或调整学习目标即可无缝集成到现有CIL方法中，有效减少灾难性遗忘，提升类增量学习效果。
...
TL;DR: CanKD是一种基于交叉注意力的非局部知识蒸馏框架，通过让学生特征图中的每个像素动态考虑教师特征图中的所有像素，实现更彻底的非局部知识转移，在目标检测和图像分割任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分捕捉像素间关系，需要一种能更彻底转移知识的蒸馏方法。

Method: 提出基于交叉注意力的非局部知识蒸馏，引入额外损失函数使学生特征图每个像素都能考虑教师特征图的所有像素。

Result: 在目标检测和图像分割任务上优于现有特征和混合蒸馏方法，表现出更优越的性能。

Conclusion: CanKD作为一种新的注意力引导蒸馏范式，在计算机视觉任务中具有广阔应用潜力，代码已开源。
...
TL;DR: 这篇论文系统研究深度伪造检测方法中不同设计选择对性能的影响，发现实现细节比核心设计更关键，旨在建立与架构无关的最佳实践标准。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测方法的有效性主要依赖于实现细节而非核心设计，这导致难以公平比较检测器并理解真正影响性能的因素。

Method: 通过系统性地分离训练、推理和增量更新等方面中不同设计选择的影响，采用实验分析来评估其对检测准确率和泛化能力的作用。

Result: 实验识别出一组能够持续提升深度伪造检测性能的设计选择，在AI-GenBench基准测试中实现了最先进的性能。

Conclusion: 研究建立了深度伪造检测系统的稳健、架构无关的最佳实践，为未来检测系统的设计和开发提供了可靠指导。
...
TL;DR: 提出一个新型多实例多标签框架，用于ANA检测，通过实例采样、概率伪标签分配和自定步调学习，在临床显微镜图像上实现自动化检测，性能显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 手动ANA检测耗时耗力且需专业训练，现有自动化方法难以处理临床环境中的多实例多标签复杂场景，需要开发更有效的端到端解决方案。

Method: 采用三组件框架：实例采样器抑制低置信度实例，概率伪标签分配器基于实例区分度自适应分配标签，自定步调学习系数根据经验标签观察调整训练过程。

Result: 在ANA数据集上，F1-Macro提升达7.0%，mAP提升达12.6%；在公开数据集上，Hamming损失和one-error分别降低18.2%和26.9%，在所有关键指标中排名前二。

Conclusion: 该框架有效解决了MIML任务在临床ANA检测中的挑战，实现了最先进的性能，为自动化疾病诊断提供了有力工具。
...
TL;DR: 提出了一种高效的遥感基础模型构建方法——专家集成框架，通过轻量级任务特定的专家模型分解训练过程，解决了传统大规模模型在计算资源和可持续性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要依赖模型规模和数据集体量的缩放策略，需要过多的计算和数据资源，且不符合可持续AI原则。旨在为遥感领域开发一个高效、可扩展的解决方案。

Method: 采用专家集成框架，将训练过程分解为轻量级的任务特定ConvNeXtV2专家模型，这些专家可以冻结和重用，支持联邦训练、剪枝和持续集成。

Result: 该框架在效率、可解释性和可扩展性方面具有显著优势，特别适合协作和资源受限的环境。

Conclusion: 这一方法为构建可扩展且高效的遥感基础模型指明了新方向，通过模块化设计解决了传统大规模模型的可持续性问题。
...
TL;DR: 该研究提出了一种基于定量指标的序列图像生成方法，用于预测阿尔茨海默病发展，通过整合年龄缩放因子生成特定年龄的MRI图像，显著提升了图像合成的准确性和长期疾病预测能力。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的及时识别对制定个性化治疗策略至关重要，但现有方法在处理不规则时间间隔序列图像时难以准确表征疾病特征，需要开发能保持疾病进展特征的新方法。

Method: 采用定量指标引导的序列图像生成方法，整合年龄缩放因子生成年龄特异性MRI图像，通过年龄缩放像素损失增强图像迭代生成效果。

Result: 消融研究表明，定量指标显著改善了MRI图像合成精度，年龄缩放像素损失提升了图像生成质量，长期疾病预后中结构相似性指数达到0.882的高值。

Conclusion: 该创新方法能有效生成保持疾病特征的序列MRI图像，为阿尔茨海默病的长期预测和个性化治疗提供了可靠的技术支持。
...
TL;DR: 提出了PRFL框架，在潜在空间进行偏好优化，大幅降低内存消耗和训练时间，同时提升与人类偏好的对齐度


<details>
  <summary>Details</summary>
Motivation: 现有的视频奖励模型依赖像素空间输入，导致计算开销大且只在去噪后期优化，缺乏对早期动态和结构一致性的监督

Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，PRFL框架在整个去噪链中实现高效的梯度反向传播，无需VAE解码

Result: PRFL显著改善了与人类偏好的对齐，同时大幅减少了内存消耗和训练时间

Conclusion: PRFL是一种高效且有效的视频生成对齐方法，证明了在潜在空间进行奖励反馈学习的优越性
...
TL;DR: UAVLight是一个无人机3D重建基准数据集，专门用于研究光照不一致问题。通过在一天中不同固定时间沿重复飞行路线采集数据，提供了一致的几何结构和自然的照明变化。


<details>
  <summary>Details</summary>
Motivation: 解决多视图3D重建中的光照不一致挑战。现有数据集要么缺乏有意义的照明多样性，要么存在几何和语义变化，无法独立研究光照鲁棒性。

Method: 创建UAVLight基准数据集：每个场景在一天中多个固定时间点沿重复的、地理参考飞行路径采集数据，确保一致的几何结构、校准和视点，同时产生自然的光照变化。

Result: 提供了一个受控但真实的基准，用于光照鲁棒性3D重建。

Conclusion: UAVLight为在真实户外环境中开发一致、准确和可重光照的重建方法提供了可靠基础。
...
TL;DR: 本论文提出了一种名为MRPD（Multimodal Robust Prompt Distillation）的师生框架，通过多模态知识蒸馏来增强3D点云模型的对抗鲁棒性。该方法在训练期间使用三个不同的教师模型（视觉深度投影模型、高性能3D模型和文本编码器）来指导轻量级提示的学习，并通过置信门控机制动态平衡各模态的贡献。


<details>
  <summary>Details</summary>
Motivation: 对抗攻击严重威胁基于学习的3D点云模型的可靠性，现有防御方法存在计算开销高和跨攻击类型泛化能力差的问题。本文旨在设计一种既高效又具有强泛化能力的防御方法。

Method: 提出MRPD师生框架，利用三个教师模型（视觉模型、3D模型、文本编码器）为点云学生模型提供鲁棒特征嵌入。采用置信门控机制动态平衡多模态知识迁移，所有蒸馏过程仅在训练阶段完成，推理时无额外计算开销。

Result: 广泛实验表明，MRPD在对抗白盒和黑盒攻击方面显著优于现有最先进的防御方法，同时在干净数据上甚至获得更好性能。

Conclusion: MRPD为构建鲁棒的3D视觉系统提供了一种新的实用范式，通过高效利用多模态知识来实现强大的防御能力和优异的性能。
...
TL;DR: 针对开源模型中音频-视频同步不准确的挑战，论文分析了联合扩散过程的3个问题并提出Harmony框架，通过跨任务协同训练和时序分离注意力等方法显著提升生成质量与同步精度。


<details>
  <summary>Details</summary>
Motivation: 开源生成式AI模型在音频-视频内容合成中面临同步困难，主要原因是联合扩散过程中的对应漂移、全局注意力效率低下和模态内偏见问题。

Method: 提出Harmony框架：跨任务协同训练减少漂移、全局-局部分离交互模块改善时间对齐、同步增强CFG在推理中优化对齐信号。

Result: 实验验证Harmony在生成保真度和细粒度音频-视频同步方面均显著优于现有方法，达到新的技术水平。

Conclusion: Harmony通过机械性强化音频-视觉同步，有效解决了联合扩散过程的挑战，为生成同步多媒体内容提供了可靠方案。
...
TL;DR: 本研究使用深度学习构建16种口腔病变的多分类模型，通过分层数据分割、数据增强和过采样技术解决数据不平衡问题，在口腔癌早期诊断中取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常常见，但由于良性与恶性病变在视觉上相似，常在晚期才被诊断。早期实施计算机辅助诊断系统可显著改善临床结果。

Method: 结合分层数据分割、高级数据增强和过采样技术来处理有限且不平衡的数据集，构建16种口腔病变的多类别分类器。

Result: 实验结果表明，该方法实现了83.33%的准确率、89.12%的精确率和77.31%的召回率，优于现有最先进方法，并有效展示了过采样和增强策略在少数类分类上的表现。

Conclusion: 该框架作为可信赖计算机辅助诊断系统的第一步，在口腔癌早期临床检测中显示出了良好的应用前景。
...
### [81] [Local Dissipativity Analysis of Nonlinear Systems](https://arxiv.org/abs/2511.20838)
*Amy K. Strong,Leila Bridgeman*

Main category: eess.SY

TL;DR: 提出了一种基于凸优化的方法，用于分析非线性仿射控制系统的局部耗散性，通过合成连续分段仿射存储函数来确定最佳的输入输出特性。


<details>
  <summary>Details</summary>
Motivation: 耗散性是分析非线性系统的重要工具，但传统方法确定系统耗散性计算复杂且依赖特定模型，需要一种通用且高效的方法。

Method: 将Hamilton-Jacobi不等式与耗散不等式的关系转化为线性矩阵不等式，通过凸优化同时搜索最优输入输出特性参数并合成连续分段仿射存储函数，还开发了混合二次和CPA存储函数的方法。

Result: 提出的方法能够保证对于严格局部耗散的系统，总能找到可行的输入输出特性参数和CPA或二次存储函数。

Conclusion: 该方法为非线性控制系统的耗散性分析提供了一种通用且可靠的计算框架，特别适用于严格局部耗散系统。
...
### [82] [Dynamic Modeling of Load Demand in Electrified Highways Based on the EV Composition](https://arxiv.org/abs/2511.20874)
*Ashutossh Gupta,Vassilis Kekatos,Dionysios Aliprantis,Steve Pekarek*

Main category: eess.SY

TL;DR: 该论文研究了电气化道路中动态无线功率传输给电网带来的谐波负荷特性，分析了不同电动汽车接收线圈长度对谐波的影响，并提出了随机模型来预测总体负荷的频谱特性。


<details>
  <summary>Details</summary>
Motivation: 由于电气化道路中发射线圈的空间布置，电动汽车接收的功率具有振荡特性，理解这种动态无线功率传输负载的动态行为对电力系统动态研究至关重要。

Method: 建立了单个电动汽车在时域和频域内的负荷模型，分析了非线性控制方案与线性方案的谐波特性差异，提出了针对电气化道路段总负荷的随机模型，并使用交通模拟器的真实流量数据进行验证。

Result: 发现非线性控制方案比线性方案产生更温和的频率谐波，电动汽车负荷的谐波幅度随接收线圈长度减小而减小，但配备更长接收线圈的电动汽车（如卡车）不一定能产生更温和的谐波。

Conclusion: 研究结果为电网运营商和电气化道路设计者提供了重要见解，表明电动汽车在电气化道路上的组成会影响其频率谱特性，谐波特性不单纯取决于接收线圈的长度。
...
### [83] [Adaptive Gradient Descent MPPT Algorithm With Complexity-Aware Benchmarking for Low-Power PV Systems](https://arxiv.org/abs/2511.20895)
*Kimia Ahmadi,Wouter A. Serdijn*

Main category: eess.SY

TL;DR: 提出一种针对低功率光伏系统的高效实时MPPT算法，融合传统P&O与自适应梯度下降机制，在动态辐照和局部遮阴条件下能快速精确跟踪最大功率点。


<details>
  <summary>Details</summary>
Motivation: 传统MPPT算法在快速变化的辐照条件和局部遮阴情况下存在跟踪速度慢、稳态振荡大的问题，尤其不适合低功率光伏系统的实时应用需求。

Method: 改进经典P&O算法，加入基于瞬时功率-电压斜率的自适应梯度下降机制，动态调整扰动步长，并可选全局MPP初始化程序应对局部遮阴。

Result: 算法在标准测试条件下达到99.94%效率，实验数据应用达99.21%，温度测试超过99.6%，局部遮阴时效率提升达7.8%，在综合指标上优于35种先进P&O算法。

Conclusion: 该算法适用于动态和资源受限条件下低功率电源管理集成电路的集成，在跟踪效率、速度和计算成本方面表现优异。
...
### [84] [Data-Driven Post-Event Analysis with Real-World Oscillation Data from Denmark](https://arxiv.org/abs/2511.20939)
*Youhong Chen,Debraj Bhattacharjee,Balarko Chaudhuri,Mark O Malley,Nan Qin,Adrian Pilkaer Expethit*

Main category: eess.SY

TL;DR: 本文展示了基于Koopman算子理论的扩展动态模态分解（EDMD）方法能有效识别电力系统中振荡的主要贡献者，使用丹麦实际0.15Hz振荡事件的PMU数据进行验证。


<details>
  <summary>Details</summary>
Motivation: 开发一种无需额外系统信息的后事件分析方法，以准确识别电力系统振荡的主要贡献源，解决传统方法（如DEF方法）在识别问题上的不足。

Method: 使用EDMD算法处理来自丹麦电网19个不同电压等级PMU记录的电压和电流相量数据，在无补充系统信息的盲测设置中进行后事件分析。

Result: EDMD准确识别了0.2Hz振荡的主要贡献位置，与Energinet后续确认的问题性IBR电站位置一致，而传统的DEF方法未能明确识别该电站。

Conclusion: 基于EDMD的后事件分析在识别主要振荡贡献者和支持针对性SSO缓解方面具有重要潜力，已在真实电网事件中得到联合验证。
...
### [85] [Adaptive Lighting Control in Visible Light Systems: An Integrated Sensing, Communication, and Illumination Framework](https://arxiv.org/abs/2511.21271)
*Xinyan Xie,Xuesong Wang,Xin Lai,Yongheng Wen,Fengrui Yang,Haoyang He,Lai Zhang,Dong Zhao*

Main category: eess.SY

TL;DR: 提出了一种自适应集成感知、通信和照明(ISCI)框架，通过分区优化解决室内可见光通信领域中性能与能耗、视觉舒适度之间的冲突。


<details>
  <summary>Details</summary>
Motivation: 现有室内可见光通信研究主要关注最大化数据速率和感知精度，导致高性能、高能耗和用户视觉舒适度之间的冲突，需要解决这一矛盾。

Method: 使用几何方法将接收平面划分为活动区和非活动区，基于NLOS感知的用户位置作为系统优化目标的动态开关，在不同区域采用不同的优化策略。

Result: 相比于非自适应系统，该方法实现53.59%的节能效果，SNR均匀性提升57.79%，满足所有照明约束，平均定位误差为0.071米。

Conclusion: 自适应ISCI框架能够有效平衡通信性能、照明质量和能效需求，在保持良好定位精度的同时显著降低能耗。
...
### [86] [Independent policy gradient-based reinforcement learning for economic and reliable energy management of multi-microgrid systems](https://arxiv.org/abs/2511.20977)
*Junkai Hu,Li Xia*

Main category: eess.SY

TL;DR: 本文研究了多微电网系统中经济可靠的能源管理问题，提出了基于均值-方差团队随机博弈的分布式优化方法，通过独立策略梯度算法实现了经济性与可靠性的平衡。


<details>
  <summary>Details</summary>
Motivation: 多微电网系统集成间歇性可再生能源，效率和可靠性对能源管理至关重要，需要分布式方案实现协同优化。

Method: 将能源管理问题建模为均值-方差团队随机博弈，提出了基于已知模型参数的分布式独立策略梯度算法，以及针对未知模型参数的深度强化学习算法。

Result: 在两个场景下的数值实验验证了所提方法的有效性，充分利用了多微电网系统的分布式计算能力。

Conclusion: 提出的方法实现了经济性能与运行可靠性之间的良好平衡，为大规模多微电网系统的能源管理提供了有效解决方案。
...
### [87] [An Exact, Finite Dimensional Representation for Full-Block, Circle Criterion Multipliers](https://arxiv.org/abs/2511.20995)
*Felix Biertümpfel,Bin Hu,Geir Dullerud,Peter Seiler*

Main category: eess.SY

TL;DR: 该论文首次为全块、圆判据乘子提供了有限维度的完整特征描述，针对离散时间线性时不变系统与非线性部分的互联结构，减少了稳定性分析的保守性。


<details>
  <summary>Details</summary>
Motivation: 现有全块乘子虽能提供最小保守性的稳定性条件，但由于包含无限多个约束而不具备计算可行性，需要找到有限维度的表征方法。

Method: 通过将非线性部分的输入输出对集合等价构造为分段线性函数的增量对集合，使用有限数量的矩阵共正性约束来描述完整乘子集合。

Result: 对输入输出维度不超过4的非线性问题，新的有限维特征具有精确且可计算的实际实施方案。

Conclusion: 提出的有限维特征使全块圆判据乘子在实际应用中变得可行，特别是在小维度非线性系统中具有重要应用价值。
...
### [88] [Stability of data-driven Koopman MPC with terminal conditions](https://arxiv.org/abs/2511.21248)
*Irene Schimperna,Lea Bold,Johannes Köhler,Karl Worthmann,Lalo Magni*

Main category: eess.SY

TL;DR: 该论文证明了在使用数据驱动替代模型作为预测模型的模型预测控制（MPC）中，即使存在逼近误差也能实现渐近稳定性，前提是满足比例误差边界条件。


<details>
  <summary>Details</summary>
Motivation: 研究在存在逼近误差情况下，基于数据驱动替代模型的MPC如何保证递归可行性和渐近稳定性，为非线性系统的控制提供理论保障。

Method: 采用带终端条件的MPC，使用基于核扩展动态模式分解（kEDMD）和Koopman算子的数据驱动替代模型作为预测模型，并证明比例误差边界条件的重要性。

Result: 证明了满足比例误差边界条件是保证递归可行性和渐近稳定的充分条件，并通过数值案例验证了所提框架的适用性。

Conclusion: 对于一类广泛的非线性系统，通过kEDMD生成的数据驱动替代模型可以满足稳定条件，为数据驱动的MPC控制提供了理论保证。
...
### [89] [Design and Measurements of mmWave FMCW Radar Based Non-Contact Multi-Patient Heart Rate and Breath Rate Monitoring System](https://arxiv.org/abs/2511.21255)
*Jewel Benny,Pranjal Mahajan,Srayan Sankar Chatterjee,Mohd Wajid,Abhishek Srivastava*

Main category: eess.SY

TL;DR: 本文提出了一种基于FMCW毫米波雷达的非接触式多患者心率和呼吸率监测系统，通过结合多种处理方法提高测量精度和通用性，实验结果显示呼吸率和心率测量准确率分别超过97%和93%。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达技术的发展使得真正非接触式心率和呼吸率监测成为可能，这在患者监测中提供了极大便利。同时，这些技术也为同时监测多名患者的心率和呼吸率提供了机会，这在高效群体监测场景中日益重要。

Method: 使用调频连续波毫米波雷达开发了非接触式多患者监测系统，并提出了一种结合多种处理方法的新方法，通过最小二乘解来提高测量精度、通用性并处理测量误差。该系统基于德州仪器的FMCW雷达开发。

Result: 实验结果显示，测量的呼吸率准确率超过97%，心率准确率超过93%，验证了系统在多受试者场景下的有效性。

Conclusion: 开发的FMCW毫米波雷达系统能有效实现非接触式多患者心率和呼吸率监测，通过提出的处理方法组合显著提高了测量准确性，具有良好的应用前景。
...
### [90] [Response-Based Frequency Stability Assessment under Multi-Scale Disturbances in High-Renewable Power Systems](https://arxiv.org/abs/2511.21269)
*Jinhui Chen,Huadong Sun,Ping Wu,Baocai Wang,Bing Zhao*

Main category: eess.SY

TL;DR: 提出基于响应的频率稳定性评估方法，统一处理高比例可再生能源电力系统中的多时间尺度扰动。通过发电机响应推断扰动功率，构建统一模型来识别扰动类型和量化强度，在不同扰动类别下推导频率响应模型并计算安全裕度。


<details>
  <summary>Details</summary>
Motivation: 在高比例可再生能源电力系统中，有功功率扰动变得更大且时间尺度更加多样化，使得在非预期事件下的频率稳定性评估变得复杂。需要统一的评估方法来处理多种时间尺度扰动。

Method: 1) 基于发电机组的实测功率响应，构建统一的扰动功率模型来在线识别扰动类型和量化扰动强度；2) 为非预期扰动推导分析性频率响应模型；3) 对阶跃扰动计算最大可承受扰动功率和安全裕度指标；4) 对斜坡型扰动使用改进的SFR模型和一次调频耗尽后的转子运动方程计算频率偏差超限时间。

Result: 该方法在CSEE-FS频率稳定性基准系统上进行了验证，证明了在高比例可再生能源电力系统中进行定量频率稳定性评估的有效性和准确性。

Conclusion: 所提出的基于响应的评估方法能够有效地统一处理电力系统中的多时间尺度扰动，为高可再生能源渗透率电力系统的频率稳定性提供准确的定量评估工具。
...
### [91] [Respiratory Motion Compensation and Haptic Feedback for X-ray-Guided Teleoperated Robotic Needle Insertion](https://arxiv.org/abs/2511.21273)
*Ana Cordon-Avila,Mostafa Selim,Momen Abayazid*

Main category: eess.SY

TL;DR: 该论文提出了一种利用呼吸运动补偿机器人系统和触觉反馈技术，以实现无辐射的腹部经皮手术精准操作。


<details>
  <summary>Details</summary>
Motivation: 呼吸运动限制了腹部经皮手术的精确度，需要通过技术手段进行补偿以提高手术准确性和减少辐射暴露。

Method: 采用机器人运动估计模型进行呼吸运动补偿，并配合基于距离的触觉反馈进行遥操作插入。

Result: 肝模型验证显示运动估计误差低于3mm，三个方向的总体插入误差分别为2.60mm、7.75mm和2.86mm。

Conclusion: 该方法能有效减少呼吸运动导致的治疗或诊断不准确，并显著降低术者辐射暴露。
...
### [92] [Sparse shepherding control of large-scale multi-agent systems via Reinforcement Learning](https://arxiv.org/abs/2511.21304)
*Luigi Catello,Italo Napolitano,Davide Salzano,Mario di Bernardo*

Main category: eess.SY

TL;DR: 提出一个强化学习框架用于大规模多智能体系统的稀疏间接控制，通过少量受控智能体来影响大量非受控智能体的集体行为。该方法结合ODE和PDE模型，通过微观控制实现宏观目标。


<details>
  <summary>Details</summary>
Motivation: 解决大规模多智能体系统中稀疏控制的挑战，即在只有少量受控智能体的情况下实现对大量非受控智能体集体行为的有效控制，避免计算昂贵的在线优化。

Method: 结合无模型强化学习和自适应交互强度补偿，耦合描述受控智能体的ODE方程和非受控群体密度的PDE方程，克服稀疏控制的局限性。

Result: 数值验证显示该方法能有效实现密度控制，系统达到目标分布同时保持对干扰和测量噪声的鲁棒性。

Conclusion: 基于学习的稀疏控制能够替代计算昂贵的在线优化，为大规模多智能体系统控制提供有效解决方案。
...
### [93] [Scalable Multisubject Vital Sign Monitoring With mmWave FMCW Radar and FPGA Prototyping](https://arxiv.org/abs/2511.21314)
*Jewel Benny,Narahari N. Moudhgalya,Mujeev Khan,Hemant Kumar Meena,Mohd Wajid,Abhishek Srivastava*

Main category: eess.SY

TL;DR: 提出了一种使用FMCW雷达进行非接触式多人生命体征监测的创新方法，解决了传统接触式测量设备的不适、校准困难和感染风险等问题。


<details>
  <summary>Details</summary>
Motivation: 解决传统生命体征监测方法的局限性，包括穿戴设备带来的不适、校准挑战以及接触测量设备的感染风险，为多种关键场景提供灵活的非接触监测解决方案。

Method: 采用调频连续波(FMCW)雷达系统进行非接触式监测，并通过FPGA硬件实现方案，提供便携式解决方案。

Result: 与先前工作相比实现了2.7倍的执行速度提升和18.4%的查找表(LUT)使用率减少，与软件版本相比提供了7400倍的加速。

Conclusion: 该系统有望重新定义生命体征监测方式，展示了在多人环境下非接触监测的潜力和可行性。
...
### [94] [Model Predictive Control and Moving Horizon Estimation using Statistically Weighted Data-Based Ensemble Models](https://arxiv.org/abs/2511.21343)
*Laura Boca de Giuli,Samuel Mallick,Alessio La Bella,Azita Dabiri,Bart De Schutter,Riccardo Scattolini*

Main category: eess.SY

TL;DR: 提出了一种基于数据驱动模型集合的模型预测控制(MPC)框架，通过Mahalanobis距离的集成规则和移动视域估计的状态观测器，在多工况下优化控制复杂系统。


<details>
  <summary>Details</summary>
Motivation: 复杂系统往往在多个工况下运行，传统单一模型难以准确描述系统动态特性，需要开发能够自适应多工况的模型预测控制方法。

Method: 1) 构建数据驱动模型集合；2) 基于Mahalanobis距离的集成权重自适应调整规则；3) 移动视域估计状态观测器；4) 模型预测控制框架。

Result: 在基准能源系统上验证了所提方法的有效性，展示了在多工况下的优越控制性能。

Conclusion: 所提出的集成模型预测控制方法能够有效处理复杂系统的多工况控制问题，Mahalanobis距离集成规则和移动视域估计观测器增强了系统的自适应能力和控制精度。
...
### [95] [Influence of converter current limiting and prioritization on protection of highly IBR-penetrated networks](https://arxiv.org/abs/2511.21385)
*Andrés E. Quintero,Vinícius A. Lacerda,Oriol Gomis-Bellmunt,Moisés J. B. B. Davi,Mario Oleskovicz*

Main category: eess.SY

TL;DR: 本文研究了电网构建型和电网追随型控制策略在逆变器主导的输电系统中对线路距离保护和差动保护的影响。研究发现距离保护的相-地回路会显著高估Zone-1区域附近的故障位置，而差动保护在外部故障时可能短暂进入跳闸区域，需要加强外部安全保障。


<details>
  <summary>Details</summary>
Motivation: 随着逆变器资源在电力系统中占比不断增加，传统保护方案在现代变流器控制和电流限制下的可靠性受到挑战，需要评估不同控制策略对保护系统性能的影响。

Method: 使用改进的IEEE 39节点系统，评估配备低压穿越逻辑、电流限制和正负序优先级控制的GFM和GFL单元。距离保护采用mho特性，线路差动保护采用alpha-plane方法。

Result: 距离保护的相-地回路在Zone-1区域附近会严重高估故障位置；差动保护在GFL控制下的ABG故障中，外部故障时健康相的运行点会短暂进入跳闸区域。

Conclusion: 现代变流器控制、电流限制和序电流优先化策略会损害传统保护方案的可靠性和安全性，需要开发更适应的保护方案。
...
### [96] [Decentralized Shepherding of Non-Cohesive Swarms Through Cluttered Environments via Deep Reinforcement Learning](https://arxiv.org/abs/2511.21405)
*Cristiana Punzo,Italo Napolitano,Cinzia Tomaselli,Mario di Bernardo*

Main category: eess.SY

TL;DR: 提出了一种分层控制架构，可在有障碍物的复杂环境中有效引导散乱目标群到达目标区域


<details>
  <summary>Details</summary>
Motivation: 研究在有限牧羊者数量、有静态障碍的杂乱环境中引导不合作扩散目标群的问题，旨在开发可扩展的无模型牧羊方法

Method: 采用分层控制架构：高层目标分配规则将牧羊者与选定目标配对，基于学习的低层驾驶模块使用PPO算法训练，训练后无需重新训练即可扩展至多智能体多障碍场景

Result: 数值模拟显示生成了平滑、无碰撞的轨迹，目标持续收敛到目标区域

Conclusion: 强化学习在复杂环境中具有实现可扩展、无模型牧羊的潜力
...
### [97] [Robust Rule-Based Sizing and Control of Batteries for Peak Shaving Applications](https://arxiv.org/abs/2511.21619)
*Lorenzo Nespoli,Vasco Medici*

Main category: eess.SY

TL;DR: 本文提出了一种随机调优的基于规则的控制器方法，相比确定性模型预测控制，该方法更快速且能提供更真实的平准化度电成本估算，在削峰应用中表现更佳。


<details>
  <summary>Details</summary>
Motivation: 随着电池成本降低，需要既快速又能在实际部署中实现预期性能的容量规划和控制方法。

Method: 使用随机调优的基于规则控制器，在真实电表数据的削峰应用场景中进行年度测试。

Result: 随机RBCs相比确定性MPC能提供更真实的平准化度电成本估算，并具有更好的运行性能。

Conclusion: 随机调优的基于规则控制器是实现快速部署和性能保证的有效解决方案。
...
### [98] [Bang-Bang Evasion: Its Stochastic Optimality and a Terminal-Set-Based Implementation](https://arxiv.org/abs/2511.21633)
*Liraz Mudrik,Yaakov Oshman*

Main category: eess.SY

TL;DR: 该论文研究了平面终端游戏中的最优规避问题，目标在有界侧向加速度下躲避导弹拦截，采用随机框架处理不完美信息和有界控制问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设完美信息或在随机设置中使用启发式机动模型，需要解决实际随机规避场景中的最优控制问题。

Method: 首先证明最优规避策略存在且包括bang-bang策略，然后提出基于闭环终端集的规避策略，在比例导航拦截器对抗中验证有效性。

Result: 蒙特卡洛仿真表明，TSE策略优于基于随机电报、Singer和摆动模型的传统随机规避策略。

Conclusion: 将确定性bang-bang规避最优性扩展到随机领域，提出的TSE策略在随机规避场景中表现优异。
...
### [99] [Model-free practical PI-Lead control design by ultimate sensitivity principle](https://arxiv.org/abs/2511.21641)
*Michael Ruderman*

Main category: eess.SY

TL;DR: 该论文提出了一种无需建模的PI-Lead控制器实用设计方法，基于极限灵敏度原理和环路整形特性，通过实验输出来确定控制器参数，确保足够的相位裕度。


<details>
  <summary>Details</summary>
Motivation: 在实际控制系统设计中，工程师常常在没有动态过程模型的情况下工作，只有对过程的一般假设（如一类稳定行为）。特别是在运动控制系统中，需要一个实用且简单的控制器设计方法。

Method: 开发了一种基于极限灵敏度原理的三步程序，通过仅实验观察输出值来确定积分时间常数、控制增益和超前元件，以确保足够的相位裕度。该方法继承自Ziegler-Nichols PID整定的经验原理，并利用环路整形的一般特性。

Result: 在带有噪声干扰的电动机械执行器系统上进行实验评估，验证了所提出方法的有效性。

Conclusion: 提出的PI-Lead控制器设计方法是一种无需建模的实用解决方案，能够保证系统的鲁棒性能，特别适用于运动控制系统。
...
TL;DR: 本文研究了如何利用大语言模型自动重构层次结构以优化双曲嵌入质量，实验证明LLM重构的层次在多指标下能产生更高质量的双曲嵌入。


<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构密切相关，而最佳嵌入需要高分支因子和单继承特性。为了帮助知识工程师重构层次知识，研究LLM是否具备自动重构层次以满足这些标准的能力。

Method: 提出基于提示的方法，利用LLM在双曲嵌入已知需求指导下转换现有层次结构。在16个不同层次上进行实验验证。

Result: LLM重构的层次在多个标准嵌入质量指标下始终产生更高质量的双曲嵌入。

Conclusion: LLM引导的层次重构不仅能提升双曲嵌入质量，还能提供可解释的重组说明，为知识工程师提供依据。
...
TL;DR: 提出了AssurAI，一个用于评估生成式AI安全性的韩语多模态数据集，包含11,480个实例，涵盖文本、图像、视频和音频四种模态，专门针对韩语社会文化背景的安全风险。


<details>
  <summary>Details</summary>
Motivation: 当前安全数据集主要是英语为中心，无法捕捉非英语社会文化背景（如韩语）的特定风险，且通常仅限于文本模态。

Method: 定义了35个AI风险因素的分类体系，采用专家领导种源和众包扩展的两阶段构建方法，进行三重独立标注和迭代的专家红队测试循环的质量控制过程。

Result: 构建了AssurAI大规模韩语多模态数据集，并通过试点研究验证了其在评估最新LLM安全性方面的有效性。

Conclusion: AssurAI数据集能够促进为韩语社区开发更安全可靠的生成式AI系统，并已向公众发布。
...
TL;DR: 提出A²Flow框架，通过自适应性抽象运算符实现完全自动化的智能体工作流生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法仍依赖手动预定义运算符，限制了泛化性和可扩展性。

Method: 三阶段运算符提取：基于案例的初生、聚类预抽象、深度抽取抽象执行算子，并引入运算符记忆机制。

Result: 在通用和具身基准测试上，平均性能提升2.4%和19.3%，资源使用减少37%。

Conclusion: A²Flow实现了无需手动预定义的全自动化工作流构建，显著提升了性能和效率。
...
TL;DR: 本文提出了一个名为Reasoning With a Star的日球物理数据集和基准测试方法。该数据集基于NASA和UCAR的Living With a Star暑期学校问题集构建，包含问题上下文、推理步骤、预期答案类型等结构。研究发现，基于系统工程原则的工作流程分解在需要演绎推理的问题上表现优于直接提示。


<details>
  <summary>Details</summary>
Motivation: 解决日球物理中大型语言模型科学推理面临的挑战，包括物理假设的整合、单位一致性和清晰的科学格式要求。

Method: 构建结构化的问题-答案数据集，使用程序化评分器检查预测结果，包括基于单位的数值容差、符号等价性和模式验证，并比较单次提示与四种多智能体模式的性能。

Result: 通过系统工程原则分解工作流程的方法在需要演绎推理的问题上优于直接提示，而不适用于纯粹的归纳回忆问题。

Conclusion: 需要专门的推理数据集和工程化的多智能体方法来增强大型语言模型在日球物理学中的科学推理能力，尤其是在演绎推理任务上。
...
TL;DR: 数字孪生技术从航天仿真扩展到医疗领域，可创建动态、数据驱动的虚拟患者模型，用于辅助诊断、治疗规划和药物开发，但仍面临互操作性、数据隐私和模型精度等挑战。


<details>
  <summary>Details</summary>
Motivation: 将数字孪生技术从工业领域引入医疗，旨在实现从被动治疗向预测性、预防性和个性化医疗的转型，提高诊断和治疗效果。

Method: 整合医学影像、生物传感器和计算模型，构建患者特异性模拟系统，支持心律失常预测、肿瘤追踪和放疗优化以及药物发现等应用。

Result: 代表性应用包括心脏数字孪生预测心律失常治疗结果、肿瘤数字孪生跟踪进展并优化放疗、药理学数字孪生加速药物发现。

Conclusion: 尽管面临互操作性、数据隐私和模型精度等挑战，但可解释AI、联邦学习和统一监管框架等解决方案有望推动数字孪生在医疗中的广泛应用，实现从反应性治疗向预测性个性化医疗的转变。
...
TL;DR: 该研究分析了多模态思维链在科学问答之外的泛化能力，发现在A-OKVQA、OKVQA和ChartQA数据集中，视觉特征可以减少推理过程中的幻觉，但不同问题类型的思维链推理效果差异显著。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态思维链在科学问答基准上取得了最先进结果，但其在不同领域的泛化能力尚未充分探索。本研究旨在评估多模态思维链推理在需要广泛常识和世界知识的多样化数据集上的有效性。

Method: 采用Zhang等人提出的两阶段框架，分离推理生成和答案推断，通过门控融合机制将视觉特征与基于T5的语言模型集成，并进行系统性消融研究分析视觉特征、推理质量和架构选择的影响。

Result: 视觉集成显著减少了推理生成中的幻觉，但思维链推理的有效性在不同问题类型间差异很大，常识推理尤其具有挑战性。

Conclusion: 本研究为实施多模态推理系统的研究人员提供了实用洞察，并确定了跨领域泛化未来改进的关键方向。
...
TL;DR: 提出了RILKE方法，通过在表示空间进行干预来实现LLMs的终身知识控制，避免昂贵的重新训练，同时保持模型通用性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生不正确或过时的内容，需要在不进行昂贵重新训练的情况下高效准确地更新知识，特别是在终身学习环境中管理多个无组织知识编辑

Method: RILKE方法将知识控制视为模型表示空间内的干预，学习抗改写和编辑局部化的模块，将每个更新限制在低维子空间以减少交叉干扰，在推理时使用查询自适应路由器选择适当的模块来指导生成

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE能够扩展到大规模数据集，表现出高编辑成功率、强改写泛化能力，并在保持通用实用性的同时拥有适度的内存开销

Conclusion: RILKE是为LLMs提供终身知识控制的有效和可扩展的解决方案
...
TL;DR: ENACT是一个评估具身认知的基准测试，将具身认知评估转换成视觉问答形式，通过序列重排序任务考察前沿视觉语言模型是否具备具身认知能力。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索现代视觉语言模型是否具备具身认知的迹象，即智能是否源于感知运动交互而非被动观察。

Method: ENACT包含两个互补的序列重排序任务：前向世界建模（给定动作重排观察序列）和逆向世界建模（给定观察重排动作序列），基于家庭规模活动合成8,972个问答对进行评估。

Result: 实验发现前沿视觉语言模型与人类存在性能差距，尤其是在交互时间延长时；模型在逆向任务表现优于前向任务，并表现出人类中心偏差。

Conclusion: 该研究揭示了视觉语言模型在具身认知方面的局限性，表明纯粹的旁观式训练不足以培养真正的具身智能，需要更直接的交互经验。
...
TL;DR: Ivy是一个AI辅导系统，结合符号化TMK模型和LLM生成层，提供结构化、多步骤的解释，显著提升了AI生成解释的教学价值。


<details>
  <summary>Details</summary>
Motivation: 在程序性技能学习中，需要传达步骤背后的因果、目标和组合逻辑，而LLM通常产生流畅但浅薄的响应，缺乏这种结构。

Method: 结合符号化Task-Method-Knowledge模型与生成解释层，使用LLM在TMK结构约束下构建解释，编码因果转换、目标层次和问题分解。

Result: 相比GPT和检索增强GPT基线，在三个推理维度上，符号约束一致提高了对'如何'和'为什么'问题解释的结构质量。

Conclusion: 研究展示了一种可扩展的AI教育方法，在智能辅导系统中增强了AI生成解释的教学价值。
...
TL;DR: 提出了ICPO方法，通过结合内在置信度驱动的群体相对偏好优化来解决RLVR中的奖励粒度粗、噪声大和探索效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法存在奖励粒度粗、奖励噪声和探索效率低等问题，导致训练不稳定和熵崩溃，限制了LLM推理能力的提升。

Method: 提出ICPO方法，通过计算不同响应的相对生成概率得到偏好优势分数，将其与可验证奖励结合来指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO能稳定提升推理能力。

Conclusion: ICPO方法有效缓解了奖励粒度粗和噪声问题，抑制了过度自信错误，提升了被低估高质量响应的相对优势，防止模型对特定策略过拟合，促进了更彻底的探索。
...
TL;DR: 本文对Halpern和Pearl的因果定义进行了抽象化，使其能应用于任何定义了反事实的模型，可以处理更复杂的逻辑公式并扩展到解释的定义。


<details>
  <summary>Details</summary>
Motivation: 现有Halpern-Pearl因果定义局限于因果模型，无法处理包含析取、否定、信念和嵌套反事实的复杂公式。

Method: 从Halpern-Pearl因果定义中提取关键特征，构建抽象定义框架，使其适用于任何定义了反事实的模型体系。

Result: 成功构建了可应用于更广泛模型的因果定义，包括支持回溯的模型，并能处理更复杂的逻辑表达式。

Conclusion: 抽象化方法扩展了因果定义的应用范围，加深了对因果定义特征的理解，并能推广到解释的定义。
...
TL;DR: 本文提出新的启发式方法用于伪布尔求解中的混合单元传播策略，显著优于当前方法


<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是观察字面量方案与计数方法的混合模式，但需要更有效的决策启发式方法来进一步提升性能

Method: 提出新的启发式方法来改进混合决策过程，在RoundingSAT求解器中实现

Result: 新启发式方法能够显著超越当前方法的表现

Conclusion: 新的混合决策启发式为伪布尔求解中的单元传播策略提供了性能改进
...
### [112] [EWE: An Agentic Framework for Extreme Weather Analysis](https://arxiv.org/abs/2511.21444)
*Zhe Jiang,Jiong Wang,Xiaoyu Yue,Zijie Guo,Wenlong Zhang,Fenghua Ling,Wanli Ouyang,Lei Bai*

Main category: cs.AI

TL;DR: 本研究提出了首个专门用于极端天气事件自动化诊断推理的智能体框架EWE，旨在突破传统专家驱动分析方法的瓶颈，促进气象领域的自动化科学发现。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益严重的威胁，而现有的专家驱动、劳动密集型诊断模式造成了关键的分析瓶颈，阻碍了科学进展。需要开发自动化诊断推理系统来应对这一挑战。

Method: 构建EWE智能体框架，模拟专家工作流程，通过知识引导规划、闭环推理和定制化气象工具包，能够从原始气象数据自主生成和解读多模态可视化结果，进行全面的诊断分析。

Result: 开发了该新兴领域的首个基准测试，包含103个高影响事件的精选数据集和新的步进式评估指标，EWE为自动生成全面诊断分析提供了可行解决方案。

Conclusion: EWE标志着向自动化科学发现迈出的重要一步，具有民主化专业知识和智力资源的潜力，特别是对于易受极端天气影响的发展中国家。
...
### [113] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
*Peiran Xu,Sudong Wang,Yao Zhu,Jianing Li,Yunjian Zhang*

Main category: cs.AI

TL;DR: 该论文提出了一个分层空间认知框架SpatialBench，将空间智能分解为五个层次，并构建了包含15个任务的大规模基准测试，用于评估多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常将空间认知过度简化为单一维度指标，无法捕捉空间能力的层次结构和相互依赖性，因此需要建立一个能够全面评估模型空间推理能力的系统性框架。

Method: 提出分层空间认知框架，将空间智能分解为从基础观察到高级规划的五个复杂性递增层次；构建覆盖15个任务的大规模基准SpatialBench；引入面向高层能力的统一评估指标。

Result: 大规模实验显示模型在不同认知层次上表现出明显的性能分层：模型在感知层面表现良好，但在符号推理、因果推断和规划等方面仍存在限制。人类测试显示人类进行选择性目标导向抽象，而模型倾向于过度关注表面细节。

Conclusion: 该研究为MLLMs建立了首个系统性的分层空间认知测量框架，为未来空间智能系统奠定了基础。
...
### [114] [Pessimistic Verification for Open Ended Math Questions](https://arxiv.org/abs/2511.21522)
*Yanxing Huang,Zihan Tang,Zejin Lin,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: 提出了悲观验证方法，通过构建多个并行验证来提高开放数学问题的验证性能，显著提升数学验证基准表现且不消耗大量计算资源。


<details>
  <summary>Details</summary>
Motivation: 验证性能的关键局限在于错误检测能力，为了解决这个问题并提高数学问题验证的可靠性。

Method: 设计了悲观验证的多种变体，为同一个证明构建多个并行验证，只要有一个报告错误就认定证明不正确。

Result: 该方法显著提升多个数学验证基准的性能，token效率甚至超越扩展长链思维，且多数误报源于原始数据集的标注错误。

Conclusion: 悲观验证能有效提高语言模型输出的可靠性和性能，在长期数学任务中发挥关键作用，有助于增强语言模型的数学能力。
...
### [115] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 语言模型在专业领域缺乏AI身份自披露的可靠性，导致用户难以信任其能力边界，可能产生"反向盖尔曼失忆症"效应 - 某些领域的透明度使用户过度信任到其他披露失败的领域。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索语言模型在专业高风险领域（如金融、神经外科）中AI身份自披露的可靠性和一致性，因为如果模型无法可靠地披露其AI身份，用户可能无法识别其能力边界，从而面临潜在风险。

Method: 采用"共同花园"实验设计，对16个开放权重模型（4B至671B参数）进行了19,200次试验审计，通过分配专业角色身份，测量模型在不同高风险领域的AI身份披露行为。

Result: 模型在不同专业领域表现出显著不一致的自披露行为：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%；模型个体而非参数数量更能预测披露行为；推理优化在某些模型中主动抑制了透明度，推理变体的披露率比基础模型低达48.4%。

Conclusion: 模型透明度反映的是训练因素而非模型规模，组织不能假设安全属性会迁移到部署环境，需要刻意设计行为并进行实证验证。
...
### [116] [From Prediction to Foresight: The Role of AI in Designing Responsible Futures](https://arxiv.org/abs/2511.21570)
*Maria Perez-Ortiz*

Main category: cs.AI

TL;DR: 本文提出'负责任计算前瞻'概念，探讨人工智能如何辅助负责任的前瞻性决策，强调AI应作为支持工具而非替代人类判断。


<details>
  <summary>Details</summary>
Motivation: 在技术快速发展和全球性复杂挑战的时代，政策制定者需要负责任的前瞻框架来应对未来不确定性和塑造未来。

Method: 建立负责任计算前瞻的基本原则体系，提出一套AI驱动的前瞻工具，结合模拟和情景分析，增强政策制定者处理不确定性和风险评估的能力。

Result: AI与计算模型能够提升政策制定者处理不确定性、评估风险和制定可持续策略的能力，但需要结合对社会、环境、经济和政治系统的深刻理解。

Conclusion: AI应作为负责任、以人为本的前瞻中的支持工具，与政策制定者判断互补，实现事前塑造有韧性和道德健全未来的目标。
...
TL;DR: 提出一种基于分数阶导数的变分方法进行拉曼光谱去噪，在频域中实施该方法，并通过香农熵优化参数，平衡噪声抑制与光谱特征保持。


<details>
  <summary>Details</summary>
Motivation: 拉曼光谱分析中荧光信号干扰和噪声常常模糊关键的光谱特征，影响分析准确性，需要一种有效方法来抑制噪声同时保留化学特征。

Method: 采用类似图像去噪的变分方法，最小化包含分数阶导数的泛函，在频域中通过傅里叶变换实现，使用香农熵优化正则化参数和导数阶数。

Result: 该方法在模拟拉曼数据和图像处理中验证有效，分数阶和正则化参数的组合能有效去除噪声并保留光谱和图像的关键特征。

Conclusion: 所提出的策略组合产生了一个高效、鲁棒且易于实现的滤波器，在拉曼光谱去噪中表现出良好性能。
...
### [118] [Knowledge Distillation for Continual Learning of Biomedical Neural Fields](https://arxiv.org/abs/2511.21409)
*Wouter Visser,Jelmer M. Wolterink*

Main category: eess.IV

TL;DR: 神经场在生物医学成像中用于轻量级连续信号表示，但面临灾难性遗忘问题。本文研究不同神经场方法的遗忘程度，并提出知识蒸馏策略来缓解该问题，在心脏电影MRI数据上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 神经场作为连续可微信号表示在医学成像中应用增多，但无法像离散表示那样容易扩展。由于灾难性遗忘，当新数据可用时，先前信号会退化。

Method: 考虑数据增量可用且只有最新数据用于神经场拟合的场景。使用知识蒸馏策略，在扩大时空域或增加信号维度时缓解灾难性遗忘。

Result: 实验在心脏电影MRI数据上显示，灾难性遗忘程度很大程度上取决于所用神经场模型，知识蒸馏能够有效缓解该问题。

Conclusion: 知识蒸馏可以使神经场实现持续学习，显著改善神经场在增量学习场景中的性能。
...
### [119] [Semantic-Enhanced Feature Matching with Learnable Geometric Verification for Cross-Modal Neuron Registration](https://arxiv.org/abs/2511.21452)
*Wenwei Li,Lingyi Cai,Hui Gong,Qingming Luo,Anan Li*

Main category: eess.IV

TL;DR: 提出了一种深度学习框架来解决体内双光子和体外荧光显微光学层析图像的精确配准问题，通过语义增强混合特征描述符和可学习的几何一致性模块来克服模态差异、数据稀缺和组织变形等挑战。


<details>
  <summary>Details</summary>
Motivation: 解决神经科学研究中体内双光子和体外荧光显微图像配准面临的模态差异大、标注数据少和组织变形严重等核心难题，为结构功能分析提供精确配准方法。

Method: 采用语义增强混合特征描述符融合局部特征的几何精度与DINOV3视觉基础模型的上下文鲁棒性；用可学习的几何一致性置信模块替代传统RANSAC；采用数据有效的两阶段训练策略，先在合成变形数据上预训练，再在有限真实数据上微调。

Result: 框架为具有挑战性的生物医学成像场景提供了鲁棒且精确的配准解决方案。

Conclusion: 该深度学习方法成功解决了跨模态图像配准的关键技术瓶颈，为大规模相关性研究提供了有效支持。
...
TL;DR: ProNECL提出了一种无需历史数据、基于原型的连续学习方法，用于解决跨个体EEG信号解码中的知识遗忘问题，在保护隐私和内存约束下实现了良好的性能


<details>
  <summary>Details</summary>
Motivation: 由于EEG信号存在显著的个体间差异，在连续EEG解码任务中，随着新用户的引入，从历史用户学到的知识容易被覆盖。现有方法依赖于存储历史数据作为回放缓冲区，但这可能涉及隐私问题或内存限制

Method: 提出Prototype-guided Non-Exemplar Continual Learning (ProNECL)框架，通过构建类级原型来总结每个用户的判别性表征，并通过跨用户特征对齐和知识蒸馏，逐步将新特征空间与全局原型记忆对齐

Result: 在BCI Competition IV 2a和2b数据集上的验证表明，该框架有效平衡了知识保留和适应性，在跨用户连续EEG解码任务中获得了优越性能

Conclusion: ProNECL框架能够在无需访问历史EEG样本的情况下保留先验知识，为处理EEG数据的个体间变异性提供了一种有效而实用的解决方案
...
### [121] [Subjective Depth and Timescale Transformers: Learning Where and When to Compute](https://arxiv.org/abs/2511.21408)
*Frederico Wieser,Martin Benfeghoul,Haitham Bou Ammar,Jun Wang,Zafeirios Fountas*

Main category: cs.LG

TL;DR: 通过贝叶斯惊奇信号动态路由计算，提出了两种Transformer变体：SDT（主观深度）和STT（主观时间尺度），分别从空间和时间维度优化计算分配，显著减少计算和KV缓存需求。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer架构中计算资源的刚性统一分配限制了其效率和可扩展性，尤其在大模型和长序列场景下。需要动态学习在何处及何时进行计算。

Method: SDT：在解码器堆栈中交替使用决策层和动态层，基于贝叶斯惊奇信号进行Top-K路由。STT：在时间域扩展条件计算，通过预测残差更新来动态执行或跳过Transformer块。

Result: 实现了预测的新奇性到预测驱动门控的转变，在减少计算容量的同时，每计算跳过层可减少75%的自注意计算和50%的KV缓存需求。

Conclusion: 提出的架构为高效模型提供了灵活框架，初步探索了条件计算的计算-精度权衡，为更高效模型铺平了道路。
...
### [122] [Pretraining Transformer-Based Models on Diffusion-Generated Synthetic Graphs for Alzheimer's Disease Prediction](https://arxiv.org/abs/2511.20704)
*Abolfazl Moslemi,Hossein Peyvandi*

Main category: cs.LG

TL;DR: 提出基于Transformer的阿尔茨海默病诊断框架，结合扩散合成数据生成、图表示学习和迁移学习，解决小样本和类别不平衡问题，显著提升诊断性能。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期准确检测对及时干预至关重要，但现有机器学习模型面临标记数据有限、多站点异质性和类别不平衡等挑战。

Method: 使用类别条件DDPM在真实NACC数据集上生成平衡诊断类别的合成队列，用模态特定图Transformer编码器在合成数据上预学习，然后在原始NACC数据上训练分类器。

Result: 在NACC数据集上进行主题交叉验证，优于包括多模态图模型MaGNet在内的基线方法，获得更高的AUC、准确率、敏感性和特异性。

Conclusion: 基于扩散的合成预训练与图Transformer相结合，可以在小样本、不平衡的临床预测设置中提高泛化能力。
...
TL;DR: 这项研究提出主动切片发现方法来识别大语言模型中的系统性错误子集，通过主动学习和有限人工标注来发现模型在特定数据子集上的错误模式。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通常在特定数据子集上出现系统性错误（错误切片），如特定人口统计群体的毒性评论检测。手动识别这些切片成本高昂，需要更有效的发现方法。

Method: 提出主动切片发现框架，使用特征表示和主动学习算法组合，利用有限的人工标注来验证样本是否共享相同的模型错误模式。在毒性分类任务中评估不同特征表示和主动学习算法的效果。

Result: 实验发现基于不确定性的主动学习算法在多个切片上最为有效，仅使用2-10%的可用切片成员信息就能达到有竞争力的准确率，显著优于基线方法。

Conclusion: 主动切片发现是识别模型错误模式的有效方法，基于不确定性的主动学习能够显著减少人工标注需求，提高错误切片发现的效率。
...
TL;DR: 论文提出了两种稳定多轮对话中PPO训练的技术：轮次级别的重要性采样和裁剪偏差校正，通过ST-PPO和S-PPO变体有效解决了PPO在LLM训练中的性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 传统的令牌级别PPO在多轮对话任务中表现不稳定且容易崩溃，主要问题是与多轮环境结构不匹配的重要性采样和不准确的置信度估计。

Method: 提出两种互补的稳定技术：1）轮次级别重要性采样，使优化与多轮推理的自然结构对齐；2）裁剪偏差校正，通过削弱不可靠的离策略样本来归一化梯度。

Result: 在多轮搜索任务实验中，ST-PPO和S-PPO能防止性能崩溃，保持较低的裁剪比率，并在多个问答基准测试中达到比标准PPO更高的任务性能。

Conclusion: 将轮次级别重要性采样与裁剪偏差校正相结合，为多轮LLM智能体训练提供了实用且可扩展的稳定解决方案。
...
TL;DR: 论文系统分析SGD、Mini-batch SGD、Momentum、Adam和Lion五种深度学习优化算法的核心优势、局限性和关键实践建议，为算法选择和参数调优提供标准化参考。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习优化算法在实际配置中的需求，为学术研究和工程实践中合理选择算法、参数调优和性能提升提供指导，应对不同规模模型和训练场景中的优化挑战。

Method: 聚焦五种主流优化算法进行系统性分析，深入研究各算法的核心特点、适用场景和调优策略。

Result: 提供了每种算法的优势、局限性分析以及关键实践建议，形成了优化算法选择和配置的标准化参考框架。

Conclusion: 建立了深度学习优化算法的系统分析框架，为不同场景下的算法选择和参数配置提供了实用指导，有助于提升模型训练效率和性能。
...
### [126] [Spatio-Temporal Trajectory Foundation Model - Recent Advances and Future Directions](https://arxiv.org/abs/2511.20729)
*Sean Bin Yang,Ying Sun,Yunyao Cheng,Yan Lin,Kristian Torp,Jilin Hu*

Main category: cs.LG

TL;DR: 本文全面综述了轨迹基础模型(TFMs)这一时空基础模型的重要子类，回顾了现有方法分类和优缺点分析，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现有的轨迹基础模型研究缺乏系统性的调查，尽管它是时空基础模型中至关重要的子类，需要填补这一研究空白。

Method: 本教程采用综述方法，提供TFMs近期进展的全面概述，包括现有方法的分类和对优缺点的重要分析。

Result: 识别出TFMs在轨迹数据分析中的关键作用，但指出缺乏系统性调查的问题。

Conclusion: 轨迹基础模型是推进空间时空通用智能的重要方向，需要开发更强健、负责任和可迁移的TFMs来解决开放挑战。
...
### [127] [CHiQPM: Calibrated Hierarchical Interpretable Image Classification](https://arxiv.org/abs/2511.20779)
*Thomas Norrenbrock,Timo Kaiser,Sovan Biswas,Neslihan Kose,Ramesh Manuvinakurike,Bodo Rosenhahn*

Main category: cs.LG

TL;DR: 论文提出了CHiQPM模型，在保持99%非可解释模型准确率的同时提供全面全局局部可解释性，支持分层解释和保形预测


<details>
  <summary>Details</summary>
Motivation: 为了在安全关键领域的可信AI，需要既提供全局解释又包含详细局部解释，以有效支持人类专家决策

Method: 提出校准分层QPM（CHiQPM），通过对比解释多数类别实现全局可解释性，提供与人思维方式更相似的分层解释，并内置可解释的保形预测方法

Result: CHiQPM作为点预测器达到最先进精度，保持非可解释模型99%准确率，校准集预测效率与其它CP方法竞争性相当

Conclusion: CHiQPM在几乎不牺牲准确性的情况下实现了可解释性，为人类-AI互补性铺平道路
...
### [128] [Physics Steering: Causal Control of Cross-Domain Concepts in a Physics Foundation Model](https://arxiv.org/abs/2511.20798)
*Rio Alexa Fear,Payel Mukhopadhyay,Michael McCabe,Alberto Bietti,Miles Cranmer*

Main category: cs.LG

TL;DR: 该工作研究了一个大型物理基础模型的内部表示，发现其内部存在与具体物理特征对应的概念方向，通过注入这些概念方向可以因果性地控制模型的物理行为预测。


<details>
  <summary>Details</summary>
Motivation: 探究基础模型内部表示的普遍性，看是否只是语言、图像等结构化数据特有的现象，还是基础模型的通用性质。

Method: 从物理基础模型的前向传播中提取激活向量，计算不同物理机制之间的delta表示作为概念方向，并在推理时注入这些方向来操控模型行为。

Result: 成功发现模型内部编码了特定的物理特征概念方向，通过注入这些方向可以因果性地控制物理行为（如在模拟中引入或移除特定物理特征）。

Conclusion: 科学基础模型学习到了物理原理的泛化表示，不仅仅是依赖模拟中的表面相关性，这为理解和控制科学基础模型开辟了新途径。
...
### [129] [Conformal Safety Monitoring for Flight Testing: A Case Study in Data-Driven Safety Learning](https://arxiv.org/abs/2511.20811)
*Aaron O. Feldman,D. Isaiah Harp,Joseph Duncan,Mac Schwager*

Main category: cs.LG

TL;DR: 该论文开发了一种数据驱动的运行时安全监控方法，用于飞机飞行测试中，通过离线随机轨迹模拟来预测短期安全风险，为飞行员提供明确的安全标准以在安全违规发生前中止操作。


<details>
  <summary>Details</summary>
Motivation: 解决飞机飞行测试中因参数不确定性导致的安全风险问题，需要为飞行员提供明确、先发制人的中止标准，以防止意外安全违规。飞行测试因其固有的安全风险、不确定性和人机交互特性而成为数据驱动安全监控的合适应用场景。

Method: 采用三个通用组件：基于近期观测预测未来状态的模型、对预测状态进行安全分类的最近邻模型，以及通过符合性预测进行分类器校准。

Result: 在具有不确定参数的飞行动力学模型上评估，证明该方法能可靠识别不安全场景，符合理论保证，并在风险预分类方面优于基线方法。

Conclusion: 开发的数据驱动方法能够为飞行测试提供可靠的安全风险监测和预判能力，确保在安全违规发生前及时中止危险操作。
...
### [130] [Autoregressive Surrogate Modeling of the Solar Wind with Spherical Fourier Neural Operator](https://arxiv.org/abs/2511.20830)
*Reza Mansouri,Dustin Kempton,Pete Riley,Rafal Angryk*

Main category: cs.LG

TL;DR: 这篇论文引入了首个使用球形傅里叶神经算子的自回归机器学习替代模型，用于预测太阳风径向速度，在保持高精度的同时大幅降低计算成本。
...
Method: 采用球形傅里叶神经算子，通过预测有限径向范围并迭代向外传播解决方案的自回归方法，相比单步方法在远距离区域具有更高精度。

Result: 与数值HUX替代模型相比，SFNO表现出相当或更优的性能，同时提供灵活、可训练和数据驱动的替代方案。

Conclusion: 该研究为高保真太阳风建模建立了新方法学，为空间天气预报提供了更高效的计算工具。
...
### [131] [Primal: A Unified Deterministic Framework for Quasi-Orthogonal Hashing and Manifold Learning](https://arxiv.org/abs/2511.20839)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: Primal是一个确定性特征映射框架，利用素数平方根的数论独立性构建可调谐、稳定的向量表示，优于传统的随机投影方法。


<details>
  <summary>Details</summary>
Motivation: 提出基于素数平方根的数论独立性构建特征投影的确定性方法，以取代标准随机投影（如随机傅里叶特征），通过无理频率调制确保相位轨迹的无限不重复性。

Method: 设计了两种算法变体：StaticPrime（生成接近Welch边界准正交性的时间位置编码）和DynamicPrime（通过单一缩放参数统一处理低频率下的等距核映射和高频率下的最大熵单向哈希）。

Result: 经验评估显示，该方法在正交性保留和分布紧密度方面优于归一化高斯基线，成为随机矩阵投影计算高效且数学严谨的替代方案。

Conclusion: Primal框架为特征映射提供了确定性、可调谐且数学严谨的解决方案，有效统一了低频率下的几何线性化与高频率下的混沌哈希特性。
...
### [132] [Selecting Belief-State Approximations in Simulators with Latent States](https://arxiv.org/abs/2511.20870)
*Nan Jiang*

Main category: cs.LG

TL;DR: 论文研究了仿真器中状态重置的问题，特别是在存在隐变量的情况下如何通过采样近似信仰状态。提出了两种不同的信仰状态选择方法：基于隐状态和基于观测的方法，并分析了它们在不同滚动方法下的性能差异。


<details>
  <summary>Details</summary>
Motivation: 状态重置是仿真器支持基于样本规划和真实数据校准的基本能力，但在复杂仿真器中实现状态重置具有挑战性，需要从给定可观测历史的后验分布中采样隐状态。现有的近似采样方法多样，但缺乏系统选择指南，特别是在仅能通过采样访问仿真器时。

Method: 将问题简化为条件分布选择任务，提出了两种信仰状态选择方法：基于隐状态选择（直接针对隐状态的条件分布）和基于观测选择（针对诱导的观测分布），并分析了它们在Single-Reset和Repeated-Reset两种不同滚动方法下的交互机制。

Result: 研究发现两种方法在不同的滚动方法下表现不同：基于观测的选择在传统的Single-Reset方法下可能失效，但在新的Repeated-Reset方法下具有理论保证。同时揭示了分布漂移和采样策略选择等复杂问题。

Conclusion: 论文揭示了看似简单的状态重置问题实际上包含丰富的算法选择、理论细微差别和开放性问题，特别是在隐变量存在的情况下，选择何种信仰状态选择方法与滚动方法的相互作用需要仔细考量。
...
### [133] [Probabilistic Hash Embeddings for Online Learning of Categorical Features](https://arxiv.org/abs/2511.20893)
*Aodong Li,Abishek Sankararaman,Balakrishnan Narayanaswamy*

Main category: cs.LG

TL;DR: 提出概率哈希嵌入（PHE）模型处理流式数据中的类别特征，通过贝叶斯在线学习方法增量学习，解决了确定性嵌入易受数据到达顺序影响和遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 传统特征哈希方法在处理流式数据中不断变化的类别特征时存在两大问题：对类别到达顺序敏感，以及在线学习中的遗忘问题，导致性能下降。

Method: 将哈希嵌入视为随机变量，应用贝叶斯在线学习框架增量学习模型参数，推导出可扩展的推理算法来学习和更新哈希嵌入及其他隐变量的后验分布。

Result: 实验在分类、序列建模和推荐系统中验证了PHE的优越性能，同时保持高内存效率（内存消耗仅为one-hot嵌入表的2-4%）。

Conclusion: PHE模型能够有效处理不断演化的类别特征，适应新项目而不遗忘旧项目，且对项目到达顺序具有不变性，为在线学习场景提供了高效解决方案。
...
### [134] [Evolved SampleWeights for Bias Mitigation: Effectiveness Depends on Optimization Objectives](https://arxiv.org/abs/2511.20909)
*Anil K. Saini,Jose Guadalupe Hernandez,Emily F. Wong,Debanshi Misra,Jason H. Moore*

Main category: cs.LG

TL;DR: 本文比较了三种数据点权重生成方法（遗传算法演化、基于数据集特征计算、等权重分配）在提升机器学习模型公平性和预测性能方面的效果，发现遗传算法能够实现更好的公平性与性能平衡。


<details>
  <summary>Details</summary>
Motivation: 现实中，基于实际数据训练的机器学习模型可能产生有偏见的预测，这种偏见会对弱势群体造成负面影响。加权方法通过为每个数据点分配权重来缓解这种偏见。

Method: 研究比较了三种权重生成方法：1）使用遗传算法演化权重；2）仅基于数据集特征计算权重；3）所有数据点赋予等权重。评估使用了两个预测指标（准确率和AUC）和两个公平性指标（人口统计均等差值和子组假阴性公平性），并作为遗传算法的优化目标。

Result: 通过11个公开数据集（包括2个医疗数据集）的实验表明，使用演化样本权重训练的模型在公平性与预测性能之间能够实现比替代方法更好的平衡。优化效果强烈依赖于优化目标的选择，其中使用准确率和人口统计均等差值作为优化目标时，演化权重在最多数据集上显著优于其他方法。

Conclusion: 演化样本权重能够产生在公平与预测性能之间具有更好权衡的模型，但优化收益的大小强烈取决于优化目标的选择，使用准确率和人口统计均等差值作为优化目标效果最佳。
...
### [135] [Exploring Time-Step Size in Reinforcement Learning for Sepsis Treatment](https://arxiv.org/abs/2511.20913)
*Yingchuan Sun,Shengpu Tang*

Main category: cs.LG

TL;DR: 本研究在脓毒症管理的强化学习领域，通过控制实验比较了四种不同时间步长（1、2、4、8小时）的影响，发现更精细的时间步长（1-2小时）能获得更好的性能和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有脓毒症管理的强化学习研究大多使用4小时时间步长的设定，而这种粗粒度的时间步可能扭曲患者动态导致次优治疗策略，需要实证研究评估时间步长对性能的实际影响。

Method: 采用相同的离线强化学习流程，比较四种时间步长，设计了动作重映射方法来确保跨时间步长的公平评估，并在两种政策学习设置下进行交叉时间步长模型选择。

Result: 结果表明，不同时间步长的性能趋势随学习设置而变化，使用静态行为策略在更精细时间步长（1-2小时）学习的政策获得了总体最佳性能和稳定性。

Conclusion: 时间步长是医疗保健离线强化学习的核心设计选择，研究结果为超越传统4小时设置提供了支持证据。
...
### [136] [Operationalizing Quantized Disentanglement](https://arxiv.org/abs/2511.20927)
*Vitoria Barin-Pacela,Kartik Ahuja,Simon Lacoste-Julien,Pascal Vincent*

Main category: cs.LG

TL;DR: Cliff方法通过鼓励密度函数的轴对齐不连续性来实现无监督解缠，利用量子化因素的理论特性，在基准测试中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有理论表明，在任意微分同胚下，量子化因素具有无监督可识别性，但将这一理论原理转化为有效的实践标准仍然具有挑战性，特别是在非线性映射下。

Method: 开发Cliff方法，通过鼓励密度函数中出现轴对齐的不连续性（称为cliffs），并且确保沿着某个因素的不连续性位置与其他因素的值独立。

Result: 在所有的解缠基准测试中，Cliff方法都优于基线方法，证明了其在无监督解缠中的有效性。

Conclusion: 通过鼓励轴对齐的不独立性作为实践标准，可以有效地实现无监督解缠学习，并且该方法在各种基准测试中都表现出优越性能。
...
### [137] [Semantic Superiority vs. Forensic Efficiency: A Comparative Analysis of Deep Learning and Psycholinguistics for Business Email Compromise Detection](https://arxiv.org/abs/2511.20944)
*Yaw Osei Adjei*

Main category: cs.LG

TL;DR: 使用CatBoost的心理学特征分析和DistilBERT的深度学习语义理解两种方法来检测BEC攻击，在对抗性测试中DistilBERT达到近乎完美的检测效果，CatBoost则提供更低延迟的解决方案，两种方法通过成本敏感学习都能实现超过99.96%的投资回报率。


<details>
  <summary>Details</summary>
Motivation: BEC攻击利用社会工程学手段导致重大经济损失，根据FBI报告年损失超过29亿美元，存在显著的经济不对称性。

Method: 提出两种检测范式：语言学特征流使用CatBoost分析心理语言线索，语义流使用DistilBERT进行深度学习语言理解，在对抗性污染数据集上进行评估。

Result: DistilBERT在Tesla T4 GPU上实现接近完美检测（AUC=1.0000，F1=0.9981），延时7.403毫秒；CatBoost达到竞争性检测（AUC=0.9905，F1=0.9486），延时降低8.4倍仅0.885毫秒。

Conclusion: 拥有GPU基础设施的组织可从DistilBERT获得更高精度，而边缘部署或成本敏感环境更适合CatBoost，两种方法通过成本敏感学习都能显著降低经济损失，实现高投资回报率。
...
TL;DR: 该研究首次分析了行为克隆策略在干净标签后门攻击下的脆弱性，发现即使使用轻微污染数据集训练的策略也会在部署时对后门触发攻击高度敏感


<details>
  <summary>Details</summary>
Motivation: 随着行为克隆策略在现实世界中部署使用，其鲁棒性和潜在漏洞成为重要关切，需要进行安全性分析

Method: 通过向演示数据集中注入视觉触发器来污染数据集，建立虚假相关性，并在测试时利用这些相关性

Result: 即使在极小比例污染数据上训练的行为克隆策略，任务性能看起来接近基线水平，但在部署时对后门触发攻击高度脆弱

Conclusion: 行为克隆策略的安全性研究急需加强，特别是在大规模数据集被用于训练现实世界物理系统策略的背景下
...
### [139] [Predictive Safety Shield for Dyna-Q Reinforcement Learning](https://arxiv.org/abs/2511.21531)
*Jin Pin,Krasowski Hanna,Vanneaux Elena*

Main category: cs.LG

TL;DR: 该论文提出了一种用于模型强化学习的前向预测安全性护盾，能在维持硬性安全保证的同时提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习缺乏安全保证，现有安全护盾方法通过随机采样安全动作或使用固定后备控制器，忽略了不同安全动作对未来性能的影响。

Method: 在离散空间中，基于环境模型的安全仿真进行安全预测，通过安全预测局部更新Q函数，构建预测性安全护盾。

Result: 网格世界环境实验表明，即使很短的预测时长也足以识别最优路径，且该方法对分布漂移（如仿真与现实差异）具有鲁棒性，无需额外训练。

Conclusion: 提出的预测安全护盾方法在保证硬性安全的前提下显著提升强化学习智能体性能，具有实际应用价值。
...
### [140] [Subgoal Graph-Augmented Planning for LLM-Guided Open-World Reinforcement Learning](https://arxiv.org/abs/2511.20993)
*Shanwei Fan*

Main category: cs.LG

TL;DR: SGA-ACR是解决大语言模型在强化学习中规划-执行对齐问题的框架，通过环境特定的子目标图谱和多LLM规划流水线，在开放世界游戏Crafter的22个任务中表现有效。


<details>
  <summary>Details</summary>
Motivation: 大语言模型为强化学习提供高层次规划能力，但存在规划-执行不对齐问题，包括子目标在环境中不可行以及单一LLM规划缺乏自我验证导致不可靠。

Method: 提出SGA-ACR框架，整合环境特定子目标图谱和结构化实体知识，采用多LLM规划流水线分离生成、评估和优化，并引入子目标跟踪器监控执行进度和自适应更新图谱。

Result: 在开放世界游戏Crafter的22个多样化任务中，该方法证明能产生可执行且可验证的子目标。

Conclusion: SGA-ACR通过结构化知识图谱和多LLM协作机制，成功解决了大语言模型在强化学习中的规划-执行对齐问题。
...
### [141] [FANoise: Singular Value-Adaptive Noise Modulation for Robust Multimodal Representation Learning](https://arxiv.org/abs/2511.20997)
*Jiaoyang Li,Jun Fang,Tianhao Gao,Xiaohui Zhang,Zhiyuan Liu,Chao Liu,Pengzhang Liu,Qixia Jiang*

Main category: cs.LG

TL;DR: 该论文提出了FANoise方法，在表示学习中引入特征自适应噪声注入机制，针对多模态任务提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有表示学习方法大多依赖启发式或静态噪声注入，忽略了训练过程中特征分布的动态特性。为了增强表示的鲁棒性和泛化性，需要研究噪声在多模态表示学习中的系统作用。

Method: 基于InfoNCE损失，从梯度和特征分布角度系统研究噪声作用，提出FANoise特征自适应噪声注入策略，利用对比学习的动态特性平衡噪声的利弊。

Result: 在多种基础VLM模型上进行综合实验，FANoise方法在多模态任务上能持续提升整体性能。

Conclusion: 特征自适应噪声注入是一种有效的表示学习方法，能够显著提升多模态表示学习的性能表现。
...
### [142] [Estimating Ising Models in Total Variation Distance](https://arxiv.org/abs/2511.21008)
*Constantinos Daskalakis,Vardis Kandiros,Rui Yao*

Main category: cs.LG

TL;DR: 本文提出了一种最大伪似然估计器(MPLE)的统一分析框架，用于估计总变差距离中的Ising模型，涵盖有界算子范数且满足改进对数索波列夫不等式(MLSI)的模型，以及有界无穷范数(宽度)的模型。


<details>
  <summary>Details</summary>
Motivation: Ising模型的统计复杂度已被充分理解，但在TV距离中识别计算和统计效率高的算法仍具挑战性。现有工作只处理特定设置(如树形图、高斯交互矩阵等)，缺乏统一的TV多项式时间估计框架。

Method: 采用最大伪似然估计器(MPLE)进行分析，利用张量化不等式、测度分解和集中界等工具，研究两类广义Ising模型：有界算子范数且满足MLSI的模型，以及有界无穷范数的模型。

Result: 为这两类模型提供了多项式时间算法，并在多种设置下获得了最优或接近最优的样本复杂度保证。

Conclusion: 提出了一种统一的TV距离下Ising模型估计框架，扩展了先前仅限于特定设置的结果，为更广泛的Ising模型提供了计算高效的估计方法。
...
### [143] [ChatGpt Content detection: A new approach using xlm-roberta alignment](https://arxiv.org/abs/2511.21009)
*Md Tasnin Tanvir,Dr Santanu Kumar Dash,Ishan Shahnan,Nafis Fuad,Tanvir Rahman,Abdullah Al Faisal,Asadullah Al Mamun*

Main category: cs.LG

TL;DR: 该研究使用XLM-RoBERTa模型开发AI生成文本检测方法，包含全AI生成和人工重写的文本检测，通过特征提取和模型微调实现高精度检测。


<details>
  <summary>Details</summary>
Motivation: 随着ChatGPT等生成式AI技术的普及，区分AI生成文本和人类撰写内容的需求日益迫切，需要维护学术诚信和AI伦理透明度。

Method: 采用XLM-RoBERTa多语言转换模型，结合严格的预处理和特征提取（困惑度、语义、可读性特征），在平衡数据集上微调模型。

Result: 模型在各种文本类型中表现出高准确率和稳健性能，特征分析发现困惑度和注意力特征对区分人类与AI生成文本至关重要。

Conclusion: 研究为维护学术诚信提供了实用工具，未来需要探索更先进模型和扩展数据集以提升泛化能力。
...
### [144] [Staggered Environment Resets Improve Massively Parallel On-Policy Reinforcement Learning](https://arxiv.org/abs/2511.21011)
*Sid Bharthulwar,Stone Tao,Hao Su*

Main category: cs.LG

TL;DR: 该论文提出交错重置(staggered resets)技术，解决了大规模并行GPU环境中同步重置导致的训练不稳定性问题，显著提升了强化学习训练的样本效率和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 为了解决大规模并行GPU环境中使用短回合进行策略更新时，标准同步重置引起的有害非平稳性，避免学习信号扭曲和训练不稳定性。

Method: 提出交错重置技术，让环境在任务时间窗口内的不同时间点进行初始化和重置，从而产生具有更大时间多样性的训练批次，减少同步回合引入的非平稳性。

Result: 在挑战性的高维机器人环境中实现了显著更高的样本效率、更快的实际时间收敛和更强的最终性能，该技术相比原始同步回合能更好地扩展并行环境数量。

Conclusion: 交错重置是一种简单有效的技术，能够缓解并行强化学习训练中的非平稳性问题，显著提升训练效率和性能表现。
...
### [145] [A Probabilistic Framework for Temporal Distribution Generalization in Industry-Scale Recommender Systems](https://arxiv.org/abs/2511.21032)
*Yuxuan Zhu,Cong Fu,Yabo Ni,Anxiang Zeng,Yuan Fang*

Main category: cs.LG

TL;DR: ELBO$_{TDS}$是一个用于解决推荐系统时间分布偏移问题的概率框架，通过数据增强和基于因果图的自监督变分目标来提升时间泛化性能。


<details>
  <summary>Details</summary>
Motivation: 时间分布偏移会削弱推荐系统的长期准确性，而现有的周期增量训练方法难以同时捕捉稳定和瞬态模式。现有方法在时间泛化稳定性、表示崩塌或数据利用效率方面存在不足。

Method: 首先通过统计分析识别关键偏移因子，设计有效的数据增强策略重新采样这些时变因子以扩展训练支持。然后基于因果图建模时序推荐场景，推导出基于因果结构的自监督变分目标ELBO$_{TDS}$。

Result: 实验证明该方法实现了优越的时间泛化性能，在GMV上获得2.33%的提升，并已在Shopee产品搜索中成功部署。

Conclusion: ELBO$_{TDS}$框架能够有效解决推荐系统中的时间分布偏移问题，具有实际工业部署价值。
...
### [146] [Prediction of Herd Life in Dairy Cows Using Multi-Head Attention Transformers](https://arxiv.org/abs/2511.21034)
*Mahdi Saki,Justin Lipman*

Main category: cs.LG

TL;DR: 开发基于AI的奶牛寿命预测模型，该研究旨在帮助奶农基于历史数据客观评估奶牛的生产力，从而优化牛群管理决策，提高经济效益。


<details>
  <summary>Details</summary>
Motivation: 奶农需要基于客观数据决定是否保留或淘汰奶牛，从而识别更具韧性的奶牛以提升牛群生产力和经济效益。这一决策过程复杂且环境影响重大。

Method: 利用多头注意力变换器分析多头历史时间序列数据，涵盖7个澳大利亚农场19000头奶牛的约780000条记录。

Result: 模型在预测牛群寿命方面达到83%的决定系数，表明其在农场管理中具有实际应用潜力。

Conclusion: AI驱动的预测模型为奶农提供了强大的决策支持工具，有望通过预测奶牛寿命优化牛群管理和资源配置。
...
### [147] [FedAPA: Federated Learning with Adaptive Prototype Aggregation Toward Heterogeneous Wi-Fi CSI-based Crowd Counting](https://arxiv.org/abs/2511.21048)
*Jingtao Guo,Yuyi Mao,Ivan Wang-Hei Ho*

Main category: cs.LG

TL;DR: 提出FedAPA算法解决Wi-Fi CSI感知系统中的联邦学习数据异构性和资源不足问题，通过自适应原型聚合策略实现个性化全局原型构建，在真实分布式Wi-Fi人群计数场景中显著提升性能并大幅降低通信开销


<details>
  <summary>Details</summary>
Motivation: Wi-Fi CSI感知虽然提供了非侵入式的设备无关感知方法，但大规模部署面临需要大量站点特定训练数据的问题。联邦学习虽然避免原始数据共享，但受到异构感知数据和设备资源的挑战

Method: 采用自适应原型聚合策略为对等原型分配基于相似度的权重，实现自适应客户端贡献，为每个客户端生成个性化全局原型而非固定权重聚合。在本地训练中采用结合分类学习和表示对比学习的混合目标函数来对齐本地和全局知识

Result: 在真实分布式Wi-Fi人群计数场景中，相比多个基线方法，FedAPA在准确性、F1分数、平均绝对误差和通信开销方面均表现更优。具体来说，准确性提升至少9.65%，F1分数提升9%，MAE减少0.29，通信开销降低95.94%

Conclusion: FedAPA算法通过自适应原型聚合策略有效解决了联邦学习中的异构数据问题，在Wi-Fi CSI感知场景中实现了优越的性能和通信效率，为大规部署提供了可行的技术方案
...
### [148] [Breaking the Safety-Capability Tradeoff: Reinforcement Learning with Verifiable Rewards Maintains Safety Guardrails in LLMs](https://arxiv.org/abs/2511.21050)
...
TL;DR: 首次从理论和实证角度分析RLVR训练方法中的安全特性，证明了在特定条件下可以消除大语言模型下游任务微调中的安全退化和能力提升的必然权衡。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在下游任务微调中存在的基本安全-能力权衡问题：改进任务性能会降低安全性对齐，即使使用SFT和RLHF方法仍有此问题。RLVR虽能优化可测量任务但安全影响未探索。

Method: 理论分析：推导KL约束优化下安全漂移上界，证明安全退化消除条件；实证分析：在5个对抗安全基准上广泛实验，包括优化算法、模型规模和任务域的消融研究。

Result: RLVR能同时增强推理能力并保持或改进安全防护；理论证明在某些条件下完全消除安全退化；实证验证此方法打破安全-能力必然权衡假设。

Conclusion: 挑战了安全-能力权衡的普遍假设，证明了特定训练方法可以同时实现两个目标，为安全部署具备推理能力的大语言模型提供新见解。
...
### [149] [Efficient Diffusion Planning with Temporal Diffusion](https://arxiv.org/abs/2511.21054)
*Jiaming Guo,Rui Zhang,Zerun Li,Yunkai Gao,Shaohui Peng,Siming Lan,Xing Hu,Zidong Du,Xishan Zhang,Ling Li*

Main category: cs.LG

TL;DR: TDP通过时间维度分布去噪步骤改进决策效率，相比现有方法将决策频率提升11-24.8倍，同时达到更高或相当的D4RL基准性能


<details>
  <summary>Details</summary>
Motivation: 现有扩散规划方法在每个时间步生全新计划，造成显著计算开销和低决策频率，而人类倾向于制定详细的短期计划和较模糊的长期计划并随时间调整

Method: 提出时间扩散规划器(TDP)：初始生成随模糊度递增的规划；后续时间步用少量去噪步骤更新而非重新生成；加入自动重新规划机制防止计划与现实偏差过大

Result: D4RL基准测试中，相较每时间步生成新计划的方法，TDP提升决策频率11-24.8倍，同时取得更高或可比的性能水平

Conclusion: TDP通过时间维度的去噪步骤分布显著提升了决策效率，同时保持了高规划质量，为扩散规划提供了新颖有效的方法
...
### [150] [A Unified Understanding of Offline Data Selection and Online Self-refining Generation for Post-training LLMs](https://arxiv.org/abs/2511.21056)
*Quan Xiao,Tianyi Chen*

Main category: cs.LG

TL;DR: 该研究提出了一个统一框架，通过双层数据选择优化离线数据筛选和在线自优化生成，为LLMs在下游任务中的适应性提供系统性解决方案。


<details>
  <summary>Details</summary>
Motivation: 针对大语言模型在特定下游任务中的适应性问题，研究者希望解决离线数据选择和在线自优化生成这两个关键步骤，以提升数据质量和模型性能。

Method: 采用双层数据选择框架进行离线数据筛选，并将在线自优化生成视为模型适配步骤，通过为每个问题和响应学习数据权重来统一理解这两个过程。

Result: 理论证明了双层数据选择框架的有效性，在质量增强和安全感知的LLM微调实验中表现出优于无筛选直接混合基线方法的性能提升。

Conclusion: 结合离线数据与基于验证权重的在线生成，该框架显著提升了微调性能，为LLMs在特定任务中的适应提供了一种有效的优化方法。
...
### [151] [Aligning LLMs with Biomedical Knowledge using Balanced Fine-Tuning](https://arxiv.org/abs/2511.21075)
...
TL;DR: 提出平衡微调(BFT)方法，通过两层加权机制从稀疏生物医学数据中学习复杂推理，无需外部奖励信号，显著提升生物医学任务性能


<details>
  <summary>Details</summary>
Motivation: 当前方法在将大语言模型与生物医学知识对齐时存在局限：标准监督微调容易对表面指令模式过拟合，而强化学习因需要实验验证而不可行

Method: BFT采用双层加权机制：1) 在token级别通过预测概率缩放损失稳定梯度；2) 在样本级别使用'最小组置信度'自适应增强困难样本学习

Result: BFT在医学任务中使LLMs学到了SFT遗漏的知识，在生物学任务中超越了GeneAgent，其生成的文本嵌入可直接用于下游任务

Conclusion: BFT方法促进了LLMs在生物医学研究中的广泛应用，能够有效从稀疏数据中学习复杂推理
...
### [152] [Deceptron: Learned Local Inverses for Fast and Stable Physics Inversion](https://arxiv.org/abs/2511.21076)
*Aaditya L. Kachhadiya*

Main category: cs.LG

TL;DR: 该论文提出了一个名为Deceptron的轻量级双向模块，用于解决物理科学中的逆问题。该模块通过优化输出空间中的下降步长，大幅减少了迭代次数。


<details>
  <summary>Details</summary>
Motivation: 物理科学中的逆问题通常在输入空间不适定，导致优化步长敏感，因此需要一个高效且稳健的求解器。

Method: Deceptron模块结合了监督拟合、前向-反向一致性、轻量级频谱惩罚、软偏置约束和雅可比组成惩罚（JCP），通过JVP/VJP探测鼓励局部逆模型的准确性。求解时使用D-IPG方法在输出空间下降步长，并通过回溯和停止规则进行优化。

Result: 在Heat-1D初始条件恢复和阻尼振荡器逆问题中，D-IPG达到固定归一化容差所需的迭代次数显著减少（Heat问题减少约20倍，Oscillator问题减少2-3倍），与高斯-牛顿方法在迭代次数和成本上竞争力相当。

Conclusion: Deceptron模块及其D-IPG方法在逆问题求解中表现出高效的迭代性能，并通过JCP惩罚验证了其准确性。此外，DeceptronNet的初步实验展示了快速的收敛特性。
...
### [153] [MLPMoE: Zero-Shot Architectural Metamorphosis of Dense LLM MLPs into Static Mixture-of-Experts](https://arxiv.org/abs/2511.21089)
*Ivan Novikov*

Main category: cs.LG

TL;DR: MLPMoE是一种无需训练、确定性的转换方法，将Transformer块中的稠密MLP转换为静态、高基数的专家混合结构，利用张量切片和求和操作，在不改变参数数量的情况下保持性能，同时通过分支稀疏化技术实现结构化剪枝。


<details>
  <summary>Details</summary>
Motivation: 解决稠密变压器模型计算效率低下的问题，因为当前部署的LLMs对所有token激活所有参数，导致计算成本随参数数量线性增长，而现有方法需要聚类、激活分析等复杂操作。

Method: MLPMoE通过重新解释张量并行性的代数，将稠密MLP转换为静态专家混合结构；引入Fractal Fade和Compensated Pruning技术实现结构化稀疏化；无需训练、梯度或校准数据。

Result: 在Qwen2.5-0.5B和DeepSeek-R1-Distill-Llama-8B模型上，零样本转换时困惑度变化小于0.05%，参数数量保持恒定；在8B模型上，微分稀疏化可移除约20%的MLP参数，困惑度保持在稠密基线的约2%以内。

Conclusion: MLPMoE提供了一种高效转换现有检查点的方法，不依赖路由训练或校准数据，显著减少计算需求同时保持模型性能。
...
### [154] [MNM : Multi-level Neuroimaging Meta-analysis with Hyperbolic Brain-Text Representations](https://arxiv.org/abs/2511.21092)
*Seunghun Baek,Jaejin Lee,Jaeyoon Sim,Minjae Jeong,Won Hwa Kim*

Main category: cs.LG

TL;DR: 提出一种利用双曲几何将神经科学文献和脑激活图嵌入共享双曲空间的新框架，通过Lorentz模型捕捉神经影像数据的语义相似性和层次结构，实现多层次神经影像元分析。


<details>
  <summary>Details</summary>
Motivation: 解决神经影像研究中样本量小导致的可靠性问题，传统元分析方法忽视大脑丰富层次结构，需要更有效的多源研究整合方法。

Method: 通过Lorentz模型将研究文本和对应脑图像嵌入共享双曲空间，实现三方面功能：脑-文本嵌入对齐、文本-脑激活层次引导、脑激活模式层次关系保持。

Result: 实验结果表明该方法优于基线模型，为多层次神经影像元分析提供了鲁棒且可解释的范式。

Conclusion: 双曲几何方法能有效捕捉神经影像数据的语义和层次特性，为多级神经影像元分析提供了新的可行途径。
...
### [155] [Generative Early Stage Ranking](https://arxiv.org/abs/2511.21095)
...
TL;DR: 提出生成式早期排名(GESR)范式，通过混合注意力机制提升用户-物品交互建模，解决传统早期排名系统的效果限制。


<details>
  <summary>Details</summary>
Motivation: 传统早期排名系统的用户-物品解耦设计效率高但效果有限，无法捕捉细粒度用户-物品亲和度和交叉信号，存在效果差距。

Method: 引入混合注意力模块：硬匹配注意力编码显式交叉信号，目标感知自注意力生成目标感知用户表征，交叉注意力促进早期用户-物品交互；通过多层参数化门控模块精炼注意力编码。

Result: 在顶层指标、参与度和消费任务上取得显著提升，通过离线和在线实验验证。

Conclusion: GESR范式成功实现了在早期排名阶段部署完整的目标感知注意力序列建模，平衡了效果与效率。
...
### [156] [From Bits to Rounds: Parallel Decoding with Exploration for Diffusion Language Models](https://arxiv.org/abs/2511.21103)
*Hengyu Fu,Baihe Huang,Virginia Adams,Charles Wang,Venkat Srinivasan,Jiantao Jiao*

Main category: cs.LG

TL;DR: 本文提出ETL解码策略解决扩散语言模型的信息瓶颈问题，通过结合跨块解码和高不确定性标记探索，在保持生成质量的同时显著减少解码轮次。


<details>
  <summary>Details</summary>
Motivation: 标准的DLM解码策略依赖高置信度tokens会导致信息理论瓶颈，限制解码进度并减慢生成速度。理论和实验表明优先处理高概率tokens本质上效率低下，因为它们携带的信息量很小。

Method: 提出探索-利用策略，结合跨块解码和对高不确定性标记的定向探索，以重塑条件分布并引发级联式的自信预测，实现信息吞吐量和解码效率最大化。

Result: 实验验证了理论界限，ETL与仅基于置信度的基线方法相比，在不影响生成质量的情况下持续减少所需的解码轮次。

Conclusion: 本研究证明扩散语言模型的解码轮次必须与样本总信息量成线性增长，与每轮信息预算成反比，提出的ETL策略有效突破了传统解码方法的信息瓶颈。
...
### [157] [BRIDGE: Building Representations In Domain Guided Program Verification](https://arxiv.org/abs/2511.21104)
*Robert Joseph George,Carson Eisenach,Udaya Ghai,Dominique Perrault-Joncas,Anima Anandkumar,Dean Foster*

Main category: cs.LG

TL;DR: BRIDGE是首个研究结构化提示在可扩展验证程序生成中的应用，通过将验证分解为代码、规范和证明三个领域，并引导不同类型推理行为，显著提升验证效率和准确率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在代码生成方面表现出色，但在程序验证，特别是Lean4等交互式证明框架中存在可扩展性挑战，很难同时处理代码、规范和证明三个领域的需求。

Method: BRIDGE采用结构化提示方法，将验证分解为代码（可执行实现）、规范（形式化意图声明）和证明（构造性正确性论证）三个相互连通的领域，引导不同的推理行为作为中间表示。

Result: 该方法显著提升了准确率和效率：函数推理将正式语言代码正确率提高了近1.5倍，推理计算效率提升2倍；规范驱动提示将Python代码通过率提升高达17.5%。

Conclusion: 结构化领域对齐是推进验证综合的有前景方向，BRIDGE为通过专家迭代或RLVR训练提供了基础，使模型能在代码、规范和证明之间内化这些推理策略。
...
### [158] [Dynamic Stratified Contrastive Learning with Upstream Augmentation for MILP Branching](https://arxiv.org/abs/2511.21107)
*Tongkai Lu,Shuai Ma,Chongyang Tao*

Main category: cs.LG

TL;DR: 提出了一种动态分层对比训练框架StratC，用于解决MILP分支策略中的语义变化、上游节点稀缺和强分支样本采集成本高的问题，通过分组训练和上游增强有效提升分支准确性和求解效率。


<details>
  <summary>Details</summary>
Motivation: 解决现有神经学习方法在MILP分支策略中面临的三个主要挑战：节点语义随深度变化、上游节点稀缺以及强分支样本收集成本高昂。

Method: 提出StratC框架，基于特征分布对分支定界节点进行分组，训练GCNN判别模型逐步分离不同组别节点，并引入上游增强MILP推导过程生成等价和扰动实例解决数据不平衡问题。

Result: 在标准MILP基准测试上显著提升分支准确性，减少求解时间，并证明对未见实例具有良好泛化能力，特别是在上游节点方面表现突出。

Conclusion: StratC框架通过动态分层对比训练和上游增强，能够有效建模节点间细微语义差异，提高MILP问题求解效率，尤其在分支策略方面取得显著改进。
...
### [159] [Interpretable Fair Clustering](https://arxiv.org/abs/2511.21109)
*Mudi Jiang,Jiahui Zhou,Xinying Liu,Zengyou He,Zhikui Chen*

Main category: cs.LG

TL;DR: 提出一个可解释的公平聚类框架，将公平约束集成到决策树结构中，无需公平超参数调节，实现了竞争性的聚类性能和更好的公平性
...
Method: 构建可解释的决策树来划分数据，同时确保受保护群体间的公平对待；引入无需公平超参数调节的变体，通过对无约束构建的树进行后剪枝

Result: 在真实世界和合成数据集上的大量实验表明，该方法不仅提供竞争性聚类性能和改善的公平性，还具有可解释性、处理多敏感属性等优势

Conclusion: 方法在复杂公平约束下表现出强大的鲁棒性，为公平透明聚类开辟了新可能性
...
### [160] [Trustless Federated Learning at Edge-Scale: A Compositional Architecture for Decentralized, Verifiable, and Incentive-Aligned Coordination](https://arxiv.org/abs/2511.21118)
*Pius Onobhayedo,Paul Osemudiame Oamen*

Main category: cs.LG

TL;DR: 该论文提出了一个解决联邦学习挑战的框架，通过密码学收据、几何新颖性度量等技术来填补当前的构成性差距。


<details>
  <summary>Details</summary>
Motivation: 当前联邦学习存在聚合器缺乏问责、缺乏经济机制且易受操纵、状态修改串行化限制可扩展性、治理允许追溯性操纵等问题，阻碍了AI民主化愿景的实现。

Method: 利用密码学收据证明聚合正确性，使用几何新颖性测量防止激励博弈，采用并行对象所有权实现线性可扩展性，以及时间锁定策略来检查追溯性操纵。

Result: 提出了一个综合性的技术框架，有效解决联邦学习中的关键挑战，为大规模分布式AI学习铺平道路。

Conclusion: 所提出的方法填补了联邦学习的关键构成性差距，为实现从集中式AI到分布式AI创造的民主化转变提供了可行路径。
...
### [161] [Learning Cell-Aware Hierarchical Multi-Modal Representations for Robust Molecular Modeling](https://arxiv.org/abs/2511.21120)
*Mengran Li,Zelin Zang,Wenbin Xing,Junzhou Chen,Ronghui Zhang,Jiebo Luo,Stan Z. Li*

Main category: cs.LG

TL;DR: CHMR是一个新颖的细胞感知分层多模态表示框架，通过联合建模分子与细胞响应之间的局部-全局依赖关系，并使用树结构向量量化模块捕获潜在生物层次结构，在9个公共基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注化学结构，而细胞反应（如形态和基因表达）对药物效应具有关键影响。当前细胞感知方法存在外部生物数据模态不完整性和分子、细胞、基因组层次依赖关系建模不足的问题。

Method: 提出CHMR框架，联合建模分子与细胞响应之间的局部-全局依赖关系，通过新颖的树结构向量量化模块捕获潜在生物层次结构，解决多模态不完整和层次依赖建模问题。

Result: 在包含728个任务的9个公共基准测试中，CHMR显著优于现有最优基线方法，分类任务平均提升3.6%，回归任务平均提升17.2%。

Conclusion: 研究证明了层次感知、多模态学习在构建可靠且生物学基础扎实的分子表示方面的优势，为整合生物医学建模提供了一个可推广的框架。
...
### [162] [How to Correctly Report LLM-as-a-Judge Evaluations](https://arxiv.org/abs/2511.21140)
*Chungpa Lee,Thomas Zeng,Jongwon Jeong,Jy-yong Sohn,Kangwook Lee*

Main category: cs.LG

TL;DR: 论文提出了一个用于修正大语言模型（LLM）评估偏差的插件框架，能够构建置信区间并减少估计不确定性。


<details>
  <summary>Details</summary>
Motivation: 减少LLM作为评估器时因不完美的特异性和敏感性导致的噪声和偏差，解决现有偏差校正方法在LLM研究中应用不足的问题。

Method: 采用简单的插件框架进行偏差校正和置信区间构建，并引入自适应算法优化校准样本量分配。

Result: 框架能够实现实用的、统计上合理的LLM评估，减少准确率估计的不确定性。

Conclusion: 该工作提供了一个统计可靠、实用的LLM评估解决方案，有效解决了校准和测试数据集中的不确定性问题。
...
### [163] [I-GLIDE: Input Groups for Latent Health Indicators in Degradation Estimation](https://arxiv.org/abs/2511.21208)
*Lucas Thil,Jesse Read,Rim Kaddah,Guillaume Doquet*

Main category: cs.LG

TL;DR: 该论文提出一个基于不确定性量化健康指标构建的新框架，通过RaPP指标、不确定性增强和机制特异性指标组，显著提升剩余寿命预测的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统健康指标构建方法难以解耦复杂多传感器系统中的退化机制，也无法量化健康指标可靠性的不确定性。

Method: 采用重建沿投影路径(RaPP)作为健康指标，通过蒙特卡洛dropout和概率潜在空间进行不确定性量化，并提出指标组方法I-GLIDE来隔离传感器子集建模系统特定退化。

Result: 在航空航天和制造系统数据上的评估显示，该方法相比现有最优方法在准确性和泛化性方面有显著提升，并能提供可操作的系统故障路径洞察。

Conclusion: 该工作填补了异常检测与预测之间的差距，为复杂系统中的不确定感知退化建模提供了一个原则性框架。
...
### [164] [Robust Gene Prioritization via Fast-mRMR Feature Selection in high-dimensional omics data](https://arxiv.org/abs/2511.21211)
*Rubén Fernández-Farelo,Jorge Paz-Ruza,Bertha Guijarro-Berdiñas,Amparo Alonso-Betanzos,Alex A. Freitas*

Main category: cs.LG

TL;DR: 提出了一个结合Fast-mRMR特征选择的基因优先级识别流程，能更稳健高效地从高维生物医学数据中识别相关基因。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理生物医学数据的高维性和标签不完整问题，需要更稳健的基因优先级识别方案。

Method: 使用Fast-mRMR特征选择方法保留相关且非冗余的特征，构建更简单有效的分类模型，并可组合不同生物特征集。

Result: 在Dietary Restriction数据集上的实验显示显著优于现有方法的性能提升。

Conclusion: 特征选择对于可靠的基因优先级识别至关重要，该方法为生物医学数据分析提供了有效解决方案。
...
### [165] [A Physics-Informed U-net-LSTM Network for Data-Driven Seismic Response Modeling of Structures](https://arxiv.org/abs/2511.21276)
*Sutirtha Biswas,Kshitij Kumar Yadav*

Main category: cs.LG

TL;DR: 本文提出了一种结合物理规律的Physics Informed U-Net LSTM框架，用于结构非线性地震响应预测，在保证精度的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 有限元方法在非线性地震分析中存在高计算开销问题，纯粹的深度学习模型虽然能降低计算成本，但泛化能力差且无法捕捉物理规律，需要一种能结合物理原理和数据驱动优势的方法。

Method: 采用Physics Informed U-Net LSTM混合框架，在深度学习过程中嵌入特定领域约束，将物理定律与机器学习相结合。

Result: 该模型在预测性能上优于传统机器学习架构，实现了精度和效率的平衡。

Conclusion: 这种混合方法弥合了纯数据驱动方法与基于物理建模之间的差距，为结构地震响应预测提供了强大且计算高效的替代方案。
...
### [166] [Sawtooth Sampling for Time Series Denoising Diffusion Implicit Models](https://arxiv.org/abs/2511.21320)
*Heiko Oppel,Andreas Spilz,Michael Munz*

Main category: cs.LG

TL;DR: 提出了一种结合隐式扩散模型和Sawtooth采样器的新方法，将DDPM的采样过程加速30倍，同时提升生成时间序列数据的分类质量


<details>
  <summary>Details</summary>
Motivation: DDPM能够生成合成时间序列数据以提高分类器性能，但其采样过程计算成本过高

Method: 结合隐式扩散模型和新型Sawtooth采样器，可应用于任何预训练的扩散模型以加速逆向过程

Result: 相比标准基准方法实现了30倍的速度提升，同时提升了生成序列在分类任务中的质量

Conclusion: 该方法显著提高了DDPM的采样效率，为合成时间序列数据的生成提供了实用解决方案
...
### [167] [Masks Can Be Distracting: On Context Comprehension in Diffusion Language Models](https://arxiv.org/abs/2511.21338)
*Julianna Piskorz,Cristina Pinneri,Alvaro Correia,Motasem Alfarra,Risheek Garrepalli,Christos Louizos*

Main category: cs.LG

TL;DR: 研究发现掩码扩散语言模型(MDLMs)在上下文理解方面存在两个关键限制：强烈的局部性偏见和附加掩码作为干扰物导致性能下降，提出了掩码无关损失函数来提高模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: MDLMs作为一种替代自回归语言模型的方案，其双向注意力和去噪目标理论上应能更均匀地利用上下文信息，但需要验证其实际上下文理解能力。

Method: 系统性地检查MDLMs的上下文理解能力，识别局部性偏见和掩码干扰问题，引入掩码无关损失函数来提高模型对不同掩码数量的不敏感性，并通过微调来减轻掩码的干扰效应。

Result: 发现MDLMs实际上存在强烈的局部性偏见，性能对相关信息在输入中的位置非常敏感；附加多个掩码标记会显著降低上下文理解能力；使用掩码无关损失函数微调后，模型对掩码干扰的鲁棒性得到显著提升。

Conclusion: 当前MDLM训练范式存在关键限制，掩码无关损失函数为构建具有更强上下文理解能力的基于扩散的语言模型提供了可行方案。
...
### [168] [Best Practices for Machine Learning Experimentation in Scientific Applications](https://arxiv.org/abs/2511.21354)
*Umberto Michelucci,Francesca Venturini*

Main category: cs.LG

TL;DR: 为科学研究中的机器学习实验提供实用结构化指南，重点关注可重复性、公平比较和透明报告，提出工作流程和评估指标来避免误导性结论。


<details>
  <summary>Details</summary>
Motivation: 机器学习在科学研究中日益普及，但实验设计和文档记录的质量参差不齐，导致结果不可靠。常见的基线选择不当、预处理不一致或验证不足等问题，可能对模型性能得出误导性结论。

Method: 提出分步工作流程，从数据集准备到模型选择和评估；引入评估过拟合和不稳定性的指标，包括对数过拟合比率(LOR)和复合过拟合分数(COS)；提供推荐实践和报告格式示例。

Result: 通过该指南和评估指标，研究人员能够建立可靠的基线，从应用于科学问题的机器学习模型中得出基于有效证据的见解。

Conclusion: 该工作为科学领域的机器学习实验提供了系统化框架，通过强调可重复性、公平比较和透明报告，提升了实验结果的可靠性和实用性，支持研究人员进行更稳健的机器学习研究。
...
### [169] [Hybrid-AIRL: Enhancing Inverse Reinforcement Learning with Supervised Expert Guidance](https://arxiv.org/abs/2511.21356)
*Bram Silue,Santiago Amaya-Corredor,Patrick Mannion,Lander Willem,Pieter Libin*

Main category: cs.LG

TL;DR: 论文通过对比传统AIDL方法在复杂不完全信息环境中的不足，提出Hybrid-AIRL（H-AIRL）方法——结合监督损失和随机正则化机制，显著提升了奖励函数推断和策略学习效果。


<details>
  <summary>Details</summary>
Motivation: Adversarial Inverse Reinforcement Learning（AIRL）在解决稀疏奖励问题方面表现优异，但在高度复杂、不完全信息环境中的性能尚未充分探索，尤其在扑克等复杂游戏中的效果有限。

Method: 提出Hybrid-AIRL（H-AIRL）方法，将专家数据的监督损失与随机正则化机制结合，增强奖励函数推断和策略学习。该方法在Gymnasium基准和HULHE扑克场景中进行验证，并通过可视化分析学习到的奖励函数。

Result: 实验结果表明，H-AIRL相比AIRL具有更高的样本效率和更稳定的学习性能，在具有挑战性的不完全信息环境中表现出色。

Conclusion: H-AIRL通过整合监督信号成功解决了复杂设置中的奖励推断问题，证明为处理具有挑战性的现实世界环境的有效框架。
...
### [170] [The Directed Prediction Change - Efficient and Trustworthy Fidelity Assessment for Local Feature Attribution Methods](https://arxiv.org/abs/2511.21363)
*Kevin Iselborn,David Dembinsky,Adriano Lucieri,Andreas Dengel*

Main category: cs.LG

TL;DR: 该研究提出了一种新颖的定向预测变化(DPC)指标，用于评估局部特征归因方法在机器学习模型中的保真度。相比现有的蒙特卡洛近似方法，DPC通过结合扰动和归因的方向性，实现了近10倍的速度提升和确定性结果。


<details>
  <summary>Details</summary>
Motivation: 现有保真度评估方法如Infidelity依赖蒙特卡洛近似，需要大量模型评估并引入随机采样不确定性，难以满足高风险医疗环境等对解释可靠性的严格要求。

Method: 在引导扰动实验中修改现有的预测变化(PC)指标，通过结合扰动和归因的方向性，提出定向预测变化(DPC)指标。在皮肤病变图像和金融表格数据两个数据集、两个黑盒模型、七个解释算法上进行了评估。

Result: DPC实现了近10倍的加速并消除了随机性，提供了确定性且可复现的评估过程。在4,744个不同解释上的测试表明，DPC与PC结合能够对面向基线和局部特征归因方法进行全面的计算效率评估。

Conclusion: DPC提供了一个确定性、可靠且高效的保真度评估程序，能够准确测量与局部Infidelity相同的属性，特别适合需要高度可信任解释的高风险应用场景。
...
### [171] [Anomaly Detection with Adaptive and Aggressive Rejection for Contaminated Training Data](https://arxiv.org/abs/2511.21378)
*Jungi Lee,Jungkwon Kim,Chi Zhang,Kwangsun Yoo,Seok-Joo Byun*

Main category: cs.LG

TL;DR: AAR方法通过动态异常排除处理污染数据，提升了异常检测在真实环境中的鲁棒性和性能


<details>
  <summary>Details</summary>
Motivation: 解决传统异常检测方法在训练数据受污染时性能下降的问题，特别是在噪声环境中正常与异常数据分布重叠的情况

Method: 提出自适应积极剔除方法，使用改进后的z分数和高斯混合模型阈值，结合硬性和软性剔除策略动态排除异常

Result: 在2个图像数据集和30个表格数据集上的实验显示，AAR在AUROC指标上比最先进方法提升了0.041

Conclusion: AAR为污染数据集提供了可扩展且可靠的解决方案，增强了在安全和医疗等领域的实际应用潜力
...
### [172] [BanglaASTE: A Novel Framework for Aspect-Sentiment-Opinion Extraction in Bangla E-commerce Reviews Using Ensemble Deep Learning](https://arxiv.org/abs/2511.21381)
*Ariful Islam,Md Rifat Hossen,Abir Ahmed,B M Taslimul Haque*

Main category: cs.LG

TL;DR: 本文提出BanglaASTE框架，用于孟加拉语方面情感三元组提取，建立了首个孟加拉语ASTE数据集，开发了结合语义相似性和图匹配的混合方法，通过集成模型实现89.9%的准确率。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语方面情感分析研究缺乏，现有研究不足且缺少专用数据集和框架，特别是在电商产品评论领域。

Method: 构建首个孟加拉语ASTE数据集(3,345条评论)，开发混合分类框架，结合图匹配和语义相似性技术，实施集成模型整合BanglaBERT和XGBoost。

Result: 集成模型取得了89.9%准确率和89.1% F1分数，在所有评估指标上显著优于基线模型。

Conclusion: 该研究推进了低资源语言情感分析的技术水平，为孟加拉语电商分析提供了可扩展解决方案，有效解决了非正式表达、拼写变化等文本处理挑战。
...
### [173] [Ensemble Performance Through the Lens of Linear Independence of Classifier Votes in Data Streams](https://arxiv.org/abs/2511.21465)
*Enes Bektas,Fazli Can*

Main category: cs.LG

TL;DR: 研究发现通过提高基分类器投票间的线性独立性可以优化集成学习性能，提出了基于线性独立性的集成规模理论框架，并在真实和合成数据集上验证了该理论的有效性


<details>
  <summary>Details</summary>
Motivation: 集成学习中增加分类器数量通常能提升准确率，但过大集成会导致计算效率低下和收益递减。本研究旨在探索集成规模与性能的关系

Method: 从数据流中分类器投票的线性独立性角度分析，提出线性独立分类器构成集成能最大化表征能力。通过建模分类器输出线性独立的概率，推导出集成规模与准确率的理论框架

Result: 在真实和合成数据集上使用OzaBagging和GOOWE两种集成方法的实验表明，理论估计能有效识别OzaBagging的性能饱和点，而对GOOWE等复杂加权方案，高理论多样性可能引发算法不稳定性

Conclusion: 提出的理论框架成功解释了集成规模与准确性之间的权衡关系，为确定最优集成规模提供了理论基础
...
TL;DR: IntAttention提出了一种完全整数化的注意力机制，避免了传统INT8量化中softmax阶段的浮点运算开销，在边缘设备上实现了显著的加速和能耗降低。


<details>
  <summary>Details</summary>
Motivation: 边缘设备部署Transformer模型受限于延迟和能耗预算。INT8量化虽然能加速矩阵乘法，但softmax操作成为主要瓶颈，需要进行昂贵的解量化-softmax-再量化操作，影响端到端整数数据流效率。

Method: 提出IntAttention方法，核心是IndexSoftmax算子，在整数域内完全替换浮点指数运算。结合稀疏感知裁剪、32条目查找表近似和直接整数归一化，消除所有数据类型转换开销。

Result: 在Armv8 CPU上，相比FP16基线可实现3.7倍加速和61%能耗降低，相比传统INT8注意力管道快2.0倍。在不同语言和视觉模型中保持与基线相当的高精度保真度。

Conclusion: IntAttention实现了完全整数化、无需重新训练即插即用的注意力管道，为商品化边缘设备提供了实用高效的Transformer推理方案。
...
TL;DR: 该论文将NLP中的机制可解释性技术应用于时间序列分类的Transformer模型，通过激活修补、注意力显著性和稀疏自编码器揭示了模型的内部因果结构和决策机制。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在时间序列分类中表现出色，但其内部决策机制复杂难懂，现有解释方法多关注输入输出归因，忽视了内部机制的理解，因此需要专门针对时间序列Transformer的机制解释方法。

Method: 将NLP领域的机制可解释性技术（激活修补、注意力显著性和稀疏自编码器）应用于专为时间序列分类设计的Transformer架构，系统探测注意力头和时间步的内部因果作用。

Result: 在基准时间序列数据集上构建了因果图，揭示了信息在模型内部的传播方式，识别出驱动正确分类的关键注意力头和时序位置，并展示了稀疏自编码器在发现可解释潜在特征方面的潜力。

Conclusion: 研究为Transformer可解释性贡献了方法论，并对时间序列分类中Transformer性能的功能机制提供了新见解，有助于更好地理解这些先进模型的内部工作原理。
...
TL;DR: 本文提出一种分析因果图变化的框架，重点解决时空数据中的空间异质性和时间非平稳性问题，通过改进基于约束的因果发现方法，构建模块化的自适应因果发现系统。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据（如气候数据）通常表现出空间异质性和时间非平稳性，这些变化不仅包含重要信息，而且可能破坏基于平稳性假设的因果发现算法的稳定性和有效性。

Method: 在独立性检验层面修改基于约束的因果发现方法，构建模块化框架。该框架可兼容现有因果发现算法（如PC、FCI、PCMCI等），并将问题分解为更易处理的子问题，借鉴变点检测、聚类等方法。

Result: 开发出极其模块化、易于扩展且广泛适用的框架，能够利用现有因果发现方法而几乎无需修改，同时便于理解基本限制、超参数权衡和结果的统计解释。

Conclusion: 该框架成功解决了因果图变化分析的核心挑战，通过系统性分解问题实现了算法稳定性和可解释性的提升，为处理非平稳时空数据提供了有效工具。
...
### [177] [A decoupled alignment kernel for peptide membrane permeability predictions](https://arxiv.org/abs/2511.21566)
*Ali Amirahmadi,Gökçe Geylan,Leonardo De Maria,Farzaneh Etminani,Mattias Ohlsson,Alessandro Tibo*

Main category: cs.LG

TL;DR: 该论文提出了一种单体感知解耦全局对齐核（MD-GAK）及其变种PMD-GAK，用于预测环肽的细胞膜通透性，在不确定性估计方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 环肽是靶向细胞内位点的有前景模式，但细胞膜通透性仍然是关键瓶颈，且现有公开数据有限且需要良好校准的不确定性估计。

Method: 提出了MD-GAK核方法，将具有化学意义的残基-残基相似性与序列对齐耦合，同时从间隙惩罚中解耦局部匹配；还引入变体PMD-GAK加入三角位置先验；使用高斯过程作为预测模型。

Result: 实验表明，该方法在全面比较中优于现有最先进模型，且在减少校准误差方面PMD-GAK相比MD-GAK具有额外优势。

Conclusion: MD-GAK和PMD-GAK在环肽细胞膜通透性预测中表现出色，特别是在不确定性估计方面优于复杂深度学习架构。
